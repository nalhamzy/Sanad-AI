...
describe('runTurn() — launch service happy path', () => {
  ...
  test('submitted request lands in DB with all FKs + 3 documents', async () => {
    ...
    // fee_omr is intentionally null for launch services where the catalogue
    // doesn't pin a number — the receiving office quotes the gov fee. Just
    // assert the column was populated either with a number or explicit null.
    assert.ok(r.fee_omr === null || r.fee_omr > 0, 'fee should be number or null');
