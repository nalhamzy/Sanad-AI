...
export const TOOL_IMPL_V2 = {
  ...
  async start_submission(ctx, { service_id }) {
    ...
    // If a previous DRAFT (no real DB request_id yet) is in flight, just
    // overwrite it — the user is starting a new service. Don't make them
    // type "/reset" first. We only refuse to clobber a request that's
    // already been submitted (status >= queued) or has a real request_id;
    // those need an explicit cancel flow.
    const draftStates = new Set(['idle', 'confirming', 'collecting', 'reviewing']);
    const hasLiveRequest = ctx.state.request_id && !draftStates.has(ctx.state.status);
    if (hasLiveRequest) {
      return {
        ok: false,
        error: 'live_request_in_flight',
        request_id: ctx.state.request_id,
        status: ctx.state.status,
        hint: 'Ask the user whether to cancel the in-flight request first, or work with it.'
      };
    }
    const replaced_draft = ctx.state.service_id && ctx.state.service_id !== svc.id
      ? { service_id: ctx.state.service_id, status: ctx.state.status } : null;
