...
export const TOOL_SPEC = [
  {
    ...
    function: {
      ...
      description: 'Search the Oman government services catalogue (453 services across MM, MOH, MOL, MOHUP, MTCIT, MOC, ROP). Returns best matches with id, name, entity, fee. Use this FIRST any time the user asks about a service.',
      parameters: {
        type: 'object',
        properties: {
...
export const TOOL_IMPL = {
  ...
  async get_service_details({ service_id }) {
    ...
    let docs = [];
    try { docs = JSON.parse(s.required_documents_json || '[]'); enrichDocsWithArabicLabels(docs); } catch {}
  ...
  async get_entity_services({ entity, limit = 10 }) {
    ...
    // Citizen-facing: only ACTIVE/verified services (parity with search and
    // the V2 tool). Inactive catalogue rows are never advertised to citizens;
    // an office activates one by assigning it to a request (reclassify).
    const { rows } = await db.execute({
  ...
  async get_request_status({ request_id }) {
    // ANONYMITY: never expose office identity to the citizen-side caller.
    // We expose office_id (boolean test for "is it claimed yet?") but no name.
    const { rows } = await db.execute({
...
export const TOOL_SPEC_V2 = [
  {
    ...
    function: {
      name: 'search_services',
      description: 'Hybrid search (FTS5 keywords + Qwen 1024-dim semantic embeddings + RRF fusion) over the 453-row scraped catalogue covering Muscat Municipality (MM), Royal Oman Police (ROP), Ministry of Health (MOH), Ministry of Labour (MOL), Ministry of Housing & Urban Planning (MOHUP), Ministry of Commerce (MOC), and Ministry of Transport, Communications & IT (MTCIT). Use this whenever the user asks about ANY service. You can combine the free-text query with structured filters. The query language can be Arabic OR English regardless of catalogue language — the semantic lane is bilingual.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      name: 'start_submission',
      description: 'Begin gathering documents for a service. Goes straight from idle/draft to COLLECTING — your reply that turn should announce the service in one short line and ask for the FIRST document. Do not double-prompt. Auto-discards any previous draft (idle/confirming/collecting/reviewing). Refuses if a real request was already submitted (queued+) — handle that with cancel_request first.',
      parameters: {
        type: 'object',
        properties: { service_id: { type: 'integer' } },
        required: ['service_id']
  ...
  },
  {
    ...
    function: {
      ...
      description: 'DEPRECATED no-op kept for compatibility. start_submission now moves directly to COLLECTING; do not call this.',
      parameters: { type: 'object', properties: {} }
    }
  },
  {
    ...
    function: {
      name: 'discard_draft',
      description: 'Throw away the current in-progress draft (service_id + collected docs) so the user can start something new. Only use when the user clearly switches topics mid-flow ("forget this, I want X instead"). Will refuse if a real request has already been submitted.',
      parameters: { type: 'object', properties: {} }
    }
  },
  {
    ...
    function: {
      ...
      description: 'Mark one REQUIRED document as provided. Use this when a file is uploaded OR when the citizen types a caption confirming they sent a specific doc. Only for slots that appear in the service\'s required_documents list. For supplementary/extra files the citizen wants to attach (not in the required list), use record_extra_document instead.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    type: 'function',
    function: {
      name: 'record_extra_document',
      description: 'Attach a SUPPLEMENTARY (non-required) file the citizen wants to include with the request — e.g. a backup ID copy, a supporting letter, proof of payment, a previous-rejection letter. Only call after the citizen explicitly confirms it is an extra/optional file (not the next required document). The file is stored alongside the required docs and dispatched to the office. Allowed in collecting and reviewing states only.',
      parameters: {
        type: 'object',
        properties: {
          caption: { type: 'string', description: 'The user\'s caption or short label for what this file is.' },
          original_name: { type: 'string', description: 'Original filename if provided by the upload.' }
        },
        required: ['caption']
      }
    }
  ...
  },
  {
    ...
    function: {
      name: 'submit_triage',
      description: 'Submit a request when the citizen is UNSURE which service they need (or no service clearly matches). Dispatches to a Sanad office with NO service set and a short free-text summary of what they need; the office reviews it, sets the right service, and continues. Use this instead of guessing a service. Include any papers the citizen already sent — they ride along automatically.',
      parameters: {
        type: 'object',
        properties: {
          intent_summary: {
            type: 'string',
            description: 'A concise 1–2 sentence summary, in Arabic, of what the citizen needs in their own words (e.g. "يريد تجديد وثيقة تخص سيارته لكنه غير متأكد من نوع الخدمة").'
          }
        },
...
async function _ensureCitizen(phone) {
  ...
  // Canonicalize so the WhatsApp `from` (96892888715) and the web OTP phone
  // (+96892888715) resolve to the SAME citizen row.
  const p = canonicalPhone(phone) || phone;
  const { rows } = await db.execute({ sql: `SELECT id FROM citizen WHERE phone=?`, args: [p] });
  if (rows.length) return rows[0].id;
  const r = await db.execute({ sql: `INSERT INTO citizen(phone) VALUES (?)`, args: [p] });
  return Number(r.lastInsertRowid);
}

// codex iter-13: filter out catalog "documents" that are actually
// payment / process steps masquerading as docs. Bench scenario #4 surfaced
// "دفع الرسوم المطلوبة" as a required document for passport issuance —
// it's a payment step, not something the citizen uploads. Strip these
// before they reach the checklist or vision classifier.
const NON_DOC_LABEL_RE = /(?:دفع\s*الرسوم|دفع\s*المبلغ|سداد\s*الرسوم|payment\s*of\s*fees?|fee\s*payment|pay\s*the\s*fees?)/i;
function _isPaymentStep(d) {
  const labels = [d?.label_ar, d?.label_en, d?.code].filter(Boolean).join(' ');
  return NON_DOC_LABEL_RE.test(labels);
}
function _parseDocs(row) {
  try {
    const docs = JSON.parse(row.required_documents_json || '[]');
    if (!Array.isArray(docs)) return [];
    const filtered = docs.filter(d => !_isPaymentStep(d));
    // Backfill Arabic labels — most catalog rows have label_ar='' from
    // the CSV import, so without this the agent renders "Civil ID" etc.
    return enrichDocsWithArabicLabels(filtered);
  } catch { return []; }
}
...
export const TOOL_IMPL_V2 = {
  ...
  async search_services(ctx, args) {
    ...
    // Annotate fee status so the LLM cannot rationalise away null fees as
    // "missing in tool, fill from training data". The only safe response when
    // fee_status === "NOT_IN_CATALOGUE" is to tell the user it isn't listed.
    const annotated = services.map(s => ({
      ...s,
      fee_status: (s.fee_omr == null) ? 'NOT_IN_CATALOGUE_DO_NOT_INVENT_A_NUMBER' : 'AVAILABLE',
    }));
    return {
      ok: true, count, query, filters: filterClean,
      services: annotated,
      reminder: 'Quote name_en/name_ar VERBATIM. If fee_status is NOT_IN_CATALOGUE_DO_NOT_INVENT_A_NUMBER, do not write any number. Call get_service_details(service_id) before listing required documents.'
    };
  },
  ...
  async get_service_details(ctx, { service_id }) {
    ...
    const docs = _parseDocs(svc);
    const steps = _parseSteps(svc);
    const feeStatus = (svc.fee_omr == null && !svc.fees_text)
      ? 'NOT_IN_CATALOGUE_DO_NOT_INVENT_A_NUMBER'
      : 'AVAILABLE';
    const docsStatus = docs.length === 0 ? 'NOT_IN_CATALOGUE_DO_NOT_INVENT_DOCUMENTS' : 'AVAILABLE';
    const timeStatus = (!svc.avg_time_en && !svc.avg_time_ar) ? 'NOT_IN_CATALOGUE' : 'AVAILABLE';
    return {
      ok: true,
      reminder: 'Quote name_en/name_ar verbatim. If fee_status/docs_status/time_status is NOT_IN_CATALOGUE, do not invent. Use the canonical "not listed" wording from the system prompt.',
      service: {
        id: svc.id,
  ...
  async start_submission(ctx, { service_id }) {
    ...
    // GUARD: a citizen may only START a REAL, ACTIVE catalogue service. This
    // stops the agent from launching a fabricated/hallucinated or inactive
    // legacy entry (which dead-ends at "غير متوفرة"). The tool returns the active
    // alternatives so the agent re-offers a real service instead of inventing one.
    if (Number(svc.is_active) !== 1) {
      return { ok: false, error: 'service_inactive', service_id: svc.id, name_ar: svc.name_ar };
    }

    // Two cases the user can be in when starting a submission:
    //   (a) A DRAFT is in progress (status in {idle, confirming, collecting,
    //       reviewing}) — no DB request yet. We overwrite the draft.
    //   (b) A real request was already submitted (status in {queued,
    //       claimed, in_progress, completed}) — that request stays in the
    //       DB untouched; we just clear the session draft to start a new
    //       independent submission for the new service.
    // Either way, never refuse — the user wants to start something new.
    const draftStates = new Set(['idle', 'confirming', 'collecting', 'reviewing']);
    const hadPrevRequest = ctx.state.request_id && !draftStates.has(ctx.state.status);
    const replaced_draft = ctx.state.service_id && ctx.state.service_id !== svc.id
      ? { service_id: ctx.state.service_id, status: ctx.state.status, prev_request_id: ctx.state.request_id || null } : null;

    // Skip the legacy "confirming" intermediate — go straight to collecting.
    // The LLM is expected to confirm + ask for the first doc in a single
    // reply, so there's no value in a separate state hop.
    ...
    ctx.state.request_id = null; // any prior draft is invalidated
    const docs = ctx.state.docs;

    // ── Attach pre-service uploads ────────────────────────────────────────
    // If the citizen sent files BEFORE choosing a service (buffered in
    // pending_uploads by the pre-service interceptor in agent.js), attach them
    // now in declaration order so they aren't orphaned. Surplus → extras.
    const buffered = Array.isArray(ctx.state.pending_uploads) ? ctx.state.pending_uploads : [];
    let prefilledCount = 0;
    if (buffered.length) {
      let i = 0;
      for (; i < buffered.length; i++) {
        const target = docs.find(d => d.code && !ctx.state.collected[d.code]);
        if (!target) break;
        const up = buffered[i];
        ctx.state.collected[target.code] = {
          filename: up.name || null, storage_url: up.url, mime: up.mime || null,
          size_bytes: null, caption: up.caption || null, at: Date.now()
        };
        prefilledCount++;
      }
      const surplus = buffered.slice(i);
      if (surplus.length) {
        ctx.state.extras = Array.isArray(ctx.state.extras) ? ctx.state.extras : [];
        for (const up of surplus) ctx.state.extras.push({
          idx: up.idx, url: up.url, name: up.name || null, mime: up.mime || null,
          caption: up.caption || up.name || null, at: Date.now()
        });
      }
      ctx.state.pending_uploads = [];
      let pi = 0;
      while (pi < docs.length && ctx.state.collected[docs[pi].code]) pi++;
      ctx.state.pending_doc_index = pi;
    }

    const firstUnfilled = docs.find(d => !ctx.state.collected[d.code]) || null;
    const allFilled = docs.length > 0 && !firstUnfilled;
    if (allFilled) ctx.state.status = 'reviewing';
    ...
    return {
      ...
      name_ar: svc.name_ar,
      entity_en: svc.entity_en,
      entity_ar: svc.entity_ar,
      fee_omr: svc.fee_omr,
      fees_text: svc.fees_text,
      required_documents: docs.map(d => ({ code: d.code, label_en: d.label_en, label_ar: d.label_ar })),
      total_docs: docs.length,
      prefilled_count: prefilledCount,
      first_doc: firstUnfilled,
      all_docs_filled: allFilled,
      replaced_draft,
      previously_submitted_request: hadPrevRequest ? replaced_draft?.prev_request_id || null : null,
      next: allFilled ? 'all_docs_received_confirm' : 'ask_for_first_document',
      transition: ctx.state.status
    };
  },

  // Kept for backwards compatibility — start_submission now goes straight
  // to collecting, so confirm_submission is a no-op that just reports the
  // current state. Old prompts / tests that still call it won't break.
  async confirm_submission(ctx) {
    if (!['confirming', 'collecting'].includes(ctx.state.status)) {
      return { ok: false, error: `not_ready (state=${ctx.state.status})` };
    }
    ctx.state.status = 'collecting';
    ctx.state.pending_doc_index = ctx.state.pending_doc_index || 0;
    const docs = ctx.state.docs || [];
    return {
      ok: true,
      transition: 'collecting',
      total_docs: docs.length,
      first_doc: docs[ctx.state.pending_doc_index] || null,
      already_collecting: true
    };
  },

  // Discard the current in-progress draft so a new service can start
  // cleanly. Refuses to touch a request that has already been submitted.
  async discard_draft(ctx) {
    if (ctx.state.request_id && !['idle','confirming','collecting','reviewing'].includes(ctx.state.status)) {
      return { ok: false, error: 'live_request', request_id: ctx.state.request_id, hint: 'use cancel_request' };
    }
    const extrasCount = Array.isArray(ctx.state.extras) ? ctx.state.extras.length : 0;
    const cleared = {
      service_id: ctx.state.service_id || null,
      status: ctx.state.status,
      docs_collected: Object.keys(ctx.state.collected || {}).length,
      extras_dropped: extrasCount
    };
    ctx.state.status = 'idle';
    ctx.state.service_id = null;
    ctx.state.service_code = null;
    ctx.state.collected = {};
    ctx.state.pending_doc_index = 0;
    ctx.state.docs = null;
    ctx.state.extras = [];
    ctx.state.request_id = null;
    return { ok: true, cleared, transition: 'idle' };
  },
  ...
  async record_document(ctx, { doc_code, filename, caption }) {
    ...
    // STATE-CORRUPTION GUARD (2026-05-06, found in trace +96892888715 #1208):
    // The LLM was calling record_document without a real file URL, leaving
    // state.collected[civil_id] with storage_url=null. Officer dashboard then
    // shows the slot as "filled" with a broken link. The required-doc flow
    // requires a real file — refuse the call when one isn't attached AND the
    // slot isn't already filled. Don't refuse on a buffered re-record (extras-
    // flush path) because those carry the URL via a synthesized ctx.attachment.
    if (!att?.url && !ctx.state.collected?.[doc_code]?.storage_url) {
      return {
        ok: false,
        error: 'no_file_attached',
        hint: `record_document requires a real file upload. Use record_extra_document for caption-only notes, or wait for the citizen to send the file.`
      };
    }
  ...
  async record_extra_document(ctx, { caption, original_name }) {
    if (!['collecting', 'reviewing'].includes(ctx.state.status)) {
      return { ok: false, error: `not_collecting (state=${ctx.state.status})` };
    }
    // Synthesize a caption when the LLM doesn't supply one — preserving
    // the file is more important than failing the call. Mirrors what the
    // auto-extra path already does at agent.js (vision_extra branch).
    const att = ctx.attachment || null;
    const cap = (caption || original_name || att?.name || 'attached file').toString().trim()
      || 'attached file';

    ctx.state.extras = Array.isArray(ctx.state.extras) ? ctx.state.extras : [];
    ctx.state.extras.push({
      caption: cap,
      original_name: original_name || att?.name || null,
      storage_url: att?.url || null,
      mime: att?.mime || null,
      size_bytes: att?.size || null,
      at: Date.now()
    });
    // Mark attachment consumed so the same upload isn't recorded twice if the
    // LLM loops and tries record_document afterwards.
    if (att) ctx.attachment = null;

    return {
      ok: true,
      extra_index: ctx.state.extras.length,
      extra_count: ctx.state.extras.length,
      has_file: !!att?.url,
      // Extras don't change which slot is next — the required flow continues.
      pending_required: ctx.state.docs?.[ctx.state.pending_doc_index ?? 0]?.code || null
    };
  },
  ...
  async submit_request(ctx) {
    ...
    // Supplementary / extra files dispatched alongside the required docs.
    const extras = Array.isArray(ctx.state.extras) ? ctx.state.extras : [];
    for (let i = 0; i < extras.length; i++) {
      const e = extras[i];
      await db.execute({
        sql: `INSERT INTO request_document(request_id,doc_code,label,storage_url,mime,size_bytes,status,caption,matched_via,original_name,is_extra,note)
              VALUES (?,?,?,?,?,?,'pending',?,?,?,1,?)`,
        args: [request_id, `extra_${i + 1}`, e.caption || `Extra file ${i + 1}`,
               e.storage_url || null, e.mime || null, e.size_bytes || null,
               e.caption || null, 'extra', e.original_name || null,
               e.caption || null]
      });
    }
    ...
    return {
      ...
      request_id,
      fee_omr: svc.fee_omr,
      docs_filed: docs.filter(d => ctx.state.collected?.[d.code]).length,
      extras_filed: extras.length,
      transition: 'queued'
    };
  },

  // ── Triage submit — citizen is UNSURE which service applies ────────────────
  // Dispatches a request with NO service (service_id=NULL) carrying a free-text
  // summary of what the citizen needs + any optional papers they sent. It lands
  // in the marketplace; the claiming office reads the summary and sets the real
  // service via reclassify, then continues the normal flow. No fee/checklist yet
  // — those come once the office picks the service.
  async submit_triage(ctx, { intent_summary } = {}) {
    const summary = String(intent_summary || '').trim().slice(0, 1000);
    if (!summary) return { ok: false, error: 'no_summary' };
    const citizen_id = await _ensureCitizen(ctx.citizen_phone);
    const ins = await db.execute({
      sql: `INSERT INTO request(session_id,citizen_id,service_id,status,fee_omr,governorate,intent_summary,state_json)
            VALUES (?,?, NULL, 'ready', NULL, 'Muscat', ?, ?)`,
      args: [ctx.session_id, citizen_id, summary, JSON.stringify(ctx.state)]
    });
    const request_id = Number(ins.lastInsertRowid);

    // Attach any optional papers the citizen sent (buffered in pending_uploads
    // since no service/checklist was active) as supplementary docs.
    const ups = Array.isArray(ctx.state.pending_uploads) ? ctx.state.pending_uploads : [];
    for (let i = 0; i < ups.length; i++) {
      const e = ups[i];
      await db.execute({
        sql: `INSERT INTO request_document(request_id,doc_code,label,storage_url,mime,size_bytes,status,caption,matched_via,original_name,is_extra,note)
              VALUES (?,?,?,?,?,?,'pending',?,?,?,1,?)`,
        args: [request_id, `extra_${i + 1}`, e.caption || e.name || `مرفق ${i + 1}`,
               e.url || null, e.mime || null, null,
               e.caption || null, 'triage', e.name || null, e.caption || null]
      });
    }

    ctx.state.status = 'queued';
    ctx.state.request_id = request_id;
    ctx.state.intent_summary = summary;
    ctx.state.pending_uploads = [];
    return {
      ok: true,
      request_id,
      triage: true,
      attachments_filed: ups.length,
      transition: 'queued',
      next: 'office_will_set_service'
    };
  },
  ...
  async get_my_requests(ctx, { status } = {}) {
    ...
    // ANONYMITY: never expose office identity to citizen-side tools. office_id
    // alone is fine (lets the LLM know "is it claimed yet?") but no name.
    const { rows } = await db.execute({
  ...
  async get_request_status(ctx, { request_id }) {
    ...
    // ANONYMITY: no office_name / office_rating — the citizen-side LLM must
    // not learn (and therefore can't surface) office identity.
    const { rows } = await db.execute({
      sql: `SELECT r.id, r.status, r.fee_omr, r.created_at, r.claimed_at, r.completed_at,
                   r.cancel_requested, r.cancelled_at, r.office_id,
                   s.name_en AS service_name, s.name_ar AS service_name_ar
              FROM request r
              LEFT JOIN service_catalog s ON s.id=r.service_id
             WHERE r.id=?`,
  ...
  async list_offers(ctx, { request_id }) {
    // ANONYMITY: this tool is used by the citizen-side LLM. Strip office name
    // and rating — under v3, citizens never pick offices anyway. Endpoint shell
    // preserved until full deletion in a later migration step.
    const { rows } = await db.execute({
