...
// ────────────────────────────────────────────────────────────
// Send a MEDIA message by public link — used to push an office-issued
// document (the deliverable, e.g. a renewed CR) straight into the citizen's
// WhatsApp thread. Meta fetches the file from `link`, which MUST be a public
// https URL (our /uploads static mount qualifies). Images go as type:image
// (inline preview), everything else as type:document (filename + caption).
//
// Same auth + stub semantics as the other senders: no creds → log + stub.
//
// @param {string} toPhone — E.164 (with or without '+').
// @param {string} link — public https URL Meta can GET (e.g. saned.ai/uploads/…).
// @param {object} [opts]
// @param {string} [opts.filename] — shown to the recipient for documents.
// @param {string} [opts.caption]  — short caption (≤ 1024).
// @param {string} [opts.mime]     — used to decide image vs document.
export async function sendWhatsAppDocument(toPhone, link, { filename = 'document', caption = '', mime = '' } = {}) {
  const to = normalisePhone(toPhone);
  if (!to) return { ok: false, error: 'no_phone', channel: 'whatsapp' };
  if (!/^https:\/\//.test(link || '')) return { ok: false, error: 'bad_link', channel: 'whatsapp' };

  // Inline-preview images; send everything else as a downloadable document.
  const isImage = /^image\//i.test(mime) || /\.(jpe?g|png|webp|gif)(\?|$)/i.test(link);
  const cap = String(caption || '').slice(0, 1024);

  if (!WHATSAPP_ENABLED) {
    console.log(`[wa:stub:media] to=${to} ${isImage ? 'image' : 'document'} link=${link}`);
    return { ok: true, channel: 'stub', stub: true };
  }

  const media = isImage
    ? { type: 'image',    image:    { link, ...(cap ? { caption: cap } : {}) } }
    : { type: 'document', document: { link, filename: String(filename || 'document').slice(0, 240), ...(cap ? { caption: cap } : {}) } };

  try {
    const r = await fetch(`https://graph.facebook.com/v20.0/${PHONE_NUMBER_ID}/messages`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'authorization': `Bearer ${ACCESS_TOKEN}` },
      body: JSON.stringify({ messaging_product: 'whatsapp', recipient_type: 'individual', to, ...media })
    });
    if (!r.ok) {
      const txt = await r.text();
      console.warn('[wa:media]', `${r.status}: ${txt.slice(0, 200)}`);
      return { ok: false, error: `wa-media ${r.status}: ${txt.slice(0, 200)}`, channel: 'whatsapp' };
    }
    const data = await r.json();
    return { ok: true, channel: 'whatsapp', message_id: data?.messages?.[0]?.id };
  } catch (e) {
    console.warn('[wa:media] threw:', e.message);
    return { ok: false, error: e.message, channel: 'whatsapp' };
  }
}
