...
// ─── Embeddings ─────────────────────────────────────────────
// Batched embeddings for the service catalogue. Returns an array of
// number[] (float vectors) aligned with `inputs`. Null-safe: when the LLM is
// disabled we return null so callers can fall back to FTS-only search.
export async function embed(inputs, { trace } = {}) {
  if (!LLM_ENABLED) return null;
  if (!Array.isArray(inputs) || inputs.length === 0) return [];
  const batchSize = 10;          // DashScope limit for text-embedding-v3
