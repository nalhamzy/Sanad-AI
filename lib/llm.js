...
// Use override:true so .env values win over a stale/empty shell env
// (some local shells inherit empty exports of these vars from a previous
// session, which would otherwise shadow the real key in .env). On Render
// there is no .env file, so this is a no-op there.
//
// Exception: in tests we explicitly stub QWEN_API_KEY/ANTHROPIC_API_KEY=''
// before importing this module — overriding from .env would un-stub them
// and let tests hit a real LLM. Skip dotenv entirely when NODE_ENV=test.
...
// ─── Provider config ────────────────────────────────────────
// Default agent provider: ANTHROPIC (Claude). Picked because Claude follows
// the per-turn word/line caps + "do not call tools" instructions in our
// system prompt much more reliably than qwen-plus, which is the chief
// reason behind the "verbose, asks per-file" reports we got from prod
// traces. Embeddings still go to Qwen (Anthropic has no embed API).
//
// Explicit override: LLM_PROVIDER=anthropic|qwen
// Auto: Anthropic if its key is set, else Qwen (kept as a graceful
// fallback in dev / local where someone may not have a Claude key).
// Production fail-fast: if LLM_PROVIDER='anthropic' but the key is missing,
// boot throws — better than silently degrading to Qwen and getting the
// same verbose-reply complaints back.
const ANTHROPIC_API_KEY = process.env.ANTHROPIC_API_KEY || '';
// Sonnet over Opus is intentional: ~3× faster + ~5× cheaper per token,
// and the per-turn quality difference for citizen-chat-with-tools is
// negligible — Sonnet 4.5 follows the system prompt's word/line caps and
// tool-use rules just as well. Override via ANTHROPIC_MODEL if you need
// Opus on a specific deploy.
// Default switched to Opus 4.5 (2026-05-08, per user) — strongest reasoning
// + instruction-following. Set ANTHROPIC_MODEL=claude-sonnet-4-5 in env if
...
const PROVIDER = (process.env.LLM_PROVIDER
  || (ANTHROPIC_API_KEY ? 'anthropic' : (QWEN_API_KEY ? 'qwen' : 'anthropic')))
  .toLowerCase();

// Production fail-fast: production explicitly opts into Anthropic and the
// key must be present. In dev/test we just log a warning so heuristic-mode
// tests (which set both keys to '') can still run.
if (PROVIDER === 'anthropic' && !ANTHROPIC_API_KEY) {
  if (process.env.NODE_ENV === 'production') {
    throw new Error(
      'LLM_PROVIDER=anthropic but ANTHROPIC_API_KEY is not set. ' +
      'Set the key in your environment, or override LLM_PROVIDER=qwen.'
    );
  }
  // dev/local: log once so it's visible without crashing the server
  if (process.env.NODE_ENV !== 'test') {
    console.warn('[llm] PROVIDER=anthropic but ANTHROPIC_API_KEY is empty — LLM calls will be no-ops');
  }
}
...
// Some models (notably Qwen) sometimes emit a tool call as TEXT inside the
// message content — e.g. "<tool_code>{"name":..,"arguments":{..}}</tool_code>"
// — instead of the structured tool_calls field. Left as-is this LEAKS raw
// syntax to the user AND the tool never runs (prod: «خدمات الشرطة» returned the
// literal <tool_code> block). These helpers recover the call so the loop runs it.
export function tryParseToolJson(s) {
  if (!s) return null;
  ...
  try {
    ...
    if (o && typeof o.name === 'string') {
      return { name: o.name, arguments: o.arguments || o.parameters || o.args || {} };
    }
  } catch {}
  return null;
}
export function extractTextToolCalls(content) {
  if (!content || typeof content !== 'string') return { tool_calls: null, content };
  const calls = [];
  const re = /<tool_code>\s*([\s\S]*?)\s*<\/tool_code>/gi;
  let m;
  while ((m = re.exec(content)) !== null) {
    const p = tryParseToolJson(m[1]);
    if (p) calls.push(p);
  }
  let cleaned = content.replace(re, '').trim();
  if (!calls.length) {
    const p = tryParseToolJson(cleaned); // whole content is a bare tool-call JSON
    if (p) { calls.push(p); cleaned = ''; }
  }
  if (!calls.length) return { tool_calls: null, content };
  const tool_calls = calls.map((c, i) => ({
    id: `txt_${Date.now()}_${i}`,
    type: 'function',
    function: { name: c.name, arguments: JSON.stringify(c.arguments || {}) }
  }));
  return { tool_calls, content: cleaned || null };
}
...
async function qwenChatWithTools({ messages, tools, temperature, max_tokens, trace }) {
  ...
  if (!res.ok) {
    ...
    // Arabic-only fallback (codex iter-7 fix). Previous bilingual text
    // "حسناً، دعني أحاول مجدداً. / Let me try again." leaked English mid-Arabic
    // AND gave the citizen no actionable next step. Caller (lib/agent.js)
    // attaches recovery buttons (status:check / service:cancel / discovery)
    // based on state.status when this reply fires.
    return { content: '⚠️ تعذّر الاتصال بالمساعد الذكي مؤقتاً. حاول مرة أخرى بعد لحظات أو استخدم الأزرار للمتابعة.', tool_calls: null };
  }
  const data = await res.json();
  const msg = data?.choices?.[0]?.message || {};
  let content = msg.content || null;
  let tool_calls = (msg.tool_calls && msg.tool_calls.length) ? msg.tool_calls : null;
  // Recover a text-form tool call (Qwen) so the loop executes it + never leaks.
  if (!tool_calls && content) {
    const ex = extractTextToolCalls(content);
    if (ex.tool_calls) {
      tool_calls = ex.tool_calls;
      content = ex.content;
      trace?.push({ step: 'text_tool_call_recovered', names: tool_calls.map(t => t.function?.name) });
    }
  }
  trace?.push({
...
async function anthropicChatWithTools({ messages, tools, temperature, max_tokens, trace }) {
  ...
  // Prompt caching — every tool round in v2 re-sends the same SYSTEM_V2
  // block (~19 KB) and the same long tool-spec list. Anthropic charges 10%
  // of normal input rate on cache hits and reads them in ~50ms instead of
  // re-tokenizing. Mark the system + tools as cacheable; the actual
  // messages array changes per turn so we don't cache it. Cache TTL is
  // 5 min on ephemeral, plenty for a multi-round in-conversation loop.
  // System block becomes a content array so we can attach cache_control.
  const cacheableSystem = system
    ? [{ type: 'text', text: system, cache_control: { type: 'ephemeral' } }]
    : null;
  // Tools array — attach cache_control to the LAST tool entry. Anthropic
  // caches everything up to and including the marked block.
  const cacheableTools = aTools.length
    ? aTools.map((t, i) => i === aTools.length - 1
  ...
  const body = {
    ...
    ...(cacheableTools.length ? { tools: cacheableTools, tool_choice: { type: 'auto', disable_parallel_tool_use: true } } : {})
  };
  ...
  const res = await fetch(`${ANTHROPIC_BASE}/v1/messages`, {
    ...
    headers: {
      ...
      // Older Claude API versions need this beta header. From 2024-11
      // prompt caching is GA and the header is a no-op, but harmless.
  ...
  if (!res.ok) {
    ...
    // Arabic-only fallback (codex iter-7 fix). See note above.
    return { content: '⚠️ تعذّر الاتصال بالمساعد الذكي مؤقتاً. حاول مرة أخرى بعد لحظات أو استخدم الأزرار للمتابعة.', tool_calls: null };
  }
  const data = await res.json();
