...
export function pickButtons(candidates, { max = 3 } = {}) {
  ...
  for (let i = 0; i < slice.length; i++) {
    ...
    const trimmed = name
      ...
      .replace(/^طلب\s+/, '')
      .replace(/^Service\s+/, '');
    // Reserve 3 chars for "1️⃣ " prefix (the glyph is 2 codepoints + space)
    // and leave room for ellipsis. The button() override-truncates anything
    // >20 chars so this is defense-in-depth.
    const room = BUTTON_TITLE_MAX - NUM_GLYPHS[i].length - 1;
    const truncated = trimmed.length > room
      ? trimmed.slice(0, room - 1) + '…'
      : trimmed;
    const id = `pick:${i + 1}`;
    const title = trimmed
      ? `${NUM_GLYPHS[i]} ${truncated}`
      : BUTTON_LABELS[id]; // fallback to canonical "1️⃣ ابدأ هذا"
    out.push({ id, title });
  }
  return out;
}
