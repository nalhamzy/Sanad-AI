...
// ── Turso/libSQL HTTP-driver compatibility shim ──────────────────────────
// The local-file driver tolerates a statement object that omits `args`
// (`db.execute({ sql })`); the remote Turso HTTP/hrana driver does NOT — it
// runs Object.entries(stmt.args) with no null-guard and throws "Cannot convert
// undefined or null to object". Many call sites + seed scripts use the no-args
// object form, so instead of auditing each, we normalize here once: every
// object-form statement is given an empty args array (bare-string statements
// already work). This lets the identical codebase run on a local SQLite file
// in dev and a remote Turso DB in prod.
const _normStmt = (s) =>
  (s && typeof s === 'object' && !Array.isArray(s) &&
   typeof s.sql === 'string' && s.args == null)
    ? { ...s, args: [] }
    : s;
const _rawExecute = db.execute.bind(db);
const _rawBatch = db.batch.bind(db);
db.execute = (stmt, ...rest) => _rawExecute(_normStmt(stmt), ...rest);
db.batch = (stmts, ...rest) =>
  _rawBatch(Array.isArray(stmts) ? stmts.map(_normStmt) : stmts, ...rest);
...
export async function migrate() {
  ...
  try {
    ...
    await addIfMissing('is_issued',     'is_issued INTEGER DEFAULT 0');  // 1 = office-issued result/deliverable (e.g. the new CR), not a citizen requirement
  ...
  // ─── Request v3: single-claim flow + payment gate ────────
  // Replaces the old multi-office bidding marketplace. Pricing is now
  // catalogue-defined (office.default_office_fee_omr + service_catalog.fee_omr,
  // optionally overridden in office_service_price). Once an office claims, no
  // other office can. The office reviews docs but cannot see chat history
  // until the citizen pays — paid_at unlocks chat visibility.
  try {
    const { rows: rCols } = await db.execute(`SELECT name FROM pragma_table_info('request')`);
    const rNames = new Set(rCols.map(c => c.name));
    const addReq = async (col, ddl) => { if (!rNames.has(col)) await db.execute(`ALTER TABLE request ADD COLUMN ${ddl}`); };
    await addReq('payment_status',  "payment_status TEXT DEFAULT 'none'");   // none | awaiting | paid | refunded
    await addReq('payment_link',    'payment_link TEXT');                     // Amwal hosted-payment URL or stub
    await addReq('payment_ref',     'payment_ref TEXT');                      // gateway reference
    await addReq('payment_amount_omr', 'payment_amount_omr REAL');            // total locked at claim time
    await addReq('paid_at',         'paid_at TEXT');                          // unlocks office ↔ citizen chat
    await addReq('released_count',  'released_count INTEGER DEFAULT 0');      // how many offices passed before claim
    await addReq('claim_review_started_at', 'claim_review_started_at TEXT');  // when the claiming office opened review
  } catch (e) { console.warn('request v3 migrate failed:', e.message); }

  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_request_payment ON request(payment_status)`); } catch {}
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_request_office_status ON request(office_id, status)`); } catch {}

  // ─── Citizen auth (web sign-up + WhatsApp OTP + Google) ──
  // The citizen table existed from day-one with just (phone, name, civil_id);
  // we extend it additively so every legacy WhatsApp-only citizen keeps working
  // and can later attach an email / Google identity without a row migration.
  try {
    const { rows: cCols } = await db.execute(`SELECT name FROM pragma_table_info('citizen')`);
    const cNames = new Set(cCols.map(c => c.name));
    const addCit = async (col, ddl) => { if (!cNames.has(col)) await db.execute(`ALTER TABLE citizen ADD COLUMN ${ddl}`); };
    await addCit('email',              'email TEXT');
    await addCit('google_sub',         'google_sub TEXT');     // Google's stable user id (sub claim)
    await addCit('display_name',       'display_name TEXT');   // separate from `name` so Google name doesn't clobber civil-record name
    await addCit('avatar_url',         'avatar_url TEXT');     // from Google profile
    await addCit('phone_verified_at',  'phone_verified_at TEXT');
    await addCit('email_verified_at',  'email_verified_at TEXT');
    await addCit('last_login_at',      'last_login_at TEXT');
    await addCit('signup_source',      "signup_source TEXT DEFAULT 'whatsapp'"); // whatsapp | web_otp | web_google
  } catch (e) { console.warn('citizen migrate failed:', e.message); }

  // Per-phone OTP slots used by web → WhatsApp registration. We hash the code
  // (bcrypt) so a leaked DB doesn't reveal active codes; attempts are capped
  // to 5 to defang brute-force.
  await db.execute(`CREATE TABLE IF NOT EXISTS citizen_otp (
       id INTEGER PRIMARY KEY AUTOINCREMENT,
       phone TEXT NOT NULL,
       ...
       consumed_at TEXT,
       purpose TEXT NOT NULL DEFAULT 'signup',  -- signup | attach_phone
       citizen_id INTEGER REFERENCES citizen(id),
       last_sent_at TEXT NOT NULL DEFAULT (datetime('now')),
       created_at TEXT NOT NULL DEFAULT (datetime('now'))
     )`);
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_citizen_otp_phone ON citizen_otp(phone, created_at DESC)`); } catch {}

  // Office password-reset OTP slots. Same shape as citizen_otp (bcrypt-hashed
  // codes, expiry, attempt cap) but keyed to an officer account. Used by the
  // "forgot password" flow which sends a code to the office's WhatsApp number.
  await db.execute(`CREATE TABLE IF NOT EXISTS password_reset_otp (
       id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_pwreset_officer ON password_reset_otp(officer_id, created_at DESC)`); } catch {}

  // Outbound WhatsApp messages queued because the citizen was OUTSIDE Meta's 24h
  // customer-care window. We send an approved template instead ("you have an
  // update — reply"), queue the real content here, and flush it when the citizen
  // next replies (their inbound message re-opens the window). See lib/wa_templates.js.
  await db.execute(`CREATE TABLE IF NOT EXISTS pending_wa (
       id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_pending_wa_session ON pending_wa(session_id, sent_at)`); } catch {}
  try { await db.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_citizen_email ON citizen(email) WHERE email IS NOT NULL`); } catch {}
  try { await db.execute(`CREATE UNIQUE INDEX IF NOT EXISTS idx_citizen_google ON citizen(google_sub) WHERE google_sub IS NOT NULL`); } catch {}
  ...
  // ─── v3 Marketplace pivot — additive schema (MARKETPLACE_SCALING.md §10) ──
  // Pure ADD migrations — no destructive changes, fully backward-compatible
  // with the v2 marketplace code. New flow ships behind feature flags later.
  //
  // - service_catalog: add office_fee_omr (default 5 at read), keep existing
  //   fee_omr as government_fee semantics (no rename — too much call-site churn,
  //   per Q18 minimize-churn principle).
  // - office: online-status + reputation + capacity columns. credits_remaining
  //   STAYS but is dormant in MVP (no read/write at gates) per Ali's directive.
  // - request: 5-clock anchors (review/payment/docs/hold/refund) + new fields.
  //   Existing claim_review_started_at is the review-clock anchor; we don't
  //   rename it.
  // - New tables: office_release_cooldown (5-min same-office no-reclaim after
  //   3c soft release), refund_request (post-payment cancel decision flow).
  //
  // No DB-level CHECK on status enum — code path manages new values
  // ('awaiting_documents', 'on_hold', 'rejected_by_office', 'refund_pending',
  // 'refunded') alongside the existing v2 set.
  try {
    const { rows: scCols } = await db.execute(`SELECT name FROM pragma_table_info('service_catalog')`);
    const scNames = new Set(scCols.map(c => c.name));
    const addSc = async (col, ddl) => { if (!scNames.has(col)) await db.execute(`ALTER TABLE service_catalog ADD COLUMN ${ddl}`); };
    // Office fee — defaults to 5 OMR at read time via resolveServiceFees().
    // Existing fee_omr column = government fee (parsed from the source CSV).
    await addSc('office_fee_omr', 'office_fee_omr REAL');
    // ── Verification (which services are trusted / fulfillable). Default
    // 'unverified' for the scraped rows; 'office_approved' for office-confirmed
    // services; 'annotator_validated' once a human annotator verifies one.
    // verification_source distinguishes the gold office set from crowd work.
    await addSc('verification_status', "verification_status TEXT DEFAULT 'unverified'");
    await addSc('verification_source', 'verification_source TEXT');   // 'office' | 'annotator' | NULL
    await addSc('verified_at',         'verified_at TEXT');
    // gov_fee_tbd: government fee unknown up front (the office only sees it after
    // logging into the gov portal). When 1, fee_omr stays NULL and the office is
    // prompted to enter the government fee before sending the payment link.
    await addSc('gov_fee_tbd',         'gov_fee_tbd INTEGER DEFAULT 0');
  } catch (e) { console.warn('service_catalog v3 migrate failed:', e.message); }

  try {
    const { rows: oCols } = await db.execute(`SELECT name FROM pragma_table_info('office')`);
    const oNames = new Set(oCols.map(c => c.name));
    const addOff = async (col, ddl) => { if (!oNames.has(col)) await db.execute(`ALTER TABLE office ADD COLUMN ${ddl}`); };
    // Check-in / heartbeat (online-status state machine: online | away | offline)
    await addOff('online_status',          "online_status TEXT DEFAULT 'offline'");
    await addOff('last_heartbeat_at',      'last_heartbeat_at TEXT');
    // Capacity + specialty
    await addOff('specialty_entities',     'specialty_entities TEXT');         // JSON e.g. ["ROP","MOH"]
    await addOff('concurrent_cap',         'concurrent_cap INTEGER DEFAULT 12');
    // Reputation (MVP discipline mechanism — replaces credits at gates)
    await addOff('completion_rate_30d',    'completion_rate_30d REAL');
    await addOff('abandonment_rate_7d',    'abandonment_rate_7d REAL DEFAULT 0');
    await addOff('review_breach_count_7d', 'review_breach_count_7d INTEGER DEFAULT 0');
    await addOff('work_breach_count_7d',   'work_breach_count_7d INTEGER DEFAULT 0');
    await addOff('time_to_payment_p50_min','time_to_payment_p50_min INTEGER');
    await addOff('time_to_complete_p50_min','time_to_complete_p50_min INTEGER');
    await addOff('citizen_rating_avg',     'citizen_rating_avg REAL');
  } catch (e) { console.warn('office v3 migrate failed:', e.message); }

  try {
    const { rows: rCols } = await db.execute(`SELECT name FROM pragma_table_info('request')`);
    const rNames = new Set(rCols.map(c => c.name));
    const addReq = async (col, ddl) => { if (!rNames.has(col)) await db.execute(`ALTER TABLE request ADD COLUMN ${ddl}`); };
    // Clock anchors. claim_review_started_at already exists as the review anchor.
    await addReq('payment_link_sent_at',     'payment_link_sent_at TEXT');     // 15-min citizen payment clock
    await addReq('docs_requested_at',        'docs_requested_at TEXT');        // 24-h citizen doc-fix clock
    await addReq('hold_started_at',          'hold_started_at TEXT');          // 4-h hold cap anchor
    await addReq('hold_reason',              'hold_reason TEXT');              // enum: portal_down|citizen_otp_pending|citizen_unreachable|manual_review_needed|other
    await addReq('hold_paused_remaining_s',  'hold_paused_remaining_s INTEGER'); // work-clock seconds left when hold engaged
    // Reject + refund
    await addReq('reject_reason',            'reject_reason TEXT');            // enum: incomplete_unfixable|wrong_service|suspected_fraud|other
    await addReq('reject_note',              'reject_note TEXT');              // free-text addendum
    await addReq('refund_status',            'refund_status TEXT');            // pending|approved_full|approved_partial|rejected|admin_override
    await addReq('refund_amount_omr',        'refund_amount_omr REAL');
  } catch (e) { console.warn('request v3 migrate failed:', e.message); }

  // 5-min same-office cooldown after 3c soft-release. Lookup is hot-path
  // (every checkout query joins against this), so we index by (office_id, expires_at).
  await db.execute(`CREATE TABLE IF NOT EXISTS office_release_cooldown (
       id INTEGER PRIMARY KEY AUTOINCREMENT,
       office_id INTEGER NOT NULL REFERENCES office(id),
       request_id INTEGER NOT NULL REFERENCES request(id),
  ...
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_cooldown_lookup ON office_release_cooldown(office_id, expires_at)`); } catch {}

  // Post-payment cancel decision flow. One row per refund decision; the
  // request.refund_status / refund_amount_omr columns mirror the latest decision
  // for fast read paths.
  await db.execute(`CREATE TABLE IF NOT EXISTS refund_request (
       id INTEGER PRIMARY KEY AUTOINCREMENT,
       request_id INTEGER NOT NULL REFERENCES request(id),
       ...
       decision_at TEXT,
       decided_by INTEGER,                  -- officer_id or admin_id
       decided_by_kind TEXT,                -- 'officer' | 'admin'
       refund_ref TEXT,                     -- Amwal refund reference (when issued)
       notes TEXT
     )`);
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_refund_request ON refund_request(request_id, decision)`); } catch {}

  // Reputation update helper: a placeholder index on audit_log so future
  // updateOfficeStats() reads (`SELECT COUNT(*) FROM audit_log WHERE action='sla_*' AND target_type='request' …`)
  // are cheap. The actual implementation lives in lib/reputation.js (next migration step).
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_audit_action_target ON audit_log(action, target_type, created_at DESC)`); } catch {}

  // ─── Request flags (multi-office quality vote) ───────────────
  // An office can FLAG a request it sees in the marketplace as wrong-service /
  // unfit / suspicious. The first office that flags only hides the request
  // from its own marketplace. Once the count of distinct offices that flagged
  // reaches FLAG_AUTO_REMOVE_THRESHOLD (default 2), the request is auto-
  // moved to status='flagged' and disappears for everyone — the citizen is
  // then notified and asked to fix or change service. This prevents spam and
  // junk from clogging every office's queue.
  await db.execute(`CREATE TABLE IF NOT EXISTS request_flag (
       id INTEGER PRIMARY KEY AUTOINCREMENT,
       request_id INTEGER NOT NULL REFERENCES request(id),
       office_id  INTEGER NOT NULL REFERENCES office(id),
       officer_id INTEGER REFERENCES officer(id),
       reason     TEXT NOT NULL,                  -- short tag: wrong_service|fake_docs|abusive|other
       note       TEXT,                            -- free-text detail
       created_at TEXT DEFAULT (datetime('now')),
       UNIQUE(request_id, office_id)
     )`);
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_flag_request ON request_flag(request_id)`); } catch {}
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_flag_office  ON request_flag(office_id)`); } catch {}

  // request status widening: 'flagged' added so the auto-quarantine works
  // without touching the CHECK constraint (status is plain TEXT — additive only).

  // ─── Reclassify proposal columns ─────────────────────────────
  // When the office proposes a service change, we DO NOT mutate service_id
  // until the citizen accepts. The pending values live alongside; status
  // flips to 'awaiting_reclassify_ack' until the citizen replies "موافق"
  // or "رفض". Keeps pricing changes consensual.
  try {
    const { rows: rCols } = await db.execute(`SELECT name FROM pragma_table_info('request')`);
    const rNames = new Set(rCols.map(c => c.name));
    const addReq = async (col, ddl) => { if (!rNames.has(col)) await db.execute(`ALTER TABLE request ADD COLUMN ${ddl}`); };
    await addReq('pending_service_id',          'pending_service_id INTEGER');
    await addReq('pending_office_fee_omr',      'pending_office_fee_omr REAL');
    await addReq('pending_government_fee_omr',  'pending_government_fee_omr REAL');
    await addReq('pending_quoted_fee_omr',      'pending_quoted_fee_omr REAL');
    await addReq('pending_reclassify_reason',   'pending_reclassify_reason TEXT');
    await addReq('pending_reclassify_at',       'pending_reclassify_at TEXT');
  } catch (e) { console.warn('reclassify migrate failed:', e.message); }

  // ─── Thawani / multi-provider payment columns ────────────────
  // payment_ref already stores OUR merchant reference. payment_provider
  // identifies who's behind a given payment (thawani | amwal | stub) so the
  // webhook router knows where to verify. payment_session_id holds the
  // gateway's own session id (Thawani: data.session_id, Amwal: orderId)
  // for retrieve / refund calls. raw_webhook_json keeps the last gateway
  // payload for audit.
  try {
    const { rows: rCols } = await db.execute(`SELECT name FROM pragma_table_info('request')`);
    const rNames = new Set(rCols.map(c => c.name));
    const addReq = async (col, ddl) => { if (!rNames.has(col)) await db.execute(`ALTER TABLE request ADD COLUMN ${ddl}`); };
    await addReq('payment_provider',       "payment_provider TEXT");      // thawani | amwal | stub
    await addReq('payment_session_id',     'payment_session_id TEXT');     // gateway's own session id
    await addReq('payment_raw_webhook',    'payment_raw_webhook TEXT');    // last gateway payload (JSON)
    // last_citizen_reply_at: stamp of the most recent inbound citizen turn.
    // last_office_view_at:   stamp of when the assigned officer last opened
    // the request detail. Diffing the two lets the inbox flag "💬 رد جديد"
    // on the card so the office never misses a reply during the multi-day
    // chat threads pre-payment can produce.
    await addReq('last_citizen_reply_at',  'last_citizen_reply_at TEXT');
    await addReq('last_office_view_at',    'last_office_view_at TEXT');
    // Triage intake: when the citizen doesn't know the exact service, the agent
    // submits with service_id=NULL and a free-text summary of their need. The
    // claiming office reads it and sets the real service via reclassify.
    await addReq('intent_summary',         'intent_summary TEXT');
  } catch (e) { console.warn('payment_provider migrate failed:', e.message); }
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_request_pay_session ON request(payment_session_id)`); } catch {}

  // ─── Subscriptions v2: time-based plans (monthly/quarterly/semi-annual/annual) ──
  // The original office_subscription table modeled a one-shot "starter-70" credit
  // pack. v2 introduces time-based access plans:
  //   • months         — plan duration (1/3/6/12)
  //   • starts_at      — when the access period begins (paid_at on first purchase,
  //                       or previous sub's expires_at on renewal)
  //   • expires_at     — when access ends; office.subscription_status flips
  //                       'expired' once now() crosses this + grace period
  //   • renewed_from_id — back-pointer to the previous sub row (for renewal chains)
  //   • thawani_*      — Thawani session/invoice/payment ids for audit + lookup
  //   • auto_renew     — Phase 6 flag; 0 = manual re-purchase, 1 = saved-card charge
  //   • cancelled_at   — admin cancel timestamp
  // Legacy 'starter-70' rows keep working (months/starts_at/expires_at stay NULL).
  try {
    const { rows: sCols } = await db.execute(`SELECT name FROM pragma_table_info('office_subscription')`);
    const sNames = new Set(sCols.map(c => c.name));
    const addSub = async (col, ddl) => { if (!sNames.has(col)) await db.execute(`ALTER TABLE office_subscription ADD COLUMN ${ddl}`); };
    await addSub('months',              'months INTEGER');                  // 1 | 3 | 6 | 12
    await addSub('starts_at',           'starts_at TEXT');                  // datetime; null until paid
    await addSub('expires_at',          'expires_at TEXT');                 // datetime; computed at finalize
    await addSub('renewed_from_id',     'renewed_from_id INTEGER REFERENCES office_subscription(id)');
    await addSub('thawani_session_id',  'thawani_session_id TEXT');         // /api/v1/checkout/session id
    await addSub('thawani_invoice',     'thawani_invoice TEXT');            // gateway invoice ref
    await addSub('thawani_payment_id',  'thawani_payment_id TEXT');         // post-payment payment id
    await addSub('auto_renew',          'auto_renew INTEGER DEFAULT 0');    // Phase 6 toggle
    await addSub('cancelled_at',        'cancelled_at TEXT');
  } catch (e) { console.warn('office_subscription v2 migrate failed:', e.message); }
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_sub_thawani_session ON office_subscription(thawani_session_id)`); } catch {}
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_sub_expires ON office_subscription(expires_at, payment_status)`); } catch {}

  // ─── Office: current plan snapshot ──────────────────────────────
  // Denormalized so dashboards and quota checks don't have to join
  // office_subscription every time. Updated when finalizeSubscriptionPayment
  // marks a new sub active, and again when the expiry watcher flips status.
  //   • current_plan            — 'monthly'|'quarterly'|'semi-annual'|'annual'|null
  //   • subscription_expires_at — mirror of latest active sub's expires_at
  //   • thawani_customer_id     — Thawani /customers id; populated on first save-card opt-in (Phase 6)
  try {
    const { rows: oCols } = await db.execute(`SELECT name FROM pragma_table_info('office')`);
    const oNames = new Set(oCols.map(c => c.name));
    const addOffice = async (col, ddl) => { if (!oNames.has(col)) await db.execute(`ALTER TABLE office ADD COLUMN ${ddl}`); };
    await addOffice('current_plan',            'current_plan TEXT');
    await addOffice('subscription_expires_at', 'subscription_expires_at TEXT');
    await addOffice('thawani_customer_id',     'thawani_customer_id TEXT');
  } catch (e) { console.warn('office subscription-snapshot migrate failed:', e.message); }
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_office_sub_expires ON office(subscription_expires_at)`); } catch {}

  // ─── Payment event log ──────────────────────────────────────────
  // Audit trail of every interaction with Thawani (and any future provider).
  // Drives two things:
  //   1. Idempotency — Thawani webhooks aren't HMAC-signed, so we re-fetch
  //      the session via API on every webhook ping and dedupe on
  //      (thawani_session_id, event_type='paid'). One row per outcome, ever.
  //   2. Admin payment-events feed — last N rows shown in /admin.html for
  //      debugging "where did this payment go".
  // subject_type/subject_id is a polymorphic pointer to office_subscription
  // or request, so we can log both subscription purchases and citizen
  // request payments through the same pipe.
  await db.execute(`CREATE TABLE IF NOT EXISTS payment_event (
       id INTEGER PRIMARY KEY AUTOINCREMENT,
       subject_type TEXT NOT NULL,                -- 'office_subscription' | 'request'
       subject_id   INTEGER NOT NULL,
       provider     TEXT NOT NULL DEFAULT 'thawani',
       thawani_session_id TEXT,
       event_type   TEXT NOT NULL,                -- session_created | webhook_received | fetch_verified | paid | failed | expired | refunded | cancelled
       amount_omr   REAL,
       raw_json     TEXT,
       created_at   TEXT DEFAULT (datetime('now'))
     )`);
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_payment_event_subject ON payment_event(subject_type, subject_id, created_at DESC)`); } catch {}
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_payment_event_session ON payment_event(thawani_session_id)`); } catch {}
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_payment_event_created ON payment_event(created_at DESC)`); } catch {}

  // ─── Auto-renew (Phase 6) ───────────────────────────────────────
  // Thawani's e-commerce API has no native subscription resource. Recurring
  // charges are built on customers + saved payment_methods + payment_intents.
  // Our flow:
  //   1. At first checkout, IF the office opted in to save_card, we create a
  //      Thawani customer (POST /api/v1/customers) and stash the customer_id
  //      on office.thawani_customer_id. Thawani then offers the citizen the
  //      "save card for future payments" checkbox on the hosted page.
  //   2. On finalize, we list the customer's payment_methods (GET) and store
  //      the most recent token on office.thawani_payment_method_id.
  //   3. The sub-watcher's autorenew sweep finds subs whose
  //      next_renewal_at <= now AND auto_renew=1, creates a payment_intent
  //      against the saved method, confirms it, and finalizes a NEW
  //      office_subscription row (the renewal). On failure, schedules a
  //      retry per the dunning policy.
  //
  // Columns added here track the retry schedule and the saved-method token:
  //   • office.thawani_payment_method_id    — last known saved-card token
  //   • office_subscription.renewal_attempt_count — dunning counter (0/1/2/3)
  //   • office_subscription.renewal_next_try_at   — when to try next charge
  //   • office_subscription.renewal_failed_at     — terminal-fail timestamp
  try {
    const { rows: oCols } = await db.execute(`SELECT name FROM pragma_table_info('office')`);
    const oNames = new Set(oCols.map(c => c.name));
    const addOffice = async (col, ddl) => { if (!oNames.has(col)) await db.execute(`ALTER TABLE office ADD COLUMN ${ddl}`); };
    await addOffice('thawani_payment_method_id', 'thawani_payment_method_id TEXT');
  } catch (e) { console.warn('office autorenew migrate failed:', e.message); }

  try {
    const { rows: sCols } = await db.execute(`SELECT name FROM pragma_table_info('office_subscription')`);
    const sNames = new Set(sCols.map(c => c.name));
    const addSub = async (col, ddl) => { if (!sNames.has(col)) await db.execute(`ALTER TABLE office_subscription ADD COLUMN ${ddl}`); };
    await addSub('renewal_attempt_count', 'renewal_attempt_count INTEGER DEFAULT 0');
    await addSub('renewal_next_try_at',   'renewal_next_try_at TEXT');
    await addSub('renewal_failed_at',     'renewal_failed_at TEXT');
  } catch (e) { console.warn('office_subscription autorenew migrate failed:', e.message); }
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_sub_autorenew ON office_subscription(auto_renew, renewal_next_try_at) WHERE auto_renew=1`); } catch {}

  // ─── Office payouts (weekly settlement) ─────────────────────────
  // Sanad-AI collects all citizen payments centrally via Thawani. End-of-week
  // we settle to each office: gross collected − platform fee = net to transfer.
  //
  // Model:
  //   • office_payout — one row per (office, period). Holds the totals, the
  //     pay-out status, the bank reference once transferred, the operator
  //     who clicked "mark paid". 'pending' → 'paid' is the happy path;
  //     'cancelled' releases the included requests back to the eligible pool.
  //   • request.payout_id — back-pointer so we never double-pay a request.
  //     NULL until the admin groups it into a payout via the admin UI.
  //
  // Platform fee (per-request, in baisa) is set via env PLATFORM_FEE_BAISA
  // (default 500 baisa = 0.5 OMR). See lib/payouts.js.
  await db.execute(`CREATE TABLE IF NOT EXISTS office_payout (
       id INTEGER PRIMARY KEY AUTOINCREMENT,
       office_id INTEGER NOT NULL REFERENCES office(id),
       period_start TEXT NOT NULL,                -- YYYY-MM-DD inclusive
       period_end   TEXT NOT NULL,                -- YYYY-MM-DD inclusive
       request_count INTEGER NOT NULL DEFAULT 0,
       gross_omr        REAL NOT NULL DEFAULT 0,   -- sum of citizen payments
       platform_fee_omr REAL NOT NULL DEFAULT 0,   -- request_count * per-request fee
       net_omr          REAL NOT NULL DEFAULT 0,   -- gross - platform_fee
       status TEXT NOT NULL DEFAULT 'pending',    -- pending | paid | cancelled
       paid_at TEXT,
       paid_by_officer_id INTEGER REFERENCES officer(id),
       paid_reference TEXT,                       -- bank txn ref / receipt #
       notes TEXT,
       created_by_officer_id INTEGER REFERENCES officer(id),
       created_at TEXT DEFAULT (datetime('now'))
     )`);
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_payout_office ON office_payout(office_id, period_end DESC)`); } catch {}
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_payout_status ON office_payout(status, created_at DESC)`); } catch {}

  // request.payout_id — links a paid request to the office_payout that
  // settles it. NULL means "eligible for the next pay-run".
  try {
    const { rows: rCols } = await db.execute(`SELECT name FROM pragma_table_info('request')`);
    const rNames = new Set(rCols.map(c => c.name));
    if (!rNames.has('payout_id')) {
      await db.execute(`ALTER TABLE request ADD COLUMN payout_id INTEGER REFERENCES office_payout(id)`);
    }
  } catch (e) { console.warn('request.payout_id migrate failed:', e.message); }
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_request_payout ON request(payout_id)`); } catch {}
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_request_paid_office ON request(office_id, paid_at) WHERE payment_status='paid'`); } catch {}

  // ─── Office bank + contact details (self-service settable) ───────
  // Required to actually transfer payouts. NOT mandatory at signup —
  // an office can register and get approved without bank info, but the
  // payouts UI flags them as "missing bank details" and blocks the
  // "mark paid" step until they fill it in. Owners edit via
  // /api/office/bank (settings panel in officer.html).
  //
  //   • iban                 — Omani IBAN (OM + 20 chars). Validated at write time.
  //   • bank_name            — display name e.g. "Bank Muscat"
  //   • account_holder_name  — may differ from CR name (e.g. owner's personal name)
  //   • bank_swift           — optional BIC/SWIFT for international transfers
  //   • billing_email        — separate from main login email (accounting inbox)
  //   • bank_updated_at      — touched on every edit (admin can see staleness)
  //   • bank_verified_at     — admin marks verified after first successful transfer
  try {
    const { rows: oCols } = await db.execute(`SELECT name FROM pragma_table_info('office')`);
    const oNames = new Set(oCols.map(c => c.name));
    const addOffice = async (col, ddl) => { if (!oNames.has(col)) await db.execute(`ALTER TABLE office ADD COLUMN ${ddl}`); };
    await addOffice('iban',                'iban TEXT');
    await addOffice('bank_name',           'bank_name TEXT');
    await addOffice('account_holder_name', 'account_holder_name TEXT');
    await addOffice('bank_swift',          'bank_swift TEXT');
    await addOffice('billing_email',       'billing_email TEXT');
    await addOffice('bank_updated_at',     'bank_updated_at TEXT');
    await addOffice('bank_verified_at',    'bank_verified_at TEXT');
  } catch (e) { console.warn('office bank-details migrate failed:', e.message); }
  try { await db.execute(`CREATE INDEX IF NOT EXISTS idx_office_has_bank ON office(iban) WHERE iban IS NOT NULL`); } catch {}
}
...
//
// CSV preference order (newest first):
//   v3 — post-sanad.om reconciliation (601 rows, the current canonical list)
//   v2 — scraped-only (453 rows)
//   v1 — legacy 3,417-row master (deleted from repo, kept for emergency)
...
// ─── Curated catalogue: keep ONLY verified services active ────
// The catalogue is curated — citizens are only ever offered services that a real
// office approved (verification_status='office_approved', which also covers
// office-added services) or that an annotator validated ('annotator_validated').
// Everything else — including scraped rows merely tagged is_launch — is
// DEACTIVATED (is_active=0, never deleted), so it stays reversible and can be
// promoted later via the annotator tool.
//
// Idempotent (re-running changes nothing) and GUARDED: if no verified service is
// active it does nothing, so a failed approved-load can never blank the catalogue.
export async function deactivateUnverifiedServices() {
  const { rows: vc } = await db.execute(
    `SELECT COUNT(*) AS n FROM service_catalog
      WHERE is_active=1 AND verification_status IN ('office_approved','annotator_validated')`);
  const verifiedActive = Number(vc[0].n);
  if (verifiedActive === 0) {
    console.warn('[catalog] curated-only skipped — no verified services active (safety guard)');
    return { skipped: true, deactivated: 0, active: null, verified: 0 };
  }
  // COALESCE is load-bearing: scraped rows can have verification_status=NULL
  // (autoImportCatalog never sets it), and `NULL NOT IN (...)` is NULL (not TRUE)
  // in SQL — so without COALESCE those rows would silently stay active.
  const r = await db.execute(
    `UPDATE service_catalog SET is_active=0, updated_at=datetime('now')
      WHERE is_active=1
        AND COALESCE(verification_status,'unverified') NOT IN ('office_approved','annotator_validated')`);
  const { rows: ac } = await db.execute(`SELECT COUNT(*) AS n FROM service_catalog WHERE is_active=1`);
  return { skipped: false, deactivated: r.rowsAffected || 0, active: Number(ac[0].n), verified: verifiedActive };
}
...
export async function seedDemoRequests() {
  ...
  for (const s of scenarios) {
    ...
    // Make citizen seeding idempotent — re-use an existing row when the same
    // phone is already in the table (the rebuild_catalogue path leaves the
    // citizen table populated, so a subsequent boot would otherwise hit the
    // UNIQUE(phone) constraint and crash the start-up).
    let citizenId;
    const { rows: cExist } = await db.execute({
      sql: `SELECT id FROM citizen WHERE phone=?`,
      args: [s.citizen.phone]
    });
    if (cExist.length) {
      citizenId = cExist[0].id;
    } else {
      const cIns = await db.execute({
        sql: `INSERT INTO citizen(phone,name,language_pref) VALUES (?,?,'ar')`,
        args: [s.citizen.phone, s.citizen.name]
      });
      citizenId = Number(cIns.lastInsertRowid);
    }
