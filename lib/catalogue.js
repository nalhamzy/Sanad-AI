...
export const LAUNCH_SERVICES = {
  'drivers_licence_renewal': {
    match_keywords: ['driving licence','driving license','driver licence','driver license','renew licence','renew license','رخصة قيادة','رخصة سياقة','رخصة السياقة','رخصة القيادة','تجديد رخصة القيادة','تجديد رخصة سياقة','أجدد رخصة','اجدد رخصة','جدد رخصة','تجديد رخصتي','تجديد رخصة'],
  ...
  'mulkiya_renewal': {
    // PROD BUG fixed 2026-05-08: name_ar was 'تجديد ملكية المركبة' which
    // doesn't exist in the catalog → every match silently fell through
    // to LLM. Real catalog row is id 140011 'خدمة طلب تجديد سجل المركبة'.
    // Also: 'بدل فاقد سند ملكية' / 'سند ملكية' / 'بدل فاقد الملكية' are
    // for a DIFFERENT service (title-deed replacement, see below) and
    // were misrouting citizens. Removed from this entry.
    match_keywords: ['mulkiya','vehicle registration','car registration','renew vehicle','renew car','ملكية المركبة','تجديد الملكية','تجديد ملكية المركبة','تجديد سجل المركبة','تجديد ملكية السيارة'],
    entity_en:'Royal Oman Police', entity_ar:'شرطة عمان السلطانية',
    name_en:'Vehicle Registration Renewal', name_ar:'خدمة طلب تجديد سجل المركبة',
    fee_omr: null,
    required_documents: [
  ...
  // PROD BUG fixed 2026-05-08: real prod citizen +96892888715 typed
  // "بدل فاقد سند ملكيه" — discover:title button hinted them to type
  // exactly that — but the deterministic match failed because no launch
  // entry existed for "title-deed replacement". Catalog row 150036
  // ("طلب إصدار سند ملكية بدل فاقد") is the right destination.
  'title_deed_replacement_lost': {
    match_keywords: ['lost title deed','duplicate title deed','replacement title deed','بدل فاقد سند ملكية','بدل فاقد سند الملكية','سند ملكية بدل فاقد','بدل فاقد الملكية','سند ملكية فاقد','سند ملكيه فاقد','فقدت سند الملكية'],
    entity_en:'Ministry of Housing and Urban Planning', entity_ar:'وزارة الإسكان والتخطيط العمراني',
    name_en:'Request Replacement Title Deed (Lost)', name_ar:'طلب إصدار سند ملكية بدل فاقد',
    fee_omr: null,
    required_documents: [
      { code:'civil_id',     label_en:'Civil ID', label_ar:'البطاقة المدنية' },
      { code:'police_report',label_en:'Police report of loss (if available)', label_ar:'بلاغ فقدان من الشرطة (إن وجد)' },
      { code:'lost_form',    label_en:'Lost-title-deed declaration form', label_ar:'إستمارة إعلان فقدان سند ملكية' }
    ]
  },
  'cr_issuance': {
  ...
  // codex iter-12: bench scenario #4 (mid_flow_pivot) and prod citizens
  // both use "تجديد جواز السفر"; the catalog has issuance, which Sanad
  // offices process for both new + renewal cases. Surfacing it here
  // unblocks the deterministic shortcut.
  'passport_issuance_renewal': {
    match_keywords: ['passport','passport issuance','passport renewal','renew passport','جواز','جواز السفر','إصدار الجواز','إصدار جواز السفر','تجديد جواز السفر','تجديد الجواز'],
    entity_en:'Royal Oman Police', entity_ar:'شرطة عمان السلطانية',
    name_en:'Omani Passport Issuance Service', name_ar:'خدمة إصدار الجواز العماني',
    fee_omr: 5,
    required_documents: [
      { code:'civil_id',     label_en:'Civil ID', label_ar:'البطاقة المدنية' },
      { code:'old_passport', label_en:'Existing passport (for renewal)', label_ar:'الجواز الحالي (للتجديد)' },
      { code:'photo',        label_en:'Personal photo (white background)', label_ar:'صورة شخصية' }
    ]
  }
};
...
export async function matchService(userText, { trace, useHybrid = true } = {}) {
  ...
  // Verified-only: the hybrid path (gated to verified + launch) is the SOLE
  // matcher. Don't fall through to the legacy LIKE path below — it scans the
  // full catalogue including unverified rows. No verified match → return null so
  // the agent routes the citizen to triage.
  if (process.env.SANAD_VERIFIED_ONLY === 'true') {
    trace?.push({ step: 'verified_only_no_match' });
    return null;
  }
...
export function parseRequiredDocs(row) {
  try {
    ...
    // Backfill Arabic labels — the imported CSV left label_ar="" on most
    // rows, which made the UI fall back to label_en. enrichDocsWithArabicLabels
    // either uses a curated label or a heuristic token translation so
    // every consumer sees Arabic text by default. See lib/doc_labels.js.
    return enrichDocsWithArabicLabels(parsed);
  } catch { return []; }
}
