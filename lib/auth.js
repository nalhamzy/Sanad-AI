...
import { DEBUG_ENABLED } from './env.js';

const ROUNDS = Number(process.env.BCRYPT_ROUNDS || 10);

// JWT signing key — must be set explicitly in production. The dev fallback
// stays so `npm test` and local development don't need a `.env`, but a
// production boot without JWT_SECRET is a hard failure: every cookie we'd
// sign would be guessable, and silently shipping that would be worse than
// crashing on startup. NODE_ENV is the gate (Render/most PaaS set it).
const JWT_SECRET = (() => {
  const v = process.env.JWT_SECRET;
  if (v && v.length >= 16) return v;
  if (process.env.NODE_ENV === 'production') {
    throw new Error(
      'JWT_SECRET is required in production (must be ≥16 chars). ' +
      'Set it in your environment before booting.'
    );
  }
  return 'sanad-dev-only-secret-change-me';
})();
...
export async function loadOfficer(officer_id) {
  const { rows } = await db.execute({
    sql: `SELECT o.id AS officer_id, o.office_id, o.full_name, o.email, o.role, o.status AS officer_status,
            ...
            FROM officer o
            LEFT JOIN office off ON off.id = o.office_id
           WHERE o.id = ?`,
    args: [officer_id]
  });
  return rows[0] || null;
...
// Platform-admin (the Sanad-AI team). Two ways to qualify:
//   1. role='platform_admin' — a STANDALONE admin account (office_id may be NULL;
//      it doesn't have to be a Sanad office). This is the durable, DB-driven path.
//   2. email in ADMIN_EMAILS — the legacy env allow-list (any signed-in officer).
// (1) lets us decouple "admin" from "owns an office" without an env change.
export function requirePlatformAdmin() {
  // Read ADMIN_EMAILS lazily inside the middleware (not at factory time) so
  // tests/scripts that set the env var after import still work.
  return (req, res, next) => {
    if (!req.officer) return res.status(401).json({ error: 'not_signed_in' });
    const adminEmails = (process.env.ADMIN_EMAILS || '')
      .split(',').map(s => s.trim().toLowerCase()).filter(Boolean);
    const email = (req.officer.email || '').toLowerCase();
    // Dev-only fallback: with no ADMIN_EMAILS configured AND debug enabled,
    // any signed-in officer is treated as admin so local testing works.
    // DEBUG_ENABLED is HARD-OFF in production (lib/env.js), so this fallback
    // can never grant admin on a real deployment — there, a non-empty
    // ADMIN_EMAILS allow-list is the only way in (enforced at boot by
    // assertProductionConfig).
    const isAdmin = req.officer.role === 'platform_admin' || (adminEmails.length
