...
- **New request vs. follow-up.** Read the session state injected at the top:
  ...
  - status=collecting/reviewing → there's a DRAFT in flight (no DB request yet). If the user is continuing (sends a doc, asks about the same service, says "yes") — keep going. If the user clearly **changes topic** ("forget that, I want a passport"), call discard_draft FIRST, then start_submission for the new service.
  - status=queued/claimed/in_progress → there's a SUBMITTED request. New unrelated questions = follow-up about that request, OR a brand-new service request. Ask one clarifying question if unclear: "Do you want me to keep tracking #R-X, or start something new?" Use cancel_request only if they say cancel.
...
function renderStateContext(state) {
  ...
  if (state.status === 'idle') {
    ...
    routing = 'Routing: a DRAFT is in progress (no DB request yet). If the user is continuing, keep collecting docs. If they switch topics, call discard_draft then start_submission for the new service.';
  } else if (state.status === 'reviewing') {
    routing = 'Routing: all docs collected, awaiting user OK to submit_request. If user changes topic, discard_draft first.';
  } else if (['queued','claimed','in_progress'].includes(state.status)) {
    routing = `Routing: a real request (#R-${state.request_id}) has been submitted. New requests must be a separate flow — clarify with the user before discarding.`;
  }
...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  if (attachment) {
    ...
    //   1. attachment.caption  — WhatsApp media caption (msg.image.caption etc.)
    //   2. raw                 — separate text in the same turn
    // Both can be present; both go to the LLM so it can match the right doc slot.
    ...
    messages.push({
      role: 'user',
      content: `[Attachment uploaded: name="${attachment.name || 'file'}", mime=${attachment.mime || 'unknown'}, size=${attachment.size || 0}B${capStr}]\n` +
        `→ Use the caption + filename + the "Document slots" list in the session header to pick the matching doc_code, then call record_document(doc_code, caption=<that caption>). ` +
        `If only one slot is unfilled, use it. If multiple slots could match and the caption is ambiguous, ask ONE short clarifying question instead of guessing.`
    });
  }
