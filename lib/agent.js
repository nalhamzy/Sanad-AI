...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  // ── Triage text trigger ────────────────────────────────────────
  // Citizen explicitly says they don't know which service they need. Enter the
  // deterministic triage intake (same as the 🤔 button) instead of searching/
  // guessing. Idle + no draft only, so it never hijacks an active flow (e.g.
  // "لست متأكد من رقم الهوية" mid-collection).
