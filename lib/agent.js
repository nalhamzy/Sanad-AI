...
async function drainBurst(session_id) {
  ...
  if (n <= 1) {
    ...
    // Compose: headline → live checklist → optional auto-match hint
    // (when 2+ files matched, signals "I made automatic decisions —
    // double-check the order") → kept-aside line if extras → close
    // with the same standard question.
    //
    // Per the user's spec (2026-05-06): never list "next required" —
    // citizen sends in any order; the buttons drive the next step.
    const sections = [headline];
    if (checklist) sections.push(checklist);
    // Auto-match hint: when multiple files were matched in this burst
    // AND the matching was automatic (not user-captioned). Helps the
    // citizen spot a wrong-slot mapping early (UX enhancement, 2026-05-07).
    if (n >= 2 && docs.length && pending.length < docs.length) {
      sections.push('💡 رتّبت الملفات تلقائياً حسب الترتيب المطلوب. لو كان أحد الملفات في خانة خاطئة، أرسله مرة أخرى مع تعليق يصفه.');
    }
    if (extrasN > 0) {
...
async function handleButtonIntent({ session_id, state, btn_id, attachment, citizen_phone, trace }) {
  ...
  if (btn_id === 'status:check' && state.request_id) {
    try {
      ...
      if (result?.ok && result.request) {
        ...
        const reply =
          `📊 *حالة طلبك*\n\n` +
          `🆔 رقم الطلب: *#R-${r.id}*\n` +
          `📂 الخدمة: ${r.service_name_ar || r.service_name}\n` +
          `${label}` +
        ...
        // Re-offer the same buttons unless completed.
        const _buttons = (r.status === 'completed' || r.status === 'cancelled')
          ? null
          : [
    ...
    return null; // fall through to LLM if tool failed
  }
