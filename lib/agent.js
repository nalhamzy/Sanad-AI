...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  if (
    ...
    if (autoRecorded) {
      ...
      if (m) justSaved.push(m.label_ar || m.label_en || autoRecorded.recorded);
    }
    for (const r of recordedThisLoop) {
      if (!justSaved.includes(r.label)) justSaved.push(r.label);
    }
    ...
    if (pending.length === 0) {
      ...
      const nextLabel = pending[0].label_ar || pending[0].label_en || pending[0].code;
