...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  const TRIAGE_TRIGGER_RE = /(?:لست\s*متأكد|لا\s*أعرف\s*(?:الخدمة|أي\s*خدمة|ايش\s*أحتاج|وش\s*أحتاج)|مش\s*متأكد|ما\s*(?:أعرف|اعرف|ادري|أدري)\s*(?:الخدمة|وش|ايش|اي\s*خدمة)|غير\s*متأكد|not\s*sure|don'?t\s*know\s*(?:which|what)|unsure|help\s*me\s*choose)/i;
  if (raw && typeof raw === 'string' && !attachment &&
      (!state.status || state.status === 'idle') && !state.service_id && !state.request_id &&
      TRIAGE_TRIGGER_RE.test(raw)) {
    state.status = 'triage';
    state.intent_summary = '';
    if (!Array.isArray(state.pending_uploads)) state.pending_uploads = [];
