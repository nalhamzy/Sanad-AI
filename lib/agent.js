...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  if (typeof raw === 'string' && raw.startsWith('__btn__:')) {
    ...
    const ALWAYS_OK_FOR_COLLECTING = new Set(['review:submit', 'burst:more', 'burst:done', 'service:cancel', 'service:switch']);
...
function attachContextualButtons({ state, finalReply, ambiguousAttachment, justBufferedThisFile, recordedRequiredThisTurn, trace }) {
  ...
  const LLM_FALLBACK_RE = /تعذّر الاتصال بالمساعد الذكي مؤقتاً/;
  ...
  // codex iter-7: LLM-error fallback in idle state → discovery hints so
  // the citizen can start a journey without typing. Without this, the
  // doesnt_know_what_he_wants scenario produced 3 buttonless replies.
