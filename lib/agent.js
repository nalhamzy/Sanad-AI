...
6. **EXTREME BREVITY — citizens are on a phone screen.** Word counts are HARD ceilings:
   ...
   - **Service info answer** (fees, docs, time): ≤ 60 words. Format: 1 line for service+fee, 1 line per doc, 1 line for time. Then ONE follow-up: "Start now? / نبدأ؟".
  ...
  if (attachment) {
    ...
    if (autoRecorded) {
      ...
      const nextLine = next
        ...
        : `ALL ${autoRecorded.total_docs} required docs collected — state="reviewing"`;
      messages.push({
        role: 'system',
        content:
          ...
          `  • ≤ 25 words total, ≤ 3 lines.\n` +
          `  • Line 1 — tick + the doc's NAME in the user's language. e.g. "✅ Got your civil ID." / "✅ استلمت بطاقتك الشخصية."\n` +
          `  • Line 2 — ${next ? 'the NEXT doc with 📎. e.g. "📎 Next: passport copy" / "📎 التالي: نسخة الجواز"' : 'one short cue + a submit prompt. e.g. "All set. Submit your file?" / "ممتاز. نُرسل ملفك؟"'}\n` +
      ...
      // Service has no required docs — the upload was auto-attached as a
      // supplementary file so the citizen doesn't get pestered.
      messages.push({
        role: 'system',
        content:
          `[Extra file auto-saved silently — total extras now: ${autoExtra.extra_count} — service has no required docs]\n\n` +
          `Reply rules (HARD CEILINGS):\n` +
          `  • ≤ 25 words total, ≤ 3 lines.\n` +
          `  • Line 1 — tick + "attached as extra". e.g. "✅ Attached as extra." / "✅ أُرفق كملف إضافي."\n` +
          `  • Line 2 — submit prompt. e.g. "Submit your file?" / "نُرسل ملفك الآن؟"\n` +
          `  • DO NOT mention caption status, filename, mime, or "no caption". DO NOT ask whether it's required vs extra — already handled.\n` +
          `  • DO NOT call record_extra_document — already done.`
      });
    } else if (ambiguousAttachment) {
      // Caption present but didn't match any pending slot. Ask, don't guess.
      ...
      messages.push({
        role: 'system',
        content:
          `[Attachment caption mismatch — needs clarification — caption="${ambiguousAttachment.caption.replace(/"/g, '\\"').slice(0, 80)}" — next required: ${nextDual}]\n\n` +
          `Reply rules (HARD CEILINGS):\n` +
          `  • ≤ 15 words, single line.\n` +
          `  • Ask ONE punchy disambiguation question. e.g. "For ${nextLabelEn}, or extra?" / "هل هذا للـ${nextLabelAr || nextLabelEn}، أم إضافي؟"\n` +
          `  • DO NOT call any tool now. Wait for the user's reply.\n` +
          `  • On the user's NEXT turn: record_document(doc_code="${next?.code || ''}", caption=<theirs>) if required, OR record_extra_document(caption=<theirs>) if extra.`
      });
    } else {
