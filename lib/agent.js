...
async function drainBurst(session_id) {
  ...
  if (n <= 1) {
    ...
    if (cur.kind === 'replacement') {
      ...
      headline = `✅ استلمت ${n} ${wordFile} وأرسلتها إلى المكتب المتولّي طلبك.`;
    } else {
      // No request yet — just collected into the draft.
      headline = `✅ استلمت ${n} ${wordFile} وحفظتها في ملف طلبك.`;
    }
