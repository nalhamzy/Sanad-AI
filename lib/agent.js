...
// separate replies.
const BURST_COOLDOWN_MS = Number(process.env.SANAD_BURST_COOLDOWN_MS || 4000);
...
async function drainBurst(session_id) {
  ...
  if (n <= 1) {
    ...
    } else if (docs.length && pending.length === 0) {
      // All required slots filled by this burst.
      headline = `📥 استلمت ${n} ${wordFile} — كل المطلوب اكتمل ✨`;
    } else {
      headline = `📥 استلمت ${n} ${wordFile}.`;
    }

    // Compose: headline → live checklist → "kept aside" if any → next-step.
    const sections = [headline];
    if (checklist) sections.push(checklist);
    if (extrasN > 0) {
      sections.push(`📎 ${extrasN} ${extrasN === 1 ? 'ملف' : 'ملفات'} لا تُطابق المطلوب — حفظتُها جانباً.`);
    }
    if (docs.length && pending.length > 0) {
      sections.push(`📎 المطلوب التالي: *${arabicLabelFor(pending[0])}*`);
    } else if (docs.length && pending.length === 0) {
      sections.push('اضغط *📤 أرسل للمراجعة* لإرسال طلبك إلى مكاتب سند.');
    } else {
      // No required docs known yet (e.g. extras-only state) — keep the
      // generic prompt the buttons handle.
      sections.push('اضغط *✓ انتهيت* لمتابعة الطلب، أو أرسل بقية الملفات الآن.');
    }
    text = sections.join('\n\n');
  }

  try {
    // Solo file (n === 1): the handler (runAgentV2) already wrote `text`
    // to the message table when it produced its reply. Storing here again
    // creates a duplicate row that surfaces as TWO identical bot bubbles
    // ~1.8s apart in the chat history (and on the citizen's WhatsApp if a
    // mirror is on). Multi-file (n >= 2): `text` is a NEW synthetic summary
    // that no handler emitted, so we must store it fresh.
    if (n >= 2) {
      await storeMessage({
        session_id,
    ...
    if (typeof session_id === 'string' && session_id.startsWith('wa:')) {
      ...
      if (n >= 2) {
        // If all required slots filled by this burst → escalate buttons
        // straight to the review-and-submit gate (per spec: skip the
        // "still need X" prompt and jump to the Review & submit button
        // when batch fully satisfies the checklist).
        const allDoneAfterBurst = docs.length > 0 && pending.length === 0;
        // codex Q1: also surface "replace" when any slot is filled (not
        // only on full burst) so a misclassified file can be fixed in
        // one tap instead of typing.
        const anyFilled = docs.length > 0 && pending.length < docs.length;
        await sendButtons(allDoneAfterBurst ? [
          { id: 'review:submit', title: '📤 أرسل للمراجعة' },
          { id: 'doc:wrong',     title: '🔄 استبدل ملف' },
          { id: 'service:cancel',title: '✕ إلغاء الطلب' }
        ] : anyFilled ? [
          { id: 'burst:done', title: '✓ انتهيت' },
...
Saned (ساند) is a **request preparation and dispatch platform** for Oman government services. The product is a two-sided marketplace:

  Citizen  ⇄  **You (ساند / Saned)**  ⇄  **Sanad offices marketplace**  ⇄  Government entities
...
async function handleButtonIntent({ session_id, state, btn_id, attachment, citizen_phone, trace }) {
  ...
  if (btn_id === 'burst:more') {
    ...
    if (!r) {
      ...
      const reply = checklist
        ...
        : '✨ كل المستندات المطلوبة عندي بالفعل. اضغط *📤 أرسل للمراجعة*.';
      const _buttons = [
        { id: 'review:submit',  title: '📤 أرسل للمراجعة' },
        { id: 'doc:extra',      title: '📎 مستند داعم' },
        { id: 'service:cancel', title: '✕ إلغاء الطلب' }
      ];
    ...
    const reply = checklist
      ? `تمام، أرسل البقية. القائمة:\n\n${checklist}\n\n📎 التالي: *${arabicLabelFor(r.first)}*`
      : `تمام، أرسل البقية.\n\n📋 *المتبقي* (${r.count}):\n${r.lines}\n\n📎 التالي: *${arabicLabelFor(r.first)}*`;
    const _buttons = [
      { id: 'doc:list',       title: '📋 الملفات المطلوبة' },
      { id: 'doc:extra',      title: '📎 مستند داعم' },
      { id: 'service:cancel', title: '✕ إلغاء الطلب' }
    ];
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
  ...
  if (btn_id === 'burst:done') {
    ...
    if (state.status === 'collecting' && pending.length === 0 && haveAny) {
      ...
      const reply =
        `📦 *جاهز للمراجعة*\n\n${checklist || ''}` +
        (extras ? `\n\n📎 ملفات إضافية: ${extras}` : '') +
        `\n\nاضغط *📤 أرسل للمراجعة* لإرسال الطلب إلى مكاتب سند المتاحة.`;
      const _buttons = [
        { id: 'review:submit',  title: '📤 أرسل للمراجعة' },
        { id: 'doc:extra',      title: '📎 مستند داعم' },
        { id: 'service:cancel', title: '✕ إلغاء الطلب' }
    ...
    if (state.status === 'collecting' && pending.length > 0) {
      // Still missing required docs. Per the spec: lead with what's
      // DONE softly, never lecture. The bot disagrees politely.
      const r = renderRemaining();
      const checklist = renderChecklist(state);
      ...
      const got = haveCount > 0
        ...
        : '';
      const reply = checklist
        ? `${got}إليك ما اكتمل وما تبقّى:\n\n${checklist}\n\n📎 أرسل الآن: *${arabicLabelFor(r.first)}*`
        : `${got}بقي ${r.count} مستند${r.count === 1 ? '' : 'ات'}:\n\n${r.lines}\n\n📎 أرسل الآن: *${arabicLabelFor(r.first)}*`;
      const _buttons = [
        { id: 'doc:list',       title: '📋 الملفات المطلوبة' },
        { id: 'doc:extra',      title: '📎 مستند داعم' },
        { id: 'service:cancel', title: '✕ إلغاء الطلب' }
      ...
      await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
      trace.push({ step: 'btn_done_blocked_missing', remaining: r.count });
      return { reply, state, trace, _buttons };
    }
  ...
  if (btn_id === 'doc:list') {
    ...
    if (!r) {
      ...
      const reply = checklist
        ...
        : '✨ كل المستندات المطلوبة عندي. اضغط *📤 أرسل للمراجعة*.';
      await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
      return {
        reply, state, trace,
        _buttons: [
  ...
  // service:cancel — confirm-first flow. Codex (gpt-5.2-codex) flagged: a
  // single accidental tap shouldn't nuke the draft. Set a pending_cancel
  // flag and ask for confirmation.
  if (btn_id === 'service:cancel') {
    if (state.status === 'idle') return null; // nothing to cancel
    state.pending_cancel = true;
    await saveSession(session_id, state);
    const reply = '⚠️ هل تؤكد إلغاء الطلب الحالي؟ ستفقد المستندات المرفقة.';
    return {
      reply, state, trace,
      _buttons: [
        { id: 'confirm:yes', title: '✓ نعم، ألغِ' },
        { id: 'confirm:no',  title: '✕ تراجع' }
      ]
    };
  }

  // confirm:yes after pending_cancel — actually discard the draft.
  if (btn_id === 'confirm:yes' && state.pending_cancel) {
    ...
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
    trace.push({ step: 'btn_cancel_confirmed' });
...
async function handleInFlight({ state, raw, trace, attachment, session_id }) {
  ...
  if (attachment && state.request_id) {
    ...
    try {
      ...
      return {
        reply: '✅ استلمت ملفك الإضافي وأرسلته إلى المكتب المتولّي طلبك.',
        state,
        _burstKind: 'extra'
      };
    } catch (e) {
...
- The reply asks about an ambiguous upload → **✓ نعم — [slot] / 🔄 ملف آخر / 📎 إضافي** buttons
- The reply is the multi-file burst summary → **✓ تم / + سأرسل المزيد** buttons
...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  if (attachment && ['collecting', 'reviewing'].includes(state.status)
    ...
    // Three triggers route a file to the buffer (silent batch mode):
    //   1. There's already a buffer with items (an ongoing burst).
    //   2. Last upload was within the burst window (rapid follow-up).
    //   3. No caption AND ≥2 required slots are still open AND the file
    //      could plausibly fill multiple of them — without a clue from
    //      the citizen, positional fast-path can put the wrong file in
    //      the wrong slot. Better to ask once for descriptions.
    // Single-slot caption-less drops still take the fast path (unambiguous).
    const slotsLeftCount = state.docs.filter(d => !collected[d.code]).length;
    const ambiguousCaptionless = !cap && slotsLeftCount >= 2;
    if (state.pending_uploads?.length || inBurstWindow || ambiguousCaptionless) {
      let v = null;
    ...
    messages.push({
      ...
      content:
        ...
        (unmatchedList.length
          ? `  • Line 2 — note the extras. e.g. "📎 Plus extras: ${unmatchedList.slice(0, 2).join(', ')}." / "📎 وملفات إضافية: …"\n`
          : '') +
        (remaining.length
          ? `  • Final line — name what's still needed and ask for the FIRST missing doc. e.g. "Still need: ${remainingLabels}. 📎 Next: ${remaining[0]?.label_en || '?'}." / "تبقى: …"\n`
  ...
  if (startSubmissionResult) {
    ...
    const fee = r.fee_omr != null
      ? `\n💰 رسوم المعاملة: ${Number(r.fee_omr).toFixed(0)} ر.ع`
      : (r.fees_text ? `\n💰 الرسوم: ${r.fees_text}` : '');
    ...
    const firstDoc = r.first_doc;
    const firstAsk = firstDoc
      ? `\n\n📎 جهّز الملفات وابدأ بإرسال: *${arabicLabelFor(firstDoc)}*`
  ...
  if (guardReason) {
    ...
    const pending = (state.docs || []).filter(d => !ctx.state.collected?.[d.code]);
    const next = pending[0];
    const nextHint = next
      ? `\n\n📎 المطلوب التالي: *${next.label_ar || next.label_en || next.code}*. أرسله مع تعليق يصفه (مثل: "هذه البطاقة المدنية").`
      : '';
    finalReply =
      '📥 استلمت الملف وحفظته كملف إضافي حتى أتأكد من خانته. أخبرني ما هو حتى أضعه في الخانة الصحيحة (مثل: "هذه البطاقة المدنية").' +
      nextHint;
  }
  ...
  if (
    ...
    const savedLine = justSaved.length
      ? `✅ استلمت: *${justSaved.join('* و *')}*`
      : '✅ تم استلام الملف';
    // Live checklist — the spec's headline UX pattern. Every reply during
    // collecting shows the full ✅/⏳ list so the citizen never has to ask
    // "what else do you need?".
    const checklist = renderChecklist(ctx.state);
    let body;
    if (pending.length === 0) {
      body = checklist
        ? `${savedLine}\n\n${checklist}\n\n📦 اكتمل ملفك. اضغط *📤 أرسل للمراجعة* لإنهاء الطلب.`
        : `${savedLine}\n\n📦 اكتمل ملفك. اضغط *📤 أرسل للمراجعة* لإنهاء الطلب.`;
    } else {
      const nextLabel = arabicLabelFor(pending[0]);
      body = checklist
        ? `${savedLine}\n\n${checklist}\n\n📎 التالي: *${nextLabel}*`
        : `${savedLine}\n\n📎 التالي: *${nextLabel}*`;
    }
  ...
  trace.push({ step: 'v2_saved', status: ctx.state.status, silent: !finalReply });
...
function attachContextualButtons({ state, finalReply, ambiguousAttachment, justBufferedThisFile, recordedRequiredThisTurn, trace }) {
  ...
  if (ambiguousAttachment || justBufferedThisFile) {
    ...
    return fire(ambiguousAttachment ? 'ambiguous_doc' : 'buffered_no_caption', [
      { id: 'doc:yes',   title: `✓ نعم — ${slotLabel}` },
      { id: 'doc:wrong', title: '🔄 خانة مختلفة' },
      { id: 'doc:extra', title: '📎 مستند داعم' }
    ]);
  }

  // CASE 2 — collecting state, file just recorded → nav buttons
  // The bot's reply is the post-save "✅ got X, send Y" line. The citizen's
  // next action is to upload the next doc — but we still want to give them
  // tap-able exits (list remaining, mark file as extra, cancel draft).
  if (state?.status === 'collecting' && recordedRequiredThisTurn) {
    if (pending.length === 0) {
      return fire('collecting_complete', [
        { id: 'review:submit',  title: '📤 أرسل للمراجعة' },
        { id: 'doc:extra',      title: '➕ ملف إضافي' },
        { id: 'service:cancel', title: '✕ إلغاء' }
      ]);
    }
    return fire('collecting_recorded', [
      { id: 'doc:list',       title: '📋 الملفات المطلوبة' },
      { id: 'doc:extra',      title: '📎 مستند داعم' },
      { id: 'service:cancel', title: '✕ إلغاء الطلب' }
    ]);
  ...
  // CASE 3 — collecting state, no record this turn (e.g. citizen text reply
  // mid-flow) → keep nav buttons visible so they don't get stuck.
  if (state?.status === 'collecting' && pending.length > 0) {
    return fire('collecting_idle', [
      { id: 'doc:list',       title: '📋 الملفات المطلوبة' },
      { id: 'doc:extra',      title: '📎 مستند داعم' },
      { id: 'service:cancel', title: '✕ إلغاء الطلب' }
    ]);
  ...
  // CASE 4 — reviewing state → submit-ready nav
  if (state?.status === 'reviewing') {
    return fire('reviewing', [
      { id: 'review:submit',  title: '📤 أرسل للمراجعة' },
      { id: 'doc:extra',      title: '📎 مستند داعم' },
      { id: 'service:cancel', title: '✕ إلغاء الطلب' }
    ]);
