    ...
    if (FEE_OR_INFO_QUERY_RE.test(raw)) {
      ...
      if (m && m.source === 'launch' && m.code && m.service) {
        ...
        if (rows[0]?.id) {
          ...
          if (r?.ok) {
            await saveSession(session_id, state);
            // Use arabicLabelFor (translates code via ARABIC_DOC_LABELS dict)
            // so prod citizens see Arabic labels even when the catalog row
            // has empty label_ar — caught in trace +96892888715 #1739 where
            // the title-deed flow rendered "1) Civil ID 2) Police report..."
            // instead of "1) البطاقة المدنية 2) تقرير الشرطة...".
