...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  if (raw && typeof raw === 'string' && state.status === 'idle' && !attachment &&
    ...
    try {
      ...
      if (top.length >= 1) {
        // Look up full rows for fee + name (search returns minimal cols).
        const lines = [];
        const ids = [];
        for (let i = 0; i < top.length; i++) {
          ...
          const fee = row.fee_omr != null ? ` — ${Number(row.fee_omr).toFixed(0)} ر.ع` : '';
          lines.push(`${i + 1}️⃣ ${name}${fee}`);
        }
        if (ids.length) {
          ...
          if (ids.length >= 3) btns.push({ id: 'pick:3', title: '3️⃣' });
          state.last_offered_buttons = btns.map(b => b.id);
          await saveSession(session_id, state);
          trace.push({ step: 'deterministic_search_results', count: ids.length, ids });
