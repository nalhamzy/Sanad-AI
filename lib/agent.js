...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  if (guardReason) {
    ...
    const pending = (state.docs || []).filter(
      d => !ctx.state.collected?.[d.code]?.storage_url && !isPlaceholderDoc(d)
    );
    const next = pending[0];
    const checklist = renderChecklist(ctx.state);
    const nextHint = next
      ? `\n\n📎 المطلوب التالي: *${arabicLabelFor(next)}*`
      : '';
    finalReply = checklist
      ? `📥 استلمت الملف وحفظته جانباً مؤقتاً.\n\n${checklist}${nextHint}`
      : '📥 استلمت الملف وحفظته جانباً مؤقتاً.' + nextHint;
  }
