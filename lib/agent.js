      ...
      if (VISION_ENABLED && attachment.url && /^image\//.test(attachment.mime || '')) {
        ...
        if (slotsLeft.length) {
          ...
          if (v?.ok && v.best && v.best.confidence >= 0.65) {
            ...
            // Low-confidence guess. Surface to LLM as a HINT (not a
            // mismatch) so the question becomes "this looks like X — save
            // as Y, or extra?" instead of "couldn't match caption".
            ambiguousAttachment = {
              caption: cap,
              filename: attachment.name || null,
              next_pending: state.docs[state.pending_doc_index ?? 0] || null,
              vision_summary: v.summary,
              vision_best: v.best,
              vision_alternatives: v.alternatives
            };
            trace.push({ step: 'auto_record_skipped', reason: 'vision_low_conf',
              vision_best: v.best?.code, vision_conf: v.best?.confidence });
          }
        }
      }
      if (!autoRecorded && !autoExtra && !ambiguousAttachment) {
        // Vision unavailable / failed / no slots — fall back to legacy
        // "ask the user" path with the caption as the only signal.
        ambiguousAttachment = {
          caption: cap,
          filename: attachment.name || null,
          next_pending: state.docs[state.pending_doc_index ?? 0] || null
        };
        trace.push({ step: 'auto_record_skipped', reason: 'caption_no_match',
