...
async function fireBurstSummary(session_id) {
  ...
  try {
    ...
    // directly with quick-reply buttons so they don't have to type تم/خلصت.
    // Webhook maps burst:done → 'تم' which the in-flight closer recognises.
    if (typeof session_id === 'string' && session_id.startsWith('wa:')) {
      ...
      try {
        ...
        const r = await sendWhatsAppButtons(phone, text, [
          { id: 'burst:done', title: '✓ تم' },
          { id: 'burst:more', title: '+ سأرسل المزيد' }
        ]);
        if (!r.ok) {
          // Plain-text fallback so the citizen still sees the summary.
          sendWhatsAppText(phone, text).catch(() => {});
        }
      } catch (e) {
        console.warn('[burst-summary wa]', e.message);
        sendWhatsAppText(phone, text).catch(() => {});
      }
    }
  } catch (e) {
