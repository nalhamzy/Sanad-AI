...
async function drainBurst(session_id) {
  ...
  if (n <= 1) {
    ...
    // Compose: headline → live checklist → "kept aside" if any → ASK
    // THE SAME QUESTION EVERY TIME: "is your file complete?". Per the
    // user's spec (2026-05-06): never list "next required" — the
    // citizen sends in any order, the buttons drive the next step.
    ...
    if (docs.length && pending.length === 0) {
      sections.push('✨ اكتمل ملفك. تأكيد الإرسال إلى مكاتب سند؟');
    } else {
      sections.push('هل اكتمل ملفك؟ تأكيد وأرسل، أو أرسل المزيد.');
    }
    text = sections.join('\n\n');
  }
  ...
  try {
    ...
    if (typeof session_id === 'string' && session_id.startsWith('wa:')) {
      ...
      if (n >= 2) {
        ...
        await sendButtons([
          { id: 'review:submit',  title: '✅ تأكيد وأرسل' },
...
async function handleButtonIntent({ session_id, state, btn_id, attachment, citizen_phone, trace }) {
  ...
  if (btn_id === 'burst:more') {
    ...
    if (!r) {
      ...
      const reply = checklist
        ...
        : '✨ كل المستندات المطلوبة عندي بالفعل. اضغط *✅ تأكيد وأرسل*.';
      const _buttons = [
        { id: 'review:submit',  title: '✅ تأكيد وأرسل' },
        { id: 'burst:more',     title: '➕ سأرسل المزيد' },
        { id: 'service:cancel', title: '✕ إلغاء الطلب' }
    ...
    const reply = checklist
      ? `تمام، أرسل البقية بأي ترتيب.\n\n${checklist}\n\nهل اكتمل ملفك؟ تأكيد وأرسل، أو أرسل المزيد.`
      : `تمام، أرسل البقية بأي ترتيب.\n\nهل اكتمل ملفك؟`;
    const _buttons = [
      { id: 'review:submit',  title: '✅ تأكيد وأرسل' },
      { id: 'burst:more',     title: '➕ سأرسل المزيد' },
  ...
  if (btn_id === 'burst:done') {
    ...
    if (state.status === 'collecting' && pending.length === 0 && haveAny) {
      ...
      const reply =
        `📦 *جاهز للمراجعة*\n\n${checklist || ''}` +
        (extras ? `\n\n📎 ملفات إضافية: ${extras}` : '') +
        `\n\nاضغط *✅ تأكيد وأرسل* لإرسال الطلب إلى مكاتب سند المتاحة.`;
      const _buttons = [
        { id: 'review:submit',  title: '✅ تأكيد وأرسل' },
        { id: 'burst:more',     title: '➕ سأرسل المزيد' },
        { id: 'service:cancel', title: '✕ إلغاء الطلب' }
  ...
  if (btn_id === 'doc:list') {
    ...
    if (!r) {
      ...
      const reply = checklist
        ...
        : '✨ كل المستندات المطلوبة عندي. اضغط *✅ تأكيد وأرسل*.';
      await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
      return {
        reply, state, trace,
        _buttons: [
  ...
  if (btn_id === 'service:cancel') {
    ...
    return {
      reply, state, trace,
      _buttons: [
        { id: 'confirm:yes', title: '🗑️ نعم، احذف الطلب' },
...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  if (startSubmissionResult) {
    ...
    const cta = docs.length
      ? '\n\n📎 أرسل المستندات الآن — كلها معاً أو واحداً تلو الآخر، بأي ترتيب.'
  ...
  if (
    ...
    if (pending.length === 0) {
      ...
      body = checklist
        ? `${savedLine}\n\n${checklist}\n\n✨ اكتمل ملفك. تأكيد الإرسال إلى مكاتب سند؟`
        : `${savedLine}\n\n✨ اكتمل ملفك. تأكيد الإرسال إلى مكاتب سند؟`;
    } else {
...
function attachContextualButtons({ state, finalReply, ambiguousAttachment, justBufferedThisFile, recordedRequiredThisTurn, trace }) {
  ...
  // No surprise menus — the choice space is always
  // [confirm-and-send / more / cancel]. Citizen knows what comes next.
