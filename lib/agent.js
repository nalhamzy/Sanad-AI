...
function parseUploadDescriptions(text, pendingUploads, docs, collected) {
  ...
  if (parts.length >= 2) {
    // Try to map each part to a slot. Pair parts to uploads positionally
    // when counts match; otherwise pair greedily (each part matched to a
    // slot, distributed across the buffered files in order — surplus parts
    // beyond pendingUploads.length are dropped, surplus files are mapped
    // to vision_best where available).
    const mappings = [];
    const usedIdxs = new Set();
    const partsToMap = parts.slice(0, pendingUploads.length);
    let i = 0;
    for (const part of partsToMap) {
      const slot = matchSlotByText(part, slotsLeft);
      if (!slot) continue;
      // Find next un-mapped upload
      while (i < pendingUploads.length && usedIdxs.has(pendingUploads[i].idx)) i++;
      if (i >= pendingUploads.length) break;
      mappings.push({ idx: pendingUploads[i].idx, doc_code: slot.code });
      usedIdxs.add(pendingUploads[i].idx);
      i++;
    }
    // Fill any remaining buffered files with vision's best guess if the user
    // didn't list enough parts.
    for (const p of pendingUploads) {
      if (usedIdxs.has(p.idx)) continue;
      if (p.vision_best) {
        mappings.push({ idx: p.idx, doc_code: p.vision_best });
        usedIdxs.add(p.idx);
      }
    }
    if (mappings.length) {
      return {
        ...
        method: parts.length === pendingUploads.length ? 'positional' : 'positional_with_fallback'
      };
    }
  }
