...
function armBurst(session_id, { reply, request_id, kind } = {}) {
  let cur = SESSION_BURST.get(session_id);
  if (!cur) cur = { count: 0, timer: null, lastReply: '', request_id: null, kind: 'extra' };
  cur.count += 1;
...
async function drainBurst(session_id) {
  ...
  try {
    ...
    // so without this push the citizen sees no ack at all). Multi-file
    // bursts get تم / المزيد quick-reply buttons; solo files just text.
...
export async function runTurn(args) {
  return withSessionLock(args.session_id, async () => {
    ...
    // returns with a meta marker; everything else is generic.
