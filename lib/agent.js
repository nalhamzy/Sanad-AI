...
5. **Plain text only — NO markdown in output. THIS IS A HARD RULE.** Your replies render on WhatsApp (which does NOT render markdown) and in a basic web chat. Never emit \`**bold**\`, \`*italic*\`, \`# headers\`, \`|tables|\`, triple-backtick code blocks, or \`---\` horizontal rules — on WhatsApp these display as literal asterisks/pipes/dashes/hashes and ruin every message they appear in. Service names, fees, and emphasis all go in plain text:
   ...
   Use line breaks for structure; emoji are fine and encouraged for emphasis (📝 ✅ 💰 ⬜ 👉 🛂). Keep replies short: 3 sentences + a <=5-item bullet list max. Fees: "X.XXX OMR" / "X.XXX ريال".
...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  if (attachment && ['collecting', 'reviewing'].includes(state.status)
    ...
    if (matched && !collected[matched.code]) {
      ...
      // Caption present, no slot matched. Don't guess — let the LLM ask.
      ambiguousAttachment = {
        caption: cap,
        filename: attachment.name || null,
        next_pending: state.docs[state.pending_doc_index ?? 0] || null
      };
      trace.push({
        step: 'auto_record_skipped',
        reason: 'caption_no_match',
        caption: cap.slice(0, 80),
        next_pending: ambiguousAttachment.next_pending?.code || null
      });
    }
  }
  ...
  if (attachment) {
    ...
    if (autoRecorded) {
      ...
      // The active service has 0 required docs (catalogue says none needed)
      // but the user attached something anyway — we already stored it as a
      // supplementary file. Tell the LLM so it acknowledges instead of
      // asking the user "is this an extra?".
      messages.push({
        role: 'user',
        content:
          `[Attachment auto-saved as a SUPPLEMENTARY/EXTRA file]\n` +
          `  filename="${attachment.name || 'file'}"  mime=${attachment.mime || '?'}  size=${attachment.size || 0}B\n` +
          (cap ? `  caption="${cap}"\n` : '') +
          `  reason: this service has no required documents in the catalogue, so the upload was attached as an extra.\n` +
          `  total extras now: ${autoExtra.extra_count}\n\n` +
          `Your reply: briefly acknowledge that the file is attached as a supplementary item and ask the user to confirm submission to Sanad offices (or to add another extra). Do NOT call record_extra_document — already done. Do NOT ask whether it's required vs extra — there is no required slot.`
      });
    } else if (ambiguousAttachment) {
      // Caption present but didn't match any pending slot. The user is
      // either uploading the wrong doc OR adding a supplementary file.
      // Ask them — never silently slot it.
      const next = ambiguousAttachment.next_pending;
      const nextLabel = next
        ? `${next.code} — ${next.label_en || ''}${next.label_ar ? ' / ' + next.label_ar : ''}`
        : '(none — all required docs collected)';
      messages.push({
        role: 'user',
        content:
          `[Attachment uploaded but NOT auto-recorded — caption did not match any pending slot]\n` +
          `  filename="${ambiguousAttachment.filename || 'file'}"  mime=${attachment.mime || '?'}  size=${attachment.size || 0}B\n` +
          `  caption="${ambiguousAttachment.caption.replace(/"/g, '\\"')}"\n` +
          `  next required slot: ${nextLabel}\n\n` +
          `The caption suggests this is NOT the next required document. Two real possibilities:\n` +
          `  (a) the user uploaded the WRONG file and meant to upload "${next?.label_en || next?.code || 'the next required doc'}"\n` +
          `  (b) the user is adding a SUPPLEMENTARY/EXTRA file beyond the required list\n` +
          `Your reply: ask ONE short clarifying question (in the user's language) like:\n` +
          `  "Is this for [${next?.label_en || next?.code || 'the next required doc'}], or is it an extra/supporting file?"\n` +
          `On the user's NEXT turn, based on their answer, call ONE of:\n` +
          `  • record_document(doc_code="${next?.code || '<code>'}", caption=<their caption>)  → if they confirm it's for the required slot\n` +
          `  • record_extra_document(caption=<their caption>, original_name="${ambiguousAttachment.filename || ''}")  → if they confirm it's supplementary\n` +
          `Do NOT call either tool right now — wait for the user's clarification.`
      });
    } else {
