...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  const TRIAGE_TRIGGER_RE = /(?:(?:لست|مش|مو|غير)\s*متاكد|لا\s*اعرف\s*(?:الخدمه|اي\s*خدمه|ايش\s*احتاج|وش\s*احتاج)|ما\s*(?:اعرف|ادري|دري)\s*(?:الخدمه|وش|ايش|اي\s*خدمه)|مدري\s*(?:وش|ايش|الخدمه)|not\s*sure|unsure|don\s*t\s*know\s*(?:which|what)|help\s*me\s*choose)/i;
  if (!GATES_DISABLED && raw && typeof raw === 'string' && !attachment &&
