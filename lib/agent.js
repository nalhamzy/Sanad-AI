...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  const recordedThisTurn = !!(
    autoRecorded ||
    autoExtra ||
    (bufferFlushed && (bufferFlushed.recorded?.length || bufferFlushed.extras?.length)) ||
    trace.some(t => t.step === 'tool_v2' && /^record_/.test(t.name || '') && t.ok !== false)
  );
  if (claimedSave && attachment && !recordedThisTurn) {
    trace.push({
      step: 'hallucination_guard_fired',
      reason: 'reply_claims_save_but_no_record_tool_ran',
    ...
    // Replace the misleading reply with a truthful holding message.
    // The citizen will see the file as an "extra" on their request and the
    // office can re-classify it during review.
    finalReply = '📥 استلمت الملف وحفظته كملف إضافي. أخبرني ما هو حتى أضعه في خانته الصحيحة (مثل: "هذه البطاقة المدنية").';
  }
