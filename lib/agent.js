...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  const TRIAGE_TRIGGER_RE = /(?:لست\s*متاكد|لا\s*اعرف\s*(?:الخدمه|اي\s*خدمه|ايش\s*احتاج|وش\s*احتاج)|مش\s*متاكد|ما\s*(?:اعرف|ادري)\s*(?:الخدمه|وش|ايش|اي\s*خدمه)|غير\s*متاكد|not\s*sure|unsure|don\s*t\s*know\s*(?:which|what)|help\s*me\s*choose)/i;
