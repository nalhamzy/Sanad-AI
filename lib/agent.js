...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  // skip __btn__ messages). Single digits 1/2/3 only, so a 4–6 digit OTP still
  // reaches the OTP-refusal gate.
  if (typeof raw === 'string' && !raw.startsWith('__btn__:') && !attachment) {
    const _digit = (raw.trim().replace(/[٠-٩]/g, d => '٠١٢٣٤٥٦٧٨٩'.indexOf(d)).match(/^([123])$/) || [])[1];
