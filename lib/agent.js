...
//   "✅ استلمت 5 ملفات وأرسلتها للمكتب."
// Two tracks: 'extra' (supplementary docs) vs 'replacement' (response to
// rejected docs). The summary string differs.
const BURST_QUIET_MS = 4500;
const PENDING_BURST = new Map();   // session_id → { count, kind, request_id, timer }

function scheduleBurstSummary({ session_id, request_id, kind }) {
  const cur = PENDING_BURST.get(session_id) || { count: 0, kind, request_id };
  cur.count += 1;
  // If the kinds differ in a single burst, prefer 'replacement' since it's
  // the more meaningful event for the citizen.
  if (kind === 'replacement') cur.kind = 'replacement';
  cur.request_id = request_id || cur.request_id;
  if (cur.timer) clearTimeout(cur.timer);
  cur.timer = setTimeout(() => fireBurstSummary(session_id), BURST_QUIET_MS);
  PENDING_BURST.set(session_id, cur);
}

async function fireBurstSummary(session_id) {
  const cur = PENDING_BURST.get(session_id);
  if (!cur) return;
  PENDING_BURST.delete(session_id);
  const n = cur.count;
  // Arabic plural rules (rough): 1=singular, 2=dual, 3-10=jam', 11+=mufrad-mansoub.
  const wordFile = n === 1 ? 'ملفاً' : (n === 2 ? 'ملفين' : (n <= 10 ? 'ملفات' : 'ملفاً'));
  const headline = cur.kind === 'replacement'
    ? `✅ استلمت ${n} ${wordFile} ${n > 1 ? 'محدَّثة' : 'محدَّثاً'} وأرسلتها إلى مكتب سند للمراجعة.`
    : `✅ استلمت ${n} ${wordFile} ${n > 1 ? 'إضافية' : 'إضافياً'} وأرسلتها إلى المكتب المتولّي طلبك.`;
  // Conversational follow-up — invites the citizen to confirm they're done
  // or to send anything they missed. Keeps the thread moving without
  // requiring the office to chase.
  const followUp = '\n\nهل تبقّى عندك ملف لم نُرفقه بعد؟ أرسله الآن. إذا انتهيت، اكتب **تم** أو **خلصت** ونتابع.';
  const text = headline + followUp;
  try {
    await storeMessage({
      session_id,
    ...
    // For WhatsApp-native sessions, push the summary to the citizen's phone
    // directly (the webhook adapter already returned '' for each file in
    // the burst, so without this the citizen gets no ack at all).
    if (typeof session_id === 'string' && session_id.startsWith('wa:')) {
      const phone = session_id.replace(/^wa:/, '');
      sendWhatsAppText(phone, text).catch(e => console.warn('[burst-summary wa]', e.message));
    }
  } catch (e) {
    console.warn('[burst-summary]', e.message);
  }
}
...
async function _runTurnLocked({ session_id, user_text, attachment, citizen_phone }) {
  ...
  } else if (state.status === 'queued' || state.status === 'claimed' || state.status === 'in_progress' || state.status === 'needs_more_info' || state.status === 'awaiting_payment') {
  ...
  if (attachment && state.request_id) {
    try {
      ...
      if (rejRows[0]) {
        ...
        // Stay silent now; one summary fires after BURST_QUIET_MS of idle.
        scheduleBurstSummary({ session_id, request_id: state.request_id, kind: 'replacement' });
        return { reply: '', state };
      }
    } catch (e) {
    ...
    try {
      ...
      scheduleBurstSummary({ session_id, request_id: state.request_id, kind: 'extra' });
      return { reply: '', state };
    } catch (e) {
