...
async function handleInFlight({ state, raw, trace, attachment, session_id }) {
  ...
  // Burst suppression: WhatsApp delivers one webhook per file. When the
  // citizen rapidly sends 3 photos, this handler runs 3 times. We don't
  // want to spam them with 3 identical acks. Strategy: the first attachment
  // in a burst gets a normal reply; subsequent attachments within 12s set
  // reply='' so the WhatsApp adapter (routes/whatsapp.js line 98) stays
  // silent. State.last_extra_ack_ts tracks the window.
  const BURST_WINDOW_MS = 12_000;
  const now = Date.now();
  const inBurst = !!attachment
    && state.last_extra_ack_ts
    && (now - Number(state.last_extra_ack_ts) < BURST_WINDOW_MS);
  ...
  if (attachment && state.request_id) {
    try {
      ...
      if (rejRows[0]) {
        ...
        state.last_extra_ack_ts = now;
        return {
          reply: inBurst ? '' : `✅ استلمت النسخة الجديدة من "${slot.label || slot.doc_code}" وأرسلتها إلى مكتب سند للمراجعة.`,
          state
        };
      }
    } catch (e) {
    ...
    try {
      ...
      state.last_extra_ack_ts = now;
      return {
        reply: inBurst ? '' : '✅ استلمت ملفك الإضافي (والملفات اللاحقة في نفس الجلسة) — أرسلتها إلى مكتب سند المتولي طلبك.',
        state
      };
    } catch (e) {
