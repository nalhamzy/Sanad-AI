...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  // typing yes/no/extra in Arabic. Drained via the burst layer so a multi-
  // file batch still collapses to one message; this only kicks in for solo
  // file turns where the bot is asking a clarification.
  //
  // Case: ambiguous attachment ("looks like X — save as X, or extra?").
  // Three buttons map to text tokens the existing flush/parse logic accepts.
  let _buttons = null;
  if (finalReply && ambiguousAttachment) {
    const next = ambiguousAttachment.next_pending;
    const vBest = ambiguousAttachment.vision_best;
    // Prefer vision's best guess as the slot label, fall back to the next
    // pending required slot. Slice to Meta's 20-char button-title cap.
    ...
    trace.push({ step: 'attached_buttons', case: 'ambiguous_doc', count: 3 });
  }
