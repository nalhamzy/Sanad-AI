...
async function drainBurst(session_id) {
  ...
  try {
    ...
    if (typeof session_id === 'string' && session_id.startsWith('wa:')) {
      ...
      if (n >= 2 || _implicitStarted) {
        // Buttons MUST match the actual state (prod bug: a no-service burst
        // got "finish uploading / send more", which dead-ended into "اختر
        // الخدمة" — the citizen tapped confirm and looped):
        //   • No service yet (files arrived before any service was chosen) →
        //     offer service picks, never submit/more (nothing to submit).
        //   • Service active (incl. a just implicit-started one) → the unified
        //     confirm-and-send set; confirm is ALWAYS available even with
        //     partial collection (the citizen knows when they're done).
        await sendButtons(_noService
