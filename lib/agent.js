...
async function runHeuristic({ state, raw, trace }) {
  ...
  if (result.launch_code) {
    ...
    // fee_omr is null on launch services under the v3 fixed-pricing model — the
    // total is computed from catalog (office_fee + government_fee). For the
    // heuristic confirm prompt we fall back to a placeholder line; the office
    // surfaces the exact total at /send-payment time.
    const feeLine = (typeof s.fee_omr === 'number')
