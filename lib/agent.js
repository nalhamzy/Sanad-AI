...
import { matchService, launchService, getServiceById, LAUNCH_SERVICES, normalize as _normalizeAr } from './catalogue.js';
import { canonicalPhone } from './phone.js';
import { enrichDocsWithArabicLabels } from './doc_labels.js';
...
async function drainBurst(session_id) {
  ...
  const n = cur.count;
  let text;
  // No-service (idle) case: files arrived BEFORE the citizen chose a service
  // (buffered by the pre-service interceptor). Guide them to pick — for ANY
  // file count — instead of the collecting-style "is your file complete?" ack
  // (which assumes an active request + checklist). Prevents the prod report of
  // a confusing "received N files / complete?" prompt with no service chosen.
  let _stIdle = null; try { _stIdle = await loadSession(session_id); } catch {}

  // ── Implicit service acceptance (prod loop fix, +96892888715 2026-06-17) ──
  // A service was PROPOSED ("هل نبدأ؟" → state.pending_service_id set) and the
  // citizen replied by UPLOADING files instead of tapping «نعم، ابدأ».
  // Uploading IS the "yes": start that service now so the buffered files flush
  // into its checklist (start_submission attaches pending_uploads + clears the
  // buffer), instead of the old behaviour — re-asking «اختر الخدمة» with
  // dead-end confirm buttons, which trapped the citizen in a loop even though
  // they HAD confirmed (by uploading) and sent their files.
  let _implicitStarted = false;
  if (_stIdle && _stIdle.pending_service_id
      && ((_stIdle.docs) || []).filter(d => !isPlaceholderDoc(d)).length === 0
      && ((_stIdle.pending_uploads) || []).length > 0) {
    try {
      const r = await TOOL_IMPL_V2.start_submission(
        { state: _stIdle, session_id, citizen_phone: null, trace: [] },
        { service_id: _stIdle.pending_service_id }
      );
      if (r?.ok) {
        delete _stIdle.pending_service_id;
        delete _stIdle.pending_service_name_ar;
        await saveSession(session_id, _stIdle);
        _implicitStarted = true;
      }
    } catch (e) { console.warn('[drainBurst implicit-start]', e?.message || e); }
  }

  // A service IS selected but its catalogue row has NO required-doc checklist
  // (docs=[]). Buffered uploads can't fill a slot, so move them straight to
  // `extras` here — otherwise the single-file branch below returns without
  // flushing and the files orphan in pending_uploads (submit_request files
  // `collected` + `extras` only, never pending_uploads). Covers solo + multi.
  if (_stIdle && (_stIdle.service_id || _stIdle.pending_service_id)
      && ((_stIdle.pending_uploads) || []).length
      && ((_stIdle.docs) || []).filter(d => !isPlaceholderDoc(d)).length === 0) {
    _stIdle.extras = Array.isArray(_stIdle.extras) ? _stIdle.extras : [];
    for (const upload of _stIdle.pending_uploads) {
      _stIdle.extras.push({
        idx: upload.idx, url: upload.url, storage_url: upload.url,
        name: upload.name || null, mime: upload.mime || null,
        caption: upload.caption || upload.name || null, at: Date.now()
      });
    }
    _stIdle.pending_uploads = [];
    try { await saveSession(session_id, _stIdle); } catch {}
  }

  // "No service yet" = the citizen hasn't picked one. Gate on service_id /
  // pending_service_id — NOT on docs.length. A selected service can legitimately
  // have an EMPTY required-doc checklist (many catalogue rows list no documents),
  // and the old `docs.length === 0` check wrongly fired «اختر الخدمة أولاً» right
  // after the citizen selected such a service and then uploaded a file.
  const _noService = _stIdle && !_stIdle.service_id && !_stIdle.pending_service_id;
  const _hasBuffered = _stIdle && ((_stIdle.pending_uploads) || []).length > 0;
  if (_noService && _hasBuffered) {
    const ids = Array.isArray(_stIdle.last_search_results) ? _stIdle.last_search_results.slice(0, 3) : [];
    let lines = '';
    if (ids.length) {
      const glyphs = ['1️⃣', '2️⃣', '3️⃣']; const rows = [];
      for (let i = 0; i < ids.length; i++) { const r = await getServiceById(ids[i]); if (r) rows.push(`${glyphs[i]} ${r.name_ar || r.name_en}`); }
      lines = rows.join('\n');
    }
    const fw = n <= 1 ? 'ملفك' : (n === 2 ? 'ملفيك' : 'ملفاتك');
    text = lines
      ? `📥 وصلتني ${fw} 👍\n\nاختر الخدمة المطلوبة أولاً لأبدأ طلبك:\n\n${lines}\n\nاضغط الرقم أو اكتب اسم الخدمة.`
      : `📥 وصلتني ${fw} 👍\n\nأخبرني بالخدمة التي تريدها (مثل: «تجديد رخصة القيادة») لأبدأ طلبك.`;
  } else if (n <= 1 && !_implicitStarted) {
    ...
    // (When _implicitStarted, fall through to the flush+summary branch so a
    // single file uploaded against a proposed service still gets a receipt.)
    ...
    const wordFile = n === 1 ? 'ملف' : n === 2 ? 'ملفين' : (n <= 10 ? 'ملفات' : 'ملفاً');
    ...
    if (st && Array.isArray(st.pending_uploads) && st.pending_uploads.length) {
      ...
      // Move everything we couldn't slot into `extras` so it still reaches the
      // office — INCLUDING all files when the selected service has no required-
      // doc checklist (docsAll empty → flushed.length === 0). Without this the
      // files sit orphaned in pending_uploads and submit_request never files
      // them (it dispatches `collected` + `extras` only). Same path handles
      // surplus: a service with 1 required doc but 5 files sent → 1 slot + 4
      // extras. (Real prod bug +96892888715 2026-05-09: surplus read as "lost".)
      // submit_request reads `storage_url` (not `url`) when filing extras — set
      // both so the file link actually survives to request_document.
      const surplus = st.pending_uploads.slice(flushed.length);
      if (flushed.length || surplus.length) {
        if (flushed.length) st.collected = collectedNow;
        if (surplus.length) {
    ...
    const filledCount = docs.length - pending.length;
    const extrasN = ((st && st.extras) || []).length;
    const checklist = renderChecklist(st);

    // Citizen feedback 2026-05-09: bot said "📥 استلمت 4 ملفات" but the
    // checklist only marked 3 ✅ (the 4th got dropped — duplicate URL,
    // vision misclassification to a filled slot, or download failure).
    // The user read this as the bot losing files. Make the count honest
    // ONLY when there's an active service (docs.length > 0) — otherwise
    // files sitting in pending_uploads with no slots to fill would
    // wrongly read as "dropped".
    //   accountedFor = filledCount + extrasN     (real placement)
    //   dropped      = max(0, n - accountedFor)  (lost in flight)
    const accountedFor = filledCount + extrasN;
    const dropped = docs.length > 0 ? Math.max(0, n - accountedFor) : 0;

    let headline;
    if (cur.kind === 'replacement') {
      headline = `📥 استلمت ${n} ${wordFile} محدَّثة وأرسلتها إلى مكتب سند للمراجعة.`;
    } else if (dropped > 0) {
      // The honest case: tell the citizen exactly how many were placed
      // vs how many didn't make it. Avoids the "bot said 4 / shows 3"
      // discrepancy. Singular vs plural for "ملف لم يكتمل".
      const dropWord = dropped === 1 ? 'لم يكتمل تحميله' : 'لم تكتمل تحميلاتها';
      headline = `📥 وصلتني ${n} ${wordFile} — رتّبت ${accountedFor} منها، و${dropped} ${dropWord} (أعد إرسال الناقص إن أردت).`;
    } else if (hasRequest) {
  ...
  try {
    ...
    if (typeof session_id === 'string' && session_id.startsWith('wa:')) {
      ...
      const sendButtons = async (btns) => {
        // Button send → plain-text fallback → if BOTH fail, log loudly.
        // The consolidated burst reply is the ONLY ack the citizen gets for
        // a multi-file upload, so a total send failure must be visible to
        // ops (was previously swallowed by an empty catch).
        const fallbackText = () =>
          sendWhatsAppText(phone, text).catch(e =>
            console.error('[drainBurst] BOTH button+text send failed for', session_id, '—', e?.message || e));
        try {
          const { sendWhatsAppButtons } = await import('./whatsapp_send.js');
          const r = await sendWhatsAppButtons(phone, text, btns);
          if (!r.ok) {
            console.warn('[drainBurst] button send failed, falling back to text:', r?.error);
            fallbackText();
          }
        } catch (e) {
          console.warn('[drainBurst] button send threw, falling back to text:', e?.message || e);
          fallbackText();
        }
      };
      // Decide buttons from the CURRENT state (the implicit-start / flush above
      // may have started a service or filled slots). Reload so a SOLO file
      // (n=1) that FINISHES the checklist still gets the انتهيت/سأرسل buttons —
      // previously only n>=2 bursts did, so a single finishing file rendered
      // "اضغط ✅ انتهيت من الرفع" with NO button and the citizen had to TYPE
      // "انتهيت" (prod report +96892888715, 2026-06-17).
      let _stBtn = null; try { _stBtn = await loadSession(session_id); } catch {}
      const _docsN  = _stBtn ? ((_stBtn.docs) || []).filter(d => !isPlaceholderDoc(d)).length : 0;
      const _filesN = _stBtn ? (Object.keys(_stBtn.collected || {}).length
                              + (_stBtn.extras || []).length
                              + (_stBtn.pending_uploads || []).length) : 0;
      const _activeSubmission = !!_stBtn && ['collecting', 'reviewing'].includes(_stBtn.status) && _filesN > 0;

      if (_activeSubmission) {
        // Active service with ≥1 file → the unified confirm-and-send set, for
        // ANY burst size. Confirm is always available even with partial docs;
        // the citizen knows when they're done. THIS is what gives a solo
        // finishing file its tappable ✅ انتهيت من الرفع button.
        await sendButtons([button('review:submit'), button('burst:more'), button('service:cancel')]);
      } else if (n >= 2 || _implicitStarted) {
        // Multi-file (or just implicit-started) with NO active service → offer
        // service picks, never submit/more (nothing to submit — would dead-end
        // into "اختر الخدمة").
        await sendButtons(_docsN === 0
          ? [button('discover:license'), button('discover:title'), button('discover:cr')]
          : [button('review:submit'), button('burst:more'), button('service:cancel')]);
...
async function handleButtonIntent({ session_id, state, btn_id, attachment, citizen_phone, trace }) {
  ...
  if (btn_id === 'review:submit') {
    ...
    const collected = state.collected || {};
    // A slot counts as filled by a FILE (storage_url) OR a TYPED value (e.g. a
    // phone number / email captured as text — matched_via 'typed', no file).
    const haveCount = docs.filter(d => collected[d.code]?.storage_url || collected[d.code]?.matched_via === 'typed').length;
    ...
    try {
      ...
      if (result?.ok) {
        ...
        // When extras > 0, the bare "استلمت N مستندات" line hides the
        // breakdown. Be explicit so the citizen can match it to the
        // checklist (Y required + Z extra).
        const fileWord = total === 1 ? 'مستند' : 'مستندات';
        const breakdownLine = extrasN > 0
          ? `\n   (${haveCount} ضمن المطلوب + ${extrasN} ملف إضافي)`
          : '';
        // Render the FINAL ✅/⏳ checklist so the citizen can see what
        // shipped vs what's still missing.
        const checklist = renderChecklist(state);
        // Citizen feedback 2026-05-09: the previous reassurance line
        // ("don't worry, the office will contact you if needed") felt
        // dismissive — like the bot was brushing off responsibility for
        // the missing docs. Replaced with a factual statement of what's
        // in the file and what happens next, no reassurance theater.
        const partial = haveCount < docs.length
          ? `\n\n📋 الملف المُرسل: ${haveCount} من ${docs.length} مستندات. المكتب سيراجع الملف ويُخبرك مباشرة إن احتاج إضافات.`
          : '';
        ...
        const reply =
          `✅ تم الإرسال — استلمت ${total} ${fileWord}.${breakdownLine}\n` +
          `🆔 *#R-${result.request_id}*` +
          partial +
  ...
  // Triage shortcut — citizen taps "not sure which service". Enter a light
  // intake state: collect a free-text description (+ optional papers), then
  // submit_triage dispatches it service-less to the marketplace, where the
  // claiming office sets the real service via reclassify.
  if (btn_id === 'discover:unsure') {
    state.status = 'triage';
    state.intent_summary = '';
    if (!Array.isArray(state.pending_uploads)) state.pending_uploads = [];
    await saveSession(session_id, state);
    const reply = '👍 لا مشكلة إن لم تكن متأكداً من الخدمة المطلوبة.\n\n' +
      'اكتب لي بكلماتك ماذا تحتاج (وأرفق صورة أي ورقة لديك إن وُجدت)، ' +
      'وسأرسل طلبك إلى مكتب سند ليحدّد لك الخدمة المناسبة ويُكمل معك. 📨';
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
    trace.push({ step: 'btn_triage_start' });
    return { reply, state, trace, _buttons: [button('service:cancel')] };
  }

  // Triage submit — citizen taps "send my request to office" after describing.
  if (btn_id === 'triage:submit') {
    const summary = String(state.intent_summary || '').trim();
    if (!summary) {
      const reply = '✍️ اكتب لي أولاً وصفاً قصيراً لما تحتاجه، ثم اضغط «📨 أرسل طلبي للمكتب».';
      await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
      trace.push({ step: 'btn_triage_submit_empty' });
      return { reply, state, trace, _buttons: [button('triage:submit'), button('service:cancel')] };
    }
    ...
    if (r?.ok) {
      const reply = '✅ تمام! أرسلت طلبك إلى مكتب سند.\n\n' +
        `📝 «${summary}»\n\n` +
        'سيراجعه موظف المكتب، يحدّد الخدمة المناسبة، ويُكمل معك الخطوات هنا. ' +
        'ستصلك التحديثات في هذه المحادثة. 🙏';
      await storeMessage({ session_id, request_id: r.request_id, direction: 'out', actor_type: 'bot', body_text: reply });
      await saveSession(session_id, state);
      trace.push({ step: 'btn_triage_submitted', request_id: r.request_id });
      return { reply, state, trace, _buttons: [button('status:check')] };
    }
    const reply = '⚠️ تعذّر إرسال طلبك الآن. حاول مرة أخرى بعد قليل.';
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
    trace.push({ step: 'btn_triage_submit_failed', error: r?.error });
    return { reply, state, trace, _buttons: [button('triage:submit'), button('service:cancel')] };
  }
      ...
      if (rows[0]?.id) {
        ...
        if (r?.ok) {
          ...
          const docsBlock = docs.kind === 'list'
            ? `المطلوب منك:\n${docs.text}\n\n` +
              `${collectStartLine(r.required_documents)}`
            : `${docs.text}`;
  ...
  //
  // CX iter-8 (2026-05-09, citizen feedback +96892888715): the previous
  // version called start_submission DIRECTLY on pick:N — meaning the
  // citizen committed to the service without ever seeing what documents
  // were required, what the fee was, or how long it would take. They
  // typed "اجدد نشاط سجل", picked option 2, and got "بدأت طلب تجديد
  // السجل التجاري" with no doc list (because that catalogue row has
  // generic labels). They had no way to back out before committing.
  //
  // New flow: pick:N → show CONFIRMATION CARD (service + entity + fee +
  // time + numbered doc list) → buttons (✓ نعم / 🔍 خدمة أخرى / ✕ إلغاء).
  // confirm:yes is what triggers start_submission. This mirrors the
  // launch-service handleConfirming flow so behavior is uniform.
  // pick:1..pick:10 — index into state.last_search_results. Entity-browse
  // lists up to 10 services and the search list up to 6, and the body text
  // invites "اكتب اسم الخدمة (أو رقمها)", so picks beyond 3 must resolve too.
  // Out-of-range indices fall through harmlessly (the `if (sid)` guard below).
  const PICK_RE = /^pick:(\d{1,2})$/;
  ...
  if (pickMatch && Array.isArray(state.last_search_results)) {
    ...
    if (sid) {
      try {
        const row = await getServiceById(sid);
        if (row) {
          const card = renderServiceConfirmCard(row);
          state.pending_service_id = sid;
          state.pending_service_name_ar = row.name_ar || row.name_en;
          state.last_offered_buttons = ['confirm:yes', 'service:switch', 'service:cancel'];
          delete state.last_search_results;
          await saveSession(session_id, state);
          await storeMessage({
            session_id, request_id: null,
            direction: 'out', actor_type: 'bot', body_text: card
          });
          trace.push({ step: 'btn_pick_confirm_card', service_id: sid });
          return {
            reply: card, state, trace,
            _buttons: [
              button('confirm:yes', '✓ نعم، ابدأ'),
              button('service:switch'),
              button('service:cancel')
            ]
          };
  ...
  // confirm:yes after pick:N (state.pending_service_id is set) — NOW we
  // actually call start_submission. The card the citizen just confirmed
  // already showed them docs/fee/time, so the post-start reply can be
  // brief: "✅ ready, send the files in any order."
  if (btn_id === 'confirm:yes' && state.pending_service_id && !state.pending_cancel) {
    const sid = state.pending_service_id;
    try {
      const r = await TOOL_IMPL_V2.start_submission(
        { state, session_id, citizen_phone, trace },
        { service_id: sid }
      ...
      if (r?.ok) {
        const svcName = r.name_ar || r.name_en;
        delete state.pending_service_id;
        delete state.pending_service_name_ar;
        const reply = `✅ تمام — جهزنا ملف *${svcName}*.\n${collectStartLine(state.docs)}`;
        await storeMessage({
          session_id, request_id: null,
          direction: 'out', actor_type: 'bot', body_text: reply
        });
        state.last_offered_buttons = ['service:switch', 'service:cancel'];
        await saveSession(session_id, state);
        trace.push({ step: 'btn_confirm_pick_started', service_id: sid });
        return {
          reply, state, trace,
          _buttons: [
            button('service:switch'),
      ...
      trace.push({ step: 'btn_confirm_pick_threw', error: e.message });
    }
  }
...
export async function ensureCitizen({ phone, name }) {
  // Canonicalize so WhatsApp (96892888715) and web OTP (+96892888715) share one row.
  const p = canonicalPhone(phone) || phone;
  const { rows } = await db.execute({ sql: `SELECT id FROM citizen WHERE phone=?`, args: [p] });
  if (rows.length) return rows[0].id;
  const r = await db.execute({ sql: `INSERT INTO citizen(phone,name) VALUES (?,?)`, args: [p, name || null] });
...
async function __runTurnLockedInner({ session_id, user_text, attachment, citizen_phone, trace }) {
  ...
  if (!raw && !attachment) {
    // Empty payload — fired when WhatsApp sends a content type the bot
    // doesn't process (sticker / contact card / location / voice note).
    // The old wording felt dismissive — citizens read it as "I don't
    // understand you". The new wording names the likely cause (voice /
    // sticker) and gives one concrete next step.
    const reply = '🤔 وصلتني رسالة لا أستطيع قراءتها (ربما تسجيل صوتي أو ملصق). اكتب طلبك بالنص، أو أرسل صورة JPG/PNG أو ملف PDF.';
  ...
  // ── Pre-service attachment buffering (deterministic + coalesced) ──────────
  // A file arrived but NO service is being collected yet (idle / confirming /
  // discovery — not collecting/reviewing, not an in-flight request). Handle it
  // deterministically here instead of letting the LLM run per file. In prod,
  // running the LLM per file produced a SEPARATE "you sent an image, pick a
  // service" message for EACH image (uncoalesced), plus a connection error when
  // the LLM hiccuped — exactly the "2 images at once is messy / something went
  // wrong" report. We buffer the file and stay silent; the burst layer above
  // arms drainBurst, which sends ONE consolidated "got N files — pick a service"
  // message. The buffered files attach to the chosen service when
  // start_submission flushes pending_uploads.
  //
  // Triage intake takes priority: when the citizen is in the "unsure which
  // service" flow (entered via the 🤔 button), handle their text/photos
  // deterministically — accumulate the description, buffer optional papers,
  // and offer to dispatch. Runs BEFORE the pre-service attachment buffer
  // (which would otherwise fire a "pick a service" message) and before the V2
  // LLM loop — triage intake is linear and needs no model.
  if (state.status === 'triage') {
    return await handleTriage({ session_id, state, raw, attachment, citizen_phone, trace });
  }

  if (attachment && !['collecting', 'reviewing'].includes(state.status)
      && !inFlightStatuses.includes(state.status)) {
    pushPendingUpload(state, attachment, '', null);
    await saveSession(session_id, state);
    trace.push({ step: 'attachment_pre_service_buffered', status: state.status, n: state.pending_uploads.length });
    return { reply: '', state, trace, _burstKind: 'collect' };
  }

  // ── Deterministic typed-value capture (phone / email / number) — BEFORE the LLM
  // A typed required field is recorded here so the slot fills even when the model
  // only "acknowledges" it (Qwen narrates "✅ تم حفظ الرقم" without recording → the
  // request is never created). Also: an affirmative in 'reviewing' for a typed
  // request submits deterministically — the bot must never claim "sent to the
  // office" without a request actually being created. See captureTypedDocValue.
  if (!attachment && raw && !raw.startsWith('__btn__')) {
    if (state.status === 'collecting') {
      const cap = await captureTypedDocValue({ session_id, state, raw, trace });
      if (cap) return cap;
    } else if (state.status === 'reviewing'
               && /^(تأكيد|تاكيد|confirm|نعم|أرسل|ارسل|ابعث|اكتمل|انتهيت|تمام|خلاص|submit|send|done|ok)/i.test(raw.trim())
               && Object.values(state.collected || {}).some(c => c?.matched_via === 'typed')) {
      const sub = await handleButtonIntent({ session_id, state, btn_id: 'review:submit', attachment: null, citizen_phone, trace });
      if (sub) return sub;
    }
  }
  ...
  if (AGENT_V2 && LLM_ENABLED && !inFlightWithAttachment) {
    try {
      ...
      // Runtime LLM failure (Anthropic outage / 401 / 429 / timeout / retired
      // model). Rather than surface an error or strand the citizen, fall
      // through to the deterministic heuristic path below — it can still
      // match services, collect documents, and drive the flow without an LLM.
      // This is the single most important resilience guarantee for the chat:
      // a provider hiccup degrades quality, never breaks the conversation.
      console.error('[agent] runAgentV2 failed, falling back to heuristic:', e?.message || e);
      trace.push({ step: 'v2_failed_fallback_heuristic', error: String(e?.message || e) });
      // continue to the heuristic dispatch below
    }
  }
  ...
  // NOTE: exclude in-flight statuses — once a request is submitted (queued/
  // claimed/…), an extra upload must reach handleInFlight() below, which
  // attaches it to the EXISTING request as a supplementary doc (is_extra=1) and
  // notifies the office. Without this guard a "forgot one more doc" upload after
  // submission wrongly hit "لم نبدأ بعد أي معاملة" (prod report +96892888715).
...
async function runLLMLoop(args) {
  // Resilience wrapper: any uncaught LLM failure (network / 401 / 429 /
  // timeout / retired model) degrades to the deterministic heuristic path
  // so the citizen always gets a useful, on-topic reply — never an error.
  try {
    return await _runLLMLoopInner(args);
  } catch (e) {
    console.error('[agent] runLLMLoop failed, falling back to heuristic:', e?.message || e);
    args.trace?.push?.({ step: 'llm_loop_failed_fallback_heuristic', error: String(e?.message || e) });
    return await runHeuristic({ state: args.state, raw: args.raw, trace: args.trace });
  }
}

async function _runLLMLoopInner({ session_id, state, raw, trace }) {
...
export function sanitizeReply(text, userText) {
  if (!text) return text;
  let t = String(text);
  // SAFETY NET: never leak raw tool-call syntax to the citizen. Some models emit
  // "<tool_code>{...}</tool_code>" (or stray tags) in the content; llm.js normally
  // extracts + executes it, but strip any residual here as a last line of defence.
  // (Prod bug: «خدمات الشرطة» replied with the literal <tool_code> block.)
  t = t.replace(/<tool_code>[\s\S]*?<\/tool_code>/gi, ' ');
  t = t.replace(/<\/?(tool_code|tool_call|function_call)[^>]*>/gi, ' ');
  // If the WHOLE reply is a bare tool-call JSON object, drop it entirely.
  const _tr = t.trim();
  if (/^\{[\s\S]*"name"\s*:/.test(_tr) && /"(arguments|parameters|args)"\s*:/.test(_tr)) t = '';
...
// Common greeting fillers that often follow the main greeting word.
// "اهلا وسهلا" / "السلام عليكم ورحمة الله وبركاته" / "good morning sir".
// We strip these BEFORE measuring leftover content so a polite multi-word
// greeting still classifies as a greeting.
const _GREETING_FILLERS = /(وسهلا|بكم|بك|بكي|عليكم|عليك|ورحمة\s*الله(\s*وبركاته)?|الله\s*يصبحك\s*بالخير|كيف\s*(حالك|الحال|حالكم|حالكي|امورك|اومرك)|كيفك|كيفكم|شخبارك|خبارك|اخبارك|وش\s*اخبارك|عساك\s*(بخير|طيب)|كثيرا?|كتير|جزيلا|جزيلاً|جدا|وايد|a\s*lot|so\s*much|very\s*much|لك|الك|إلك|العافيه|العافية|على\s*مساعدتك|على\s*(?:ال)?مساعده|ما\s*قصرت|ماقصرت|يا\s*أخي|يا\s*أختي|sir|ma'?am|there|friend|how\s*(are|r)\s*(you|u|things)|how\s*(are\s*)?(you|u)\s*doing|👋|🙏|🌹|✨|🌟)/gi;

// Real prod bug from +96892888715 trace 2026-05-10 10:20:15:
// citizen typed "هلا توجد خدمات اهرى" (= "hi, are there other services?") —
// _GREETING_RE matched "هلا " at the start, deterministic catch fired the
// welcome message and the bot ignored the actual question.
// Fix: a greeting catch ONLY fires when the message is mostly-a-greeting.
// After stripping the matched greeting prefix + common fillers + emoji,
// if there's any content word left → it's a query, not a greeting.
function _isMostlyGreeting(text, matchRegex) {
  // Strip Arabic diacritics (tashkeel) + tatweel FIRST — "شكراً" must classify
  // the same as "شكرا". Otherwise a trailing fatHatan/kasra breaks the
  // start-anchored _END lookahead and a polite "شكراً جزيلاً لك" fell through
  // to search (live prod). Diacritics never carry intent here.
  text = String(text).replace(/[ً-ْٰـ]/g, '');
  const m = matchRegex.exec(text);
  if (!m) return false;
  // Remove the greeting prefix from the message.
  let rest = text.slice(m[0].length);
  // Strip greeting fillers.
  rest = rest.replace(_GREETING_FILLERS, ' ');
  // Strip punctuation, whitespace, emoji, common interjections.
  rest = rest.replace(/[!.,،؟?؛:\-—\s]+/g, ' ').trim();
  // If anything substantive (≥2 chars) remains → it's a query, not a greeting.
  return rest.length < 2;
}

export function isGreetingOrHelp(text) {
  if (!text || typeof text !== 'string') return false;
  if (_isMostlyGreeting(text, _GREETING_RE)) return true;
  if (_isMostlyGreeting(text, _HELP_RE))     return true;
  if (_isMostlyGreeting(text, _THANKS_RE))   return true;
  return false;
}

export function greetingIntent(text) {
  if (!text || typeof text !== 'string') return null;
  if (_isMostlyGreeting(text, _THANKS_RE))   return 'thanks';
  if (_isMostlyGreeting(text, _HELP_RE))     return 'help';
  if (_isMostlyGreeting(text, _GREETING_RE)) return 'greeting';
  return null;
}
...
async function runHeuristic({ state, raw, trace }) {
  ...
  if (entMatch) {
    ...
    if (services.length) {
      ...
      const feeOf = (s) => {
        if (s.fee_omr != null && Number.isFinite(Number(s.fee_omr)) && Number(s.fee_omr) > 0) {
          return isAr ? ` · ${Number(s.fee_omr).toFixed(0)} ر.ع` : ` · ${s.fee_omr} OMR`;
        }
        return '';
      };
      const lines = services.map((s, i) => `${i + 1}. ${nameOf(s)}${feeOf(s)}`).join('\n');
      const header = isAr
        ? `🏛 ${entity} — ${services.length} خدمة:\n\n${lines}\n\nاضغط الرقم للتفاصيل.`
        : `🏛 ${entity} — ${services.length} services:\n\n${lines}\n\nType a number for details.`;
      return { reply: header, state };
    }
  }
  ...
  if (result.launch_code) {
    ...
    const docs = s.required_documents.map(d => `• ${d.label_ar || d.label_en || d.code}`).join('\n');
  ...
  // multiple ambiguous — mirror the user's language. Real prod bug from
  // +96892888715 (2026-05-09): user typed "أحتاج ترخيص صحي" (Arabic)
  // and got "Found 5 candidates — pick a number for details:" with
  // English service names + English entity names mixed in. Violates the
  // SYSTEM_V2 "one language per reply" rule. Use name_ar/entity_ar
  // (with English fallback only if Arabic is empty) when query is Arabic.
  state.last_candidates = result.services.map(s => s.id);
  const isAr = /[؀-ۿ]/.test(raw);
  const nameOf   = (s) => isAr ? (s.name_ar   || s.name_en   || '?') : (s.name_en   || s.name_ar);
  const entityOf = (s) => isAr ? (s.entity_ar || s.entity_en || '')  : (s.entity_en || s.entity_ar);
  const feeOf = (s) => {
    if (s.fee_omr != null && Number.isFinite(Number(s.fee_omr)) && Number(s.fee_omr) > 0) {
      return isAr ? ` · ${Number(s.fee_omr).toFixed(0)} ر.ع` : ` · ${s.fee_omr} OMR`;
    }
  ...
  const lines = result.services.map((s, i) => {
    const name = nameOf(s);
    const entity = entityOf(s);
    return `${i + 1}. ${name}${entity ? ` — ${entity}` : ''}${feeOf(s)}`;
  }).join('\n');
  const header = isAr
    ? `🔎 وجدت ${result.count} خدمات قد تناسبك — اضغط الرقم للمتابعة:`
    : `🔎 Found ${result.count} candidates — pick a number to continue:`;
  return {
    reply: `${header}\n\n${lines}`,
    state
  };
...
// Triage intake handler — citizen is UNSURE which service applies. Entered via
// the "🤔 لست متأكد" button (state.status='triage'). Deterministic and
// LLM-independent: accumulate a free-text description, buffer any optional
// papers, and dispatch service-less via submit_triage when the citizen taps
// "📨 أرسل طلبي للمكتب" (or types تم/أرسل). The claiming office then sets the
// real service via reclassify and continues the normal flow.
async function handleTriage({ session_id, state, raw, attachment, citizen_phone, trace }) {
  if (!Array.isArray(state.pending_uploads)) state.pending_uploads = [];
  const summaryBtns = [button('triage:submit'), button('service:cancel')];

  // Optional paper — buffer it; the citizen describes their need separately.
  if (attachment) {
    pushPendingUpload(state, attachment, '', null);
    await saveSession(session_id, state);
    trace.push({ step: 'triage_attachment', n: state.pending_uploads.length });
    const hasSummary = String(state.intent_summary || '').trim().length > 0;
    const reply = hasSummary
      ? `📎 أضفت المرفق (${state.pending_uploads.length}). أرسل المزيد أو اضغط «📨 أرسل طلبي للمكتب».`
      : `📎 أضفت المرفق (${state.pending_uploads.length}). اكتب الآن وصفاً قصيراً لما تحتاجه ثم أرسله للمكتب.`;
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
    return { reply, state, trace, _buttons: hasSummary ? summaryBtns : [button('service:cancel')] };
  }

  let text = String(raw || '').trim();
  // A button tap can reach here as raw text (the __btn__ prefix), since this
  // handler runs before the V2 button gating. Map the two triage buttons to
  // their text equivalents so taps work in every path (including the heuristic
  // fallback when the LLM is down); ignore any stray id.
  if (text.startsWith('__btn__:')) {
    const b = text.slice('__btn__:'.length);
    text = b === 'triage:submit' ? 'تم' : (b === 'service:cancel' ? 'إلغاء' : '');
  }

  // Abandon.
  if (/^(الغاء|إلغاء|cancel|توقف|خروج|رجوع)$/i.test(text)) {
    state = { status: 'idle', collected: {}, pending_doc_index: 0 };
    await saveSession(session_id, state);
    const reply = 'تمام، ألغيت الطلب. اكتب لي وقتما تريد المساعدة. 👋';
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
    trace.push({ step: 'triage_cancelled' });
    return { reply, state, trace };
  }

  // Submit signal typed instead of tapped.
  const isSubmit = /^(تم|ارسل|أرسل|ارسله|أرسله|أرسلها|ارسلها|انتهيت|خلاص|ابعت|send|done)$/i.test(text);
  if (isSubmit) {
    const summary = String(state.intent_summary || '').trim();
    if (!summary) {
      const reply = '✍️ اكتب لي أولاً وصفاً قصيراً لما تحتاجه، ثم أرسله للمكتب.';
      await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
      return { reply, state, trace, _buttons: [button('service:cancel')] };
    }
    const r = await TOOL_IMPL_V2.submit_triage({ state, session_id, citizen_phone, trace }, { intent_summary: summary });
    if (r?.ok) {
      const reply = '✅ تمام! أرسلت طلبك إلى مكتب سند.\n\n' +
        `📝 «${summary}»\n\n` +
        'سيراجعه موظف المكتب، يحدّد الخدمة المناسبة، ويُكمل معك هنا. ستصلك التحديثات في هذه المحادثة. 🙏';
      await storeMessage({ session_id, request_id: r.request_id, direction: 'out', actor_type: 'bot', body_text: reply });
      await saveSession(session_id, state);
      trace.push({ step: 'triage_submitted', request_id: r.request_id });
      return { reply, state, trace, _buttons: [button('status:check')] };
    }
    const reply = '⚠️ تعذّر إرسال طلبك الآن. حاول مرة أخرى بعد قليل.';
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
    trace.push({ step: 'triage_submit_failed', error: r?.error });
    return { reply, state, trace, _buttons: summaryBtns };
  }

  // Accumulate the description across turns.
  if (text) {
    const prev = String(state.intent_summary || '').trim();
    state.intent_summary = ((prev ? prev + ' ' : '') + text).slice(0, 1000);
    await saveSession(session_id, state);
    trace.push({ step: 'triage_text', len: state.intent_summary.length });
    const reply = `📝 سجّلت طلبك:\n«${state.intent_summary}»\n\n` +
      'أضف تفاصيل أو أرفق صور أوراقك إن أردت، ثم اضغط «📨 أرسل طلبي للمكتب».';
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
    return { reply, state, trace, _buttons: summaryBtns };
  }

  // Empty turn — re-prompt.
  const reply = 'اكتب لي وصفاً قصيراً لما تحتاجه (وأرفق أوراقك إن وُجدت)، ثم اضغط «📨 أرسل طلبي للمكتب».';
  await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
  return { reply, state, trace, _buttons: summaryBtns };
}
...
async function handleCollecting({ session_id, state, raw, attachment, trace }) {
  ...
  const _lbl = doc.label_ar || doc.label_en;
  return {
    reply: typedDocKind(doc)
      ? `اكتب **${_lbl}** في رسالة.`
      : `محتاجين **${_lbl}**. ابعثها كصورة أو PDF.`,
    state
  };
}

// ── Typed-value document capture (phone number / email / typed field) ────────
// Some services ask for a TYPED value (the new phone number for "تحديث رقم
// الهاتف", a valid email for customs registration) rather than a file upload.
// The catalogue still marks these type='file', and the LLM tends to merely
// *acknowledge* the value ("✅ تم حفظ الرقم") WITHOUT calling record_document — so
// the slot never fills, the state never reaches 'reviewing', submit_request never
// runs, and the request is silently never created (real prod bug, +96892888715,
// "خدمة تحديث رقم الهاتف"). These helpers capture such values deterministically,
// BEFORE the LLM, so the flow always advances and the request reaches the office.
export function typedDocKind(d) {
  const t = String(d?.type || '').toLowerCase();
  if (['email', 'number', 'date', 'text'].includes(t)) return t;
  const lbl = ((d?.label_ar || '') + ' ' + (d?.label_en || '') + ' ' + (d?.code || '')).toLowerCase();
  if (/e-?mail|بريد|ايميل|إيميل/.test(lbl)) return 'email';
  if (/الرقم|\bرقم\b|number|هاتف|phone|جوال|mobile|نمبر/.test(lbl)) return 'number';
  return null; // treat as a file-upload slot
}
export function typedValueValid(kind, raw) {
  const v = String(raw || '').trim();
  if (!v) return false;
  if (kind === 'email')  return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(v);
  if (kind === 'number') return /^[\d\s+\-()]+$/.test(v) && v.replace(/\D/g, '').length >= 4;
  return v.length >= 2; // text / date — any non-trivial value
}
function activeRequiredDocs(state) {
  if (Array.isArray(state.docs) && state.docs.length) return state.docs.filter(d => !isPlaceholderDoc(d));
  const s = state.service_code ? launchService(state.service_code) : null;
  return (s?.required_documents || []).filter(d => !isPlaceholderDoc(d));
}

// Returns a turn result when it captured a typed value, else null (let the
// normal file/LLM flow handle it — pending slot is a file, or the text isn't a
// plausible value, e.g. the citizen asked a question).
export async function captureTypedDocValue({ session_id, state, raw, trace }) {
  const v = String(raw || '').trim();
  if (!v) return null;
  if (/^(\/?reset|cancel|إلغاء|خدمة أخرى|new service|stop|تم|خلص|done|finished|انتهيت|\?|؟)/i.test(v)) return null;
  const docs = activeRequiredDocs(state);
  if (!docs.length) return null;
  const collected = state.collected || (state.collected = {});
  const pending = docs.find(d => !collected[d.code]);
  if (!pending) return null;
  const kind = typedDocKind(pending);
  if (!kind) return null;                       // pending slot is a file → not ours
  // Persist the doc list so the downstream submit (submit_request /
  // review:submit) can file the typed value even on the v1 path.
  if (!Array.isArray(state.docs) || !state.docs.length) state.docs = docs;
  if (!typedValueValid(kind, v)) return null;   // not a plausible value → let the LLM reply
  collected[pending.code] = { caption: v, matched_via: 'typed', mime: 'text/plain', size: v.length };
  trace?.push?.({ step: 'typed_value_captured', code: pending.code, kind });
  let idx = 0;
  while (idx < docs.length && collected[docs[idx].code]) idx++;
  state.pending_doc_index = idx;
  const stillPending = docs.filter(d => !collected[d.code]);
  let reply, _buttons;
  if (stillPending.length === 0) {
    state.status = 'reviewing';
    reply = `✅ تم الحفظ: ${v}\n\n📦 طلبك جاهز. اضغط *✅ انتهيت* أو اكتب «أرسل» لإرساله إلى مكتب سند.`;
    _buttons = [button('review:submit'), button('service:cancel')];
    state.last_offered_buttons = _buttons.map(b => String(b.id));
  } else {
    const nx = stillPending[0];
    const nxLabel = nx.label_ar || nx.label_en || nx.code;
    reply = typedDocKind(nx)
      ? `✅ تم الحفظ: ${v}\nالآن اكتب: ${nxLabel}`
      : `✅ تم الحفظ: ${v}\nالآن أرسل: ${nxLabel} (صورة/PDF)`;
    // Issue #3: keep a submit-to-office button available throughout collection.
    _buttons = [button('review:submit'), button('service:cancel')];
    state.last_offered_buttons = _buttons.map(b => String(b.id));
  }
  await saveSession(session_id, state);
...
// File-vs-text aware "what to send" line for the start of collection. A service
// whose required items are TYPED values (phone number, email) must not be told
// to "send the files" — it should be told to TYPE the value. Mixed services get
// both instructions. (Issue #1: differentiate files vs text across surfaces.)
export function collectStartLine(docs) {
  const real = (docs || []).filter(d => !isPlaceholderDoc(d));
  if (!real.length) return 'أرسل الملفات بأي ترتيب وسأرتّبها لك.';
  if (real.every(d => typedDocKind(d))) {
    return real.length === 1
      ? `اكتب *${arabicLabelFor(real[0])}* في رسالة.`
      : `اكتب القيم المطلوبة في رسالة: ${real.map(d => arabicLabelFor(d)).join('، ')}.`;
  }
  if (real.some(d => typedDocKind(d))) {
    const typed = real.filter(d => typedDocKind(d)).map(d => arabicLabelFor(d)).join('، ');
    return `أرسل الملفات المطلوبة كصور/PDF، واكتب الحقول النصية (${typed}) في رسالة.`;
  }
  return 'أرسل الملفات بأي ترتيب وسأرتّبها لك.';
}
...
function renderChecklist(state) {
  ...
  return docs.map(d => {
    ...
    const typed = typedDocKind(d);
    // Satisfied by a FILE (storage_url) OR, for a typed field, a captured value.
    const got = !!(slot && (slot.storage_url || ((typed || slot.matched_via === 'typed') && slot.caption)));
    const label = arabicLabelFor(d) + (typed ? ' ✍️' : '');
    const mark = got ? '✅' : '⏳';
    return `${mark} ${label}`;
  }).join('\n');
}

// Render a "you're about to start this service" CONFIRMATION CARD
// shown after the citizen picks a search result. Includes everything
// they need to commit: service name + entity, fee, processing time,
// and the numbered required-documents list.
//
// Why this exists: prior to CX iter-8, picking a search result jumped
// straight to "✅ بدأت طلب X" — meaning the citizen committed without
// seeing the docs/fee/time. Real prod complaint from +96892888715 on
// 2026-05-09. Now they see this card first and tap ✓ to confirm.
//
// Plain text only (per WhatsApp rendering constraints + SYSTEM_V2).
// Renders fee + time only when present; falls back to "غير محددة"
// for time and "تُحدّد عند المراجعة" for fee. Doc list uses
// renderDocListOrPrompt() so generic rows degrade to one open prompt
// instead of "1) مستند 2) مستند".
export function renderServiceConfirmCard(row) {
  if (!row) return '';
  const name = row.name_ar || row.name_en;
  const entity = row.entity_ar || row.entity_en || '';
  const lines = [`📝 ${name}`];
  if (entity) lines.push(entity);
  // Fee
  let feeStr = null;
  if (row.fee_omr != null && Number.isFinite(Number(row.fee_omr)) && Number(row.fee_omr) > 0) {
    feeStr = `${Number(row.fee_omr).toFixed(0)} ر.ع`;
  } else if (row.fees_text && /[٠-٩0-9]/.test(row.fees_text) && /(ر\.?\s*ع|ريال|OMR\b)/i.test(row.fees_text)) {
    feeStr = String(row.fees_text).slice(0, 80);
  } else if (row.fees_text && /(مجان|بدون\s*رسوم|لا\s*يوجد)/.test(row.fees_text)) {
    feeStr = 'مجانية';
  }
  lines.push(`💰 ${feeStr || 'تُحدّد عند المراجعة'}`);
  // Time
  const time = row.avg_time_ar || row.working_time_ar || null;
  if (time) lines.push(`⏱️ ${time}`);
  // Docs
  let docs = [];
  try { docs = JSON.parse(row.required_documents_json || '[]'); enrichDocsWithArabicLabels(docs); } catch {}
  const docBlock = renderDocListOrPrompt(docs, name);
  lines.push('');
  if (docBlock.kind === 'list') {
    lines.push('📋 المطلوب منك:');
    lines.push(docBlock.text);
  } else {
    lines.push(docBlock.text);
  }
  lines.push('');
...
export function renderDocListOrPrompt(docs, serviceName) {
  ...
  if (total > 0 && real.length >= Math.ceil(total / 2)) {
    const lines = (docs || [])
      .map((d, i) => `${i + 1}) ${arabicLabelFor(d) || 'مستند'}${typedDocKind(d) ? ' — ✍️ اكتبه نصاً (بدون ملف)' : ''}`)
      .join('\n');
...
- **Citizen is UNSURE / no good match → submit_triage.** If the user explicitly doesn't know which service they need ("لست متأكد", "ما أعرف وش الخدمة المطلوبة", "I don't know which service"), OR one genuine search returns nothing relevant, do NOT guess or push a wrong service. Gather a one-line description of what they're trying to do (ask a brief clarifying question if needed), then call submit_triage(intent_summary='<their need, in Arabic>'). This dispatches the request to a Sanad office with NO service set; the office reviews the summary, sets the correct service, and continues with the citizen. Any files they already sent ride along automatically. Confirm warmly: "📨 أرسلت طلبك لمكتب سند — سيحدّد الخدمة المناسبة ويُكمل معك هنا." Prefer submit_triage over leaving an unsure citizen stuck on a search loop.
...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  // EXPERIMENT RETIRED: SANAD_DISABLE_GATES was meant to A/B the LLM-driven flow
  // by skipping the deterministic triage/greeting/thanks/out-of-scope/fee
  // shortcuts. But gates-off did NOT route those turns to the LLM — the
  // always-on deterministic SEARCH path caught them first and "answered" a
  // greeting / thanks / weather query with 6 random services (verified live in
  // the scenario loop). So the flag only broke essential UX without testing
  // anything. Forced OFF — gates always on. (Env value now ignored.)
  const GATES_DISABLED = false;  // was: process.env.SANAD_DISABLE_GATES === 'true'

  // ── TYPED equivalents of the deterministic buttons ─────────────
  // The web chat has no tappable buttons, and citizens often TYPE the choice:
  // "1" to pick a search result, "نعم" to confirm, "الغي الطلب" to cancel. The
  // search footer literally says "اضغط الرقم للبدء", yet a typed "1" used to
  // fall through to the LLM (which resolved it to a RANDOM service) and a typed
  // "الغي الطلب" got SEARCHED as a service. Normalize these into the same
  // __btn__ ids — ONLY when the current state offers that action — so the
  // deterministic button handlers fire. Runs before the gates/search (which
  // skip __btn__ messages). Numbers 1–10 only (the longest list we render is
  // the entity browse at 10), AND only when last_search_results actually holds
  // that many rows — so a 4–6 digit OTP won't match ^([1-9]|10)$ and still
  // reaches the OTP-refusal gate. Live WhatsApp bug: citizen browsed MoH's 10
  // services, typed "٧", and the bot said "service 7 doesn't exist".
  if (typeof raw === 'string' && !raw.startsWith('__btn__:') && !attachment) {
    const _digit = (raw.trim().replace(/[٠-٩]/g, d => '٠١٢٣٤٥٦٧٨٩'.indexOf(d)).match(/^([1-9]|10)$/) || [])[1];
    const _na = _normalizeAr(String(raw).normalize('NFC')).trim();
    if (_digit && Array.isArray(state.last_search_results) && state.last_search_results.length >= Number(_digit)) {
      raw = '__btn__:pick:' + _digit;
    } else if (state.pending_cancel && /^(نعم|ايوه|اكد|اكيد|احذف|موافق|تمام|اوك|ok|yes|confirm)(?:\s|$)/i.test(_na)) {
      raw = '__btn__:confirm:yes';
    } else if (state.pending_cancel && /^(لا|تراجع|رجوع|كمل|لا\s*تلغ|no|keep)(?:\s|$)/i.test(_na)) {
      raw = '__btn__:confirm:no';
    } else if (state.pending_service_id && !state.pending_cancel && /^(نعم|ايوه|ابدا|اكد|موافق|تمام|اوك|ok|yes|start)(?:\s|$)/i.test(_na)) {
      raw = '__btn__:confirm:yes';
    } else if (
      (state.service_id || state.pending_service_id || state.request_id ||
       ['collecting', 'reviewing', 'triage'].includes(state.status)) &&
      (/^(الغاء|الغي|كنسل|cancel)$/i.test(_na) ||
       /(الغاء|الغي|كنسل|cancel)\s+(هذا\s+)?(الطلب|طلبي|المعامله|الحالي|الحاليه)/i.test(_na))
    ) {
      raw = '__btn__:service:cancel';
    }
  }
  ...
  // ── Triage text trigger ────────────────────────────────────────
  // Citizen explicitly says they don't know which service they need. MUST run
  // before the greeting/help catch below — "لست متأكد من الخدمة" otherwise reads
  // as a help request and never reaches triage. Idle + no draft only, so it
  // never hijacks an active flow (e.g. "لست متأكد من رقم الهوية" mid-collection).
  // Match against the NORMALIZED text (NFC → strip tashkeel → unify alef/hamza
  // أإآ→ا, ة→ه, ى→ي, lowercase). Raw-text matching failed in prod for Arabic:
  // the incoming "متأكد" hamza form didn't byte-match the regex literal, while
  // the same string matched locally — a classic NFC/NFD + hamza mismatch. So
  // the regex uses BARE forms ("متاكد", "الخدمه") and tests the normalized input.
  // The trailing "لم أجد خدمتي" / "ما لقيت خدمتي" / "none of these" branch is the
  // escape hatch advertised in the deterministic search-results footer (req #3):
  // when none of the suggested services fit, the citizen says so and we route
  // the request to a Sanad office as an inquiry instead of dead-ending.
  const TRIAGE_TRIGGER_RE = /(?:(?:لست|مش|مو|غير)\s*متاكد|لا\s*اعرف\s*(?:الخدمه|اي\s*خدمه|ايش\s*احتاج|وش\s*احتاج)|ما\s*(?:اعرف|ادري|دري)\s*(?:الخدمه|وش|ايش|اي\s*خدمه)|مدري\s*(?:وش|ايش|الخدمه)|لم\s*اجد\s*خدمت|ما\s*لقيت\s*خدمت|not\s*sure|unsure|don\s*t\s*know\s*(?:which|what)|help\s*me\s*choose|none\s*of\s*(?:these|them|the\s*above))/i;
  // Triage is a UX safety net, NOT a shortcut that masks the LLM: when a user
  // explicitly says "I don't know which service", the office should handle it as
  // an inquiry instead of the bot dumping irrelevant matches. The gates-off A/B
  // proved Qwen does NOT triage on its own, so this gate stays ON even when
  // SANAD_DISABLE_GATES=true (same standing exemption as the OTP gate). Req #3.
  if (raw && typeof raw === 'string' && !attachment &&
      (!state.status || state.status === 'idle') && !state.service_id && !state.request_id &&
      TRIAGE_TRIGGER_RE.test(_normalizeAr(String(raw).normalize('NFC')))) {
    state.status = 'triage';
    state.intent_summary = '';
    if (!Array.isArray(state.pending_uploads)) state.pending_uploads = [];
    state.last_offered_buttons = ['service:cancel'];
    await saveSession(session_id, state);
    const reply = '👍 لا مشكلة إن لم تكن متأكداً من الخدمة المطلوبة.\n\n' +
      'اكتب لي بكلماتك ماذا تحتاج (وأرفق صورة أي ورقة لديك إن وُجدت)، ' +
      'وسأرسل طلبك إلى مكتب سند ليحدّد لك الخدمة المناسبة ويُكمل معك. 📨';
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
    trace.push({ step: 'deterministic_triage_trigger', preview: raw.slice(0, 60) });
    return { reply, state, trace, _buttons: [button('service:cancel')] };
  }

  if (!GATES_DISABLED && raw && typeof raw === 'string' && !attachment && !state.request_id &&
    ...
    // Surface the discovery shortcuts (incl. the 🤔 triage option) on a plain
    // greeting/help turn so an unsure citizen has a one-tap path into triage.
    // Skipped for "thanks" (the conversation is winding down).
    const greetBtns = (intent === 'thanks')
      ? null
      : [button('discover:license'), button('discover:cr'), button('discover:unsure')];
    if (greetBtns) {
      state.last_offered_buttons = greetBtns.map(b => b.id);
      await saveSession(session_id, state);
    }
    trace.push({ step: 'deterministic_greeting', intent });
    return { reply, state, trace, _buttons: greetBtns };
  }
  ...
  const OUT_OF_SCOPE_RE = /(?:طقس|الطقس|weather|أخبار|الاخبار|news|نكت(?:ة)?|joke|كرة\s*القدم|football|soccer|سعر\s*(?:الذهب|البتكوين|stocks?|usd|eur|دولار|عملة)|سعر\s*صرف|صرف\s*(?:ال)?(?:دولار|عمل|يورو)|الدولار|اليورو|دولار|يورو|stocks?|crypto|bitcoin|ethereum|recipe|وصفة|طبخ|كيف\s*(?:أصلي|أصوم|أحج)|capital\s*of|عاصمة)/i;
  if (!GATES_DISABLED && raw && typeof raw === 'string' && (state.status === 'idle' || !state.status) && !attachment &&
  ...
  if (!GATES_DISABLED && raw && typeof raw === 'string' && THANKS_RE.test(raw.trim()) && !attachment) {
  ...
  if (!GATES_DISABLED && raw && typeof raw === 'string' && state.status === 'idle' && !attachment &&
  ...
  if (!GATES_DISABLED && raw && typeof raw === 'string' && FEE_QUERY_RE.test(raw.trim()) &&
  ...
  if (raw && typeof raw === 'string' && state.status === 'idle' && !attachment) {
    ...
    if (FEE_OR_INFO_QUERY_RE.test(raw)) {
      ...
      if (m && m.source === 'launch' && m.code && m.service) {
        ...
        const { rows } = await db.execute({
          sql: `SELECT id FROM service_catalog WHERE name_ar = ? AND is_active=1 LIMIT 1`,
          args: [m.service.name_ar]
        });
        if (rows[0]?.id) {
          const r = await TOOL_IMPL_V2.start_submission(
  ...
  // ── Entity-listing intent (CX prod bug 2026-05-10 +96892888715) ──
  // Real prod: citizen typed "اريد الخدمات المقدمة من الشرطة" then
  // "ارسلي كل خدمات الشرطه،اللي معكم" then "فقط ٣ خدمات؟". Bot returned
  // 3 random services each time because the hybrid-search shortcut just
  // does keyword matching on "الشرطة" — yielding services that *mention*
  // police but aren't ROP services. The user's intent was to BROWSE the
  // entity's catalogue.
  // Fix: detect "all/list services of <entity>" phrasing → route to
  // TOOL_IMPL.get_entity_services which returns the full catalogue list
  // for that entity (up to 10 with pagination hint).
  if (raw && typeof raw === 'string' && state.status === 'idle' && !attachment &&
      !raw.startsWith('__btn__:')) {
    // ⚠️ DO NOT use \b in Arabic regexes — \b is a no-op between Arabic
    // chars in JS (both non-\w), so /الشرطة\b/ would never fire.
    //
    // Citizens often type ة as ه ("الشرطه" instead of "الشرطة") and skip
    // the hamza ("الاسكان" instead of "الإسكان"). normalize() handles
    // these unifications, so we match against the NORMALIZED form of raw
    // and write the regex hints in the same normalized shape (e.g.
    // "شرطه" not "شرطة"). See catalogue.js::normalize().
    const _norm = _normalizeAr(raw);
    const ENTITY_HINTS = [
      { en: 'Royal Oman Police',                                          re: /(شرطه|rop|police)/i },
      { en: 'Ministry of Health',                                         re: /(وزاره\s*الصحه|الصحه|moh|health\s*ministry)/i },
      { en: 'Ministry of Labour',                                         re: /(وزاره\s*العمل|العمل|labou?r|mol)/i },
      { en: 'Ministry of Commerce, Industry and Investment Promotion',    re: /(وزاره\s*التجاره|التجاره|commerce|moc|invest\s*easy)/i },
      { en: 'Ministry of Housing and Urban Planning',                     re: /(وزاره\s*الاسكان|الاسكان|housing|mohup)/i },
      { en: 'Muscat Municipality',                                        re: /(بلديه\s*مسقط|بلديه|muscat\s*municipality|municipality)/i },
    ];
    // Plural "خدمات" + entity hint is sufficient — verb is optional.
    // "خدمة" (singular) is a SERVICE-NAME prefix; only the plural triggers.
    const SERVICES_PLURAL = /(خدمات|services\b)/i;
    if (SERVICES_PLURAL.test(_norm)) {
      const hit = ENTITY_HINTS.find(h => h.re.test(_norm));
      if (hit) {
        try {
          const { entity, services } = await TOOL_IMPL.get_entity_services({ entity: hit.en, limit: 10 });
          if (services && services.length) {
            const isAr = /[؀-ۿ]/.test(raw);
            const totalCount = services.length;
            const listed = services.slice(0, 10);
            // Keycap emojis only combine with a SINGLE digit — "10️⃣" renders
            // broken (live WhatsApp showed item 10 as a stray "1 رفع مستوى…").
            // Map 1–9 → 1️⃣–9️⃣ and 10 → 🔟.
            const KEYCAP = ['1️⃣','2️⃣','3️⃣','4️⃣','5️⃣','6️⃣','7️⃣','8️⃣','9️⃣','🔟'];
            const lines = listed.map((s, i) => {
              const name = isAr ? (s.name_ar || s.name_en) : (s.name_en || s.name_ar);
              return `${KEYCAP[i] || `${i + 1}.`} ${name}`;
            }).join('\n');
            const more = totalCount > 10
              ? `\n\nوغيرها — اكتب اسم خدمة محدّدة لتفاصيلها.`
              : `\n\nاكتب اسم الخدمة (أو رقمها) للتفاصيل وبدء الطلب.`;
            const reply = `🏛 ${entity} — ${totalCount} خدمة:\n\n${lines}${more}`;
            state.last_search_results = listed.map(s => s.id);
            // Offer pick:1..N so a TYPED number beyond the 3 tappable buttons
            // (the citizen can pick any of the 10 listed) passes the
            // button-intent guard and maps to the right service.
            state.last_offered_buttons = listed.map((_, i) => 'pick:' + (i + 1));
            await saveSession(session_id, state);
            await storeMessage({
              session_id, request_id: null,
              direction: 'out', actor_type: 'bot', body_text: reply
            });
            trace.push({ step: 'deterministic_entity_list', entity: hit.en, count: totalCount });
            // Buttons: pick:1/2/3 for the first three so the citizen can
            // tap directly without typing.
            const btns = [];
            const NUM = ['1️⃣','2️⃣','3️⃣'];
            for (let i = 0; i < Math.min(3, listed.length); i++) {
              const name = (listed[i].name_ar || listed[i].name_en || '').replace(/^خدمة\s*(?:طلب\s*)?|^طلب\s*/, '');
              btns.push({
                id: `pick:${i + 1}`,
                title: `${NUM[i]} ${name.slice(0, 17)}${name.length > 17 ? '…' : ''}`
              });
            }
            return { reply, state, trace, _buttons: btns.length ? btns : undefined };
          } else {
            // Entity recognized but it has NO active/verified services yet
            // (e.g. MoH today — all inactive). Do NOT fall through to the
            // generic keyword search: it surfaces unrelated active services
            // (live: work/laundry permits for a Health-Ministry query). Give a
            // clean empty-state with the by-name + inquiry escape hatch.
            const reply = 'لا توجد حالياً خدمات مُفعّلة من هذه الجهة في قائمتنا.\n\n' +
              'اكتب اسم الخدمة التي تريدها بالضبط وسأبحث عنها، أو صِف حاجتك بكلماتك ' +
              'وسأحوّل طلبك إلى مكتب سند ليجهّزها لك. 📨';
            state.last_offered_buttons = ['discover:unsure'];
            await saveSession(session_id, state);
            await storeMessage({ session_id, request_id: null, direction: 'out', actor_type: 'bot', body_text: reply });
            trace.push({ step: 'deterministic_entity_list_empty', entity: hit.en });
            return { reply, state, trace, _buttons: [button('discover:unsure')] };
          }
        } catch (e) {
          trace.push({ step: 'entity_list_threw', error: e.message });
        }
      }
    }
  }
  ...
  if (raw && typeof raw === 'string' && state.status === 'idle' && !attachment &&
    ...
    try {
      ...
      // Req #4 — "suggest more than three". Pull a wider net (k=8) and show up
      // to 6 candidates so the citizen has real choice instead of 3 near-dupes.
      // Buttons still cap at WhatsApp's 3; the numbered text list carries 4–6
      // and typing the number maps via state.last_search_results.
      const { services } = await searchServices(raw, {}, { k: 8, trace });
      // Relevance trim — drop the weak tail. A focused query ("renew licence")
      // has a couple of strong matches then a steep score drop to unrelated
      // semantic-only rows; padding the list to 6 buried the right answer (live
      // WhatsApp: a licence query showed 2 licences then 4 unrelated visas).
      // Keep matches scoring within HALF the top match; never fewer than 3 (so
      // the list isn't thin) nor more than 6. Tunable via SANAD_RELEVANCE_BAND.
      const _ranked = (services || []).slice(0, 6);
      const _topScore = Number((_ranked[0] || {}).score || 0);
      const _band = Number(process.env.SANAD_RELEVANCE_BAND || 0.5);
      const _strong = _topScore > 0
        ? _ranked.filter(s => Number(s.score || 0) >= _topScore * _band)
        : _ranked;
      const top = _strong.length >= 3 ? _strong : _ranked.slice(0, 3);
      // Seeded catalogue names carry a verbose "خدمة تقديم …" prefix and a
      // redundant "(تجديد)"/"(جديدة)" tag. Strip both for DISPLAY so (a) the
      // numbered list reads cleanly and (b) the name fits WhatsApp's 20-char
      // button cap — otherwise pickButtons drops the name and falls back to a
      // bare "1️⃣/2️⃣/3️⃣" glyph that doesn't match the descriptive list (the
      // "weird / inconsistent buttons" reported live). Display-only: the pick
      // handler still maps the tapped number to the real service via its id.
      const cleanSvcName = (s) => String(s || '')
        .replace(/^خدمة\s+(?:تقديم\s+)?/, '')
        .replace(/^طلب\s+/, '')
        .replace(/^Service\s+/i, '')
        .replace(/\s*\((?:تجديد|جديدة|جديد|إصدار|اصدار)\)\s*$/, '')
        .trim();
      if (top.length >= 1) {
        ...
        // Bug from prod trace +96892888715 (2026-05-09): search results
        // for "اجدد نشاط سجل" rendered "1️⃣ إنشاء سجل تجاري جديد —
        // The applicant is not entitled to refund". The Arabic service
        // name was getting an English fees_text appended because some
        // catalogue rows have non-fee text (English section headers,
        // "Renew Commercial Registrations Access and Delivery Time")
        // stored in fees_text. Mixed-language output violates the
        // SYSTEM_V2 "one language per reply" rule. Filter to fee-shaped
        // strings only: must contain a digit AND a fee marker.
        const looksLikeFee = (s) => {
          const t = String(s || '');
          if (!t) return false;
          const hasDigit = /[٠-٩0-9]/.test(t);
          const hasFreeMarker = /(مجان(ي|اً|ا)?|بدون\s*رسوم|لا\s*يوجد\s*رسوم)/.test(t);
          const hasFeeUnit = /(ر\.?\s*ع|ريال|OMR\b|baisa|بيسة)/i.test(t);
          return (hasDigit && hasFeeUnit) || hasFreeMarker;
        };
        const fragmentFor = (row, type) => {
          if (type === 'fee' || !type) {
            if (row.fee_omr != null && Number.isFinite(Number(row.fee_omr)) && Number(row.fee_omr) > 0) {
              return ` — ${Number(row.fee_omr).toFixed(0)} ر.ع`;
            }
            if (looksLikeFee(row.fees_text)) {
              return ` — ${String(row.fees_text).slice(0, 40)}`;
            }
            return '';  // never render English garbage as a fee fragment
          }
        ...
        for (let i = 0; i < top.length; i++) {
          ...
          ids.push(row.id);
          fullRows.push(row);
          const name = cleanSvcName(row.name_ar || row.name_en);
        ...
        if (ids.length) {
          ...
          const reply =
            `${header}\n\n${lines.join('\n')}\n\n` +
            `اضغط الرقم للبدء، أو اكتب اسم الخدمة التي تريدها.\n` +
            `ولو ما لقيت خدمتك من القائمة، اكتب «لم أجد خدمتي» ` +
            `وسنحوّل طلبك إلى مكتب سند ليحدّدها لك ويُكملها. 📨`;
          await storeMessage({
            session_id, request_id: null,
            direction: 'out', actor_type: 'bot', body_text: reply
          });
          ...
          const btns = pickButtons(fullRows.slice(0, 3).map(r => ({
            ...r,
            name_ar: cleanSvcName(r.name_ar || r.name_en),
            name_en: cleanSvcName(r.name_en || r.name_ar)
          })));
  ...
  if (typeof raw === 'string' && raw.startsWith('__btn__:')) {
    ...
    const ALWAYS_OK_IDLE = new Set(['discover:license', 'discover:title', 'discover:cr', 'discover:unsure', 'pick:1', 'pick:2', 'pick:3']);
    // pick:N from idle is a tap/typed shortcut for a freshly-shown list. Honor
    // ANY N that indexes into the list we just offered (last_search_results) —
    // entity browse lists up to 10 and search up to 6, so pick:4..10 must pass
    // too. Never spoofable to escalate: it only stages a service the citizen
    // was just shown (out-of-range / no-list picks resolve to nothing).
    const _pickN = /^pick:(\d{1,2})$/.exec(btnId);
    const _pickIdleOk = !!_pickN && Array.isArray(state.last_search_results) &&
      Number(_pickN[1]) >= 1 && Number(_pickN[1]) <= state.last_search_results.length;
    const stateAllows = (
      ...
      ((state.status === 'idle' || !state.status) && (ALWAYS_OK_IDLE.has(btnId) || _pickIdleOk))
    );
  ...
  if (isGreeting && isFreshSession) {
    ...
    const reply =
      ...
      'وإن لم تكن متأكداً من الخدمة المطلوبة، اضغط *🤔 لست متأكد* وسأرسل طلبك لمكتب سند ليساعدك.';
    state.last_offered_buttons = ['discover:license', 'discover:cr', 'discover:unsure'];
    await saveSession(session_id, state);
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
  ...
  if (attachment && finalReply &&
   ...
   if (['collecting', 'reviewing'].includes(ctx.state.status)) {
    // The LLM (vision) is unavailable, so it never called record_document for
    // this file. WITHOUT deterministic assignment the citizen uploads forever
    // and no slot fills — the request can never complete during an Anthropic
    // outage. So we mirror the heuristic path: assign this attachment to the
    // next empty REQUIRED slot, in order. Caption-less files just fill slots
    // 1..N; the office still reviews every doc before accepting.
    ctx.state.collected = ctx.state.collected || {};
    const reqDocs = (ctx.state.docs || []).filter(d => !isPlaceholderDoc(d));
    const nextEmpty = reqDocs.find(d => d.code && !ctx.state.collected[d.code]);
    if (nextEmpty && attachment.url) {
      ctx.state.collected[nextEmpty.code] = {
        url: attachment.url, mime: attachment.mime, size: attachment.size,
        name: attachment.name || null, caption: null,
        matched_via: 'order_fallback', vision: null
      };
      trace.push({ step: 'doc_assigned_llm_fallback', code: nextEmpty.code });
    }
    // If every required slot is now filled, advance to review so the citizen
    // can confirm + submit even though the LLM is down.
    const allFilled = reqDocs.length > 0 && reqDocs.every(d => ctx.state.collected[d.code]);
    if (allFilled && ctx.state.status === 'collecting') ctx.state.status = 'reviewing';

    const checklist = renderChecklist(ctx.state);
    const filesIn = Object.keys(ctx.state.collected || {}).length
                  + (ctx.state.extras || []).length;
    if (allFilled) {
      finalReply = `📥 استلمت كل الملفات المطلوبة ✅\n\n${checklist || ''}\n\nاكتب *تأكيد* لإرسال الطلب لمكتب سند.`;
    } else if (filesIn > 0 && checklist) {
    ...
    trace.push({ step: 'cx_attachment_fallback_override', files_in: filesIn, all_filled: allFilled });
   } else {
    // LLM down + file(s) arrived BEFORE a service was started (idle/discovery).
    // Don't surface the scary "تعذّر الاتصال" outage error — acknowledge the
    // files and guide the citizen to name a service. drainBurst then sends this
    // single clean message instead of a connection error. (Prod report: a
    // citizen sent "تجديد بطاقة عامل" + 2 photos during an LLM hiccup and saw
    // "something went wrong, couldn't read files" before the file ack.)
    finalReply = '📥 استلمت ملفاتك. فقط أخبرني الخدمة التي تريدها (مثل: «تجديد بطاقة مقيم وعمل» أو «تجديد رخصة القيادة») وسأرفق ملفاتك بطلبك مباشرة.';
    trace.push({ step: 'cx_attachment_idle_llm_fallback' });
   }
  }
...
function attachContextualButtons({ state, finalReply, ambiguousAttachment, justBufferedThisFile, recordedRequiredThisTurn, trace }) {
  ...
  if (isLlmFallback && (!state?.status || state.status === 'idle')) {
    // No clear service match — surface the triage option FIRST so an unsure
    // citizen can dispatch their need to an office instead of guessing.
