...
async function drainBurst(session_id) {
  ...
  if (n <= 1) {
    ...
    // Multi-file burst summary already gets ✓ تم / + سأرسل المزيد buttons
    // attached below — telling the citizen to "اكتب تم" contradicts the
    // buttons. Keep the prompt short; the buttons speak for themselves.
    const followUp = '\n\nهل تبقّى ملف لم نُرفقه؟ أرسله الآن.';
