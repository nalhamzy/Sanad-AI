...
async function drainBurst(session_id) {
  ...
  if (n <= 1) {
    ...
    // THE SAME QUESTION EVERY TIME. Per the user's spec (2026-05-06):
    // never list "next required" — citizen sends in any order; the
    // buttons drive the next step.
    const sections = [headline];
    if (checklist) sections.push(checklist);
    if (extrasN > 0) {
      sections.push(`📎 ${extrasN} ${extrasN === 1 ? 'ملف مُرفق' : 'ملفات مُرفقة'} غير مرتبطة بالمطلوب.`);
    }
    if (docs.length && pending.length === 0) {
      sections.push('✨ اكتمل ملفك. اضغط *✅ اكتمل، أرسل لسند*.');
    } else {
      sections.push('هل اكتمل ملفك؟ اضغط *✅ اكتمل، أرسل لسند* أو أرسل المزيد.');
    }
    text = sections.join('\n\n');
  }
  ...
  try {
    ...
    if (typeof session_id === 'string' && session_id.startsWith('wa:')) {
      ...
      if (n >= 2) {
        ...
        await sendButtons([
          { id: 'review:submit',  title: '✅ اكتمل، أرسل لسند' },
  ...
  if (btn_id === 'burst:more') {
    ...
    if (!r) {
      ...
      const reply = checklist
        ? `✨ كل المستندات المطلوبة عندي بالفعل:\n\n${checklist}\n\nاضغط *✅ اكتمل، أرسل لسند*.`
        : '✨ كل المستندات المطلوبة عندي بالفعل. اضغط *✅ اكتمل، أرسل لسند*.';
      const _buttons = [
        { id: 'review:submit',  title: '✅ اكتمل، أرسل لسند' },
        { id: 'burst:more',     title: '➕ سأرسل المزيد' },
        { id: 'service:cancel', title: '✕ إلغاء الطلب' }
    ...
    const reply = checklist
      ? `تمام، أرسل البقية بأي ترتيب.\n\n${checklist}\n\nهل اكتمل ملفك؟ اضغط *✅ اكتمل، أرسل لسند* أو أرسل المزيد.`
      : `تمام، أرسل البقية بأي ترتيب.\n\nهل اكتمل ملفك؟`;
    const _buttons = [
      { id: 'review:submit',  title: '✅ اكتمل، أرسل لسند' },
      { id: 'burst:more',     title: '➕ سأرسل المزيد' },
  ...
  if (btn_id === 'burst:done') {
    ...
    if (state.status === 'collecting' && pending.length === 0 && haveAny) {
      ...
      const reply =
        `📦 *جاهز للمراجعة*\n\n${checklist || ''}` +
        (extras ? `\n\n📎 ملفات مُرفقة: ${extras}` : '') +
        `\n\nاضغط *✅ اكتمل، أرسل لسند* لإرسال الطلب إلى مكاتب سند المتاحة.`;
      const _buttons = [
        { id: 'review:submit',  title: '✅ اكتمل، أرسل لسند' },
        { id: 'burst:more',     title: '➕ سأرسل المزيد' },
        { id: 'service:cancel', title: '✕ إلغاء الطلب' }
  ...
  if (btn_id === 'doc:list') {
    ...
    if (!r) {
      ...
      const reply = checklist
        ...
        : '✨ كل المستندات المطلوبة عندي. اضغط *✅ اكتمل، أرسل لسند*.';
      await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
      return {
        reply, state, trace,
        _buttons: [
...
- Any collecting/reviewing reply → unified **✅ اكتمل، أرسل لسند / ➕ سأرسل المزيد / ✕ إلغاء الطلب** buttons
...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
    ...
    messages.push({
      ...
      content:
        ...
        (unmatchedList.length
          ...
          : '') +
        (remaining.length
          ? `  • Final line — ask "is your file complete?". DO NOT name a "next" doc. e.g. "Is your file complete?" / "هل اكتمل ملفك؟ اضغط *✅ اكتمل، أرسل لسند* أو أرسل المزيد."\n`
  ...
  if (
    ...
    if (pending.length === 0) {
      ...
      body = checklist
        ? `${savedLine}\n\n${checklist}\n\n✨ ✨ اكتمل ملفك. اضغط *✅ اكتمل، أرسل لسند*.`
        : `${savedLine}\n\n✨ ✨ اكتمل ملفك. اضغط *✅ اكتمل، أرسل لسند*.`;
    } else {
