...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  if (raw && typeof raw === 'string' && state.status === 'idle' && !attachment &&
    ...
    try {
      ...
      // Relevance floor (req #3). The RRF score cleanly separates a real catalogue
      // match (renew-licence / work-visa ≈ 0.033–0.049) from a query with no true
      // match (medical exam / "play football" ≈ 0.016–0.027 — measured live against
      // the seeded catalogue). When even the best hit is weak we must NOT claim "N
      // services that suit you" — reframe the header honestly and lead the
      // «لم أجد خدمتي» → office escape. Results are never suppressed (the citizen
      // still sees + can tap the closest); only the false confidence is removed.
      // Env-tunable so the threshold can be retuned live without a code change.
      const RELEVANCE_FLOOR = Number(process.env.SANAD_RELEVANCE_FLOOR || 0.03);
      const _bestScore = Number((services && services[0] && services[0].score) || 0);
      const lowConfidence = !(services && services.length) || _bestScore < RELEVANCE_FLOOR;
      if (top.length >= 1) {
        ...
        if (ids.length) {
          ...
          const header = lowConfidence
            ? `🤔 ما لقيت خدمة مطابقة تماماً لطلبك — هذه أقرب الخدمات. وإن لم تناسبك أيٌّ منها، اكتب «لم أجد خدمتي» وأحوّل طلبك لمكتب سند:`
            : (qType
          ...
          await saveSession(session_id, state);
          trace.push({ step: 'deterministic_search_results', count: ids.length, ids, q: qType, lowConfidence, bestScore: _bestScore });
