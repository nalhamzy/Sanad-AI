...
async function drainBurst(session_id) {
  ...
  if (n <= 1) {
    ...
    const headline = cur.kind === 'replacement'
      ? `✅ استلمت ${n} ${wordFile} محدَّثة وأرسلتها إلى مكتب سند للمراجعة.`
      : `✅ استلمت ${n} ${wordFile} وأرسلتها إلى المكتب المتولّي طلبك.`;
    const followUp = '\n\nهل تبقّى عندك ملف لم نُرفقه بعد؟ أرسله الآن. إذا انتهيت، اكتب **تم** أو **خلصت** ونتابع.';
    text = headline + followUp;
  }
...
Most citizens are on **WhatsApp**. The webhook layer auto-attaches **quick-reply buttons** to your reply when it detects a yes/no question — citizens TAP, they don't type. This means:

- **NEVER write "type نعم أو لا" / "اكتب نعم/لا" / "type yes/no" / "reply with X or Y"**. The buttons appear on their own; that instruction is wrong AND makes the reply look broken.
- End yes/no questions naturally — *"نتابع؟"*, *"هل تؤكد؟"*, *"نُرسل الملف؟"* — and stop. The wrapper handles the buttons.
- For free-form answers (describe a file, give a name) it's fine to write "اكتب ..." since no buttons fit.
...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  if (startSubmissionResult) {
    ...
    const svcName = (r.service_name_ar || r.service_name_en || r.service_code || 'الخدمة').toString();
    ...
    const docList = docs.length
      ? '\n\n📋 *المستندات المطلوبة* (' + docs.length + '):\n' +
        docs.map((d, i) => `${i + 1}. ${d.label_ar || d.label_en || d.code}`).join('\n')
      : '\n\n📋 لا توجد مستندات مطلوبة لهذه الخدمة.';
    const firstDoc = r.first_doc;
    const firstAsk = firstDoc
      ? `\n\n📎 جهّز الملفات وابدأ بإرسال: *${firstDoc.label_ar || firstDoc.label_en || firstDoc.code}*`
  ...
  const HALLUCINATED_SAVE_RE =
    /✅\s*(?:حفظت|تم\s*الحفظ|تم\s*التسجيل|saved|recorded|stored)|(?:^|\s)(?:حُفِظ(?:ت|ا)?|سجّلت|سُجّل)(?:\s|$|[:.،])/i;
  ...
  // Quick-reply buttons for known answerable cases — saves the citizen from
  // typing نعم / لا / إضافي in Arabic on a phone keyboard. Drained via the
  // burst layer so a multi-file batch still collapses to one message; this
  // kicks in for the SOLO-file turns where the bot is asking a clarification.
  //
  // Two cases attach the same 3-button quick-reply set (mapped to canonical
  // text tokens by routes/whatsapp.js — no agent state-machine changes):
  //   1. ambiguousAttachment — caption present but no slot matched + vision
  //      gave a best-guess.
  //   2. attachment landed in pending_uploads (no caption, vision unsure):
  //      the bot's reply is asking the citizen to describe / classify it.
  //      This is the CASE the user-reported trace was hitting most.
  let _buttons = null;
  const justBufferedThisFile = !!(attachment && state.pending_uploads?.some(p => p.url === attachment.url));

  // CASE 1 — doc-classification 3-button (specific): bot is asking
  // "is this for [slot X]?" after an ambiguous or buffered upload.
  if (finalReply && (ambiguousAttachment || justBufferedThisFile)) {
    const next = ambiguousAttachment?.next_pending
      || (state.docs || []).find(d => !state.collected?.[d.code]);
    const vBest = ambiguousAttachment?.vision_best;
    const guessSlot = vBest
      ? (state.docs || []).find(d => d.code === vBest.code)
      : null;
    const slotForYes = guessSlot || next;
    const slotLabel = (slotForYes?.label_ar || slotForYes?.label_en || 'هذا المستند').slice(0, 14);
    _buttons = [
      { id: 'doc:yes',   title: `✓ نعم — ${slotLabel}`.slice(0, 20) },
      { id: 'doc:wrong', title: '🔄 ملف آخر' },
      { id: 'doc:extra', title: '📎 إضافي' }
    ];
    trace.push({
      step: 'attached_buttons',
      case: ambiguousAttachment ? 'ambiguous_doc' : 'buffered_no_caption',
      count: 3,
      yes_slot: slotForYes?.code || null
    });
  }
  // CASE 2 — generic yes/no (fallback): if no specific buttons fired AND
  // the reply is asking the citizen for a yes/no confirmation, attach
  // generic confirm:yes / confirm:no so they tap instead of typing. Catches
  // service confirms ("هل تؤكد البدء؟"), submit prompts ("نُرسل الملف؟"),
  // explicit "type yes/no" instructions ("اكتب نعم أو لا"), etc. Skip if
  // CASE 1 already fired so we don't shadow the richer doc-specific UX.
  if (!_buttons && finalReply && looksLikeYesNoAsk(finalReply)) {
    _buttons = [
      { id: 'confirm:yes', title: '✓ نعم' },
      { id: 'confirm:no',  title: '✕ لا' }
    ];
    trace.push({ step: 'attached_buttons', case: 'generic_yes_no', count: 2 });
  }
...
function looksLikeYesNoAsk(reply) {
  ...
  return (
    ...
    /(?:^|\s)هل\s+(?:تريد|ترغب|تؤكد|تأكد|توافق|تكمل|نكمل|نتابع|نُرسل|نرسل|نبدأ|أبدأ|أحفظ|نحفظ|تحب|تود|تكتفي)/i.test(t) ||
    // English question patterns
    /\b(?:do you want|would you like|shall i|should i|are you sure|confirm)\b.*\?/i.test(t) ||
    // Trailing yes/no-style question prompts
    /(?:^|\s)(?:نتابع|نُرسل|نرسل|نبدأ|أبدأ|نُجهّز|نجهز|نُكمل|نكمل|نُؤكد|نؤكد)\s*[؟?]\s*$/m.test(t) ||
    /(?:submit|continue|proceed|confirm|start)\s*\?\s*$/im.test(t)
  );
}
