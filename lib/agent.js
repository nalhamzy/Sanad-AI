...
async function handleButtonIntent({ session_id, state, btn_id, attachment, citizen_phone, trace }) {
  ...
  // codex iter-7: discovery shortcuts surfaced when the LLM-error
  // fallback fires in idle state. Pure UX hint — we don't start the
  // submission yet (that requires LLM tool-call to start_submission with
  // the proper service_id + doc list); we just echo the service name as
  // a typed query so the next turn's LLM call (or the citizen typing
  // a follow-up) routes correctly.
  const DISCOVERY = {
    'discover:license': 'تجديد رخصة القيادة',
    'discover:title':   'بدل فاقد سند ملكية',
    'discover:cr':      'إصدار سجل تجاري'
  };
  if (DISCOVERY[btn_id]) {
    const phrase = DISCOVERY[btn_id];
    const reply = `🔍 *${phrase}* — لإنشاء الطلب اكتب رسالة قصيرة تقول "${phrase}" وسأبدأ معك خطوة بخطوة.`;
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
    trace.push({ step: 'btn_discovery_hint', service: phrase });
    return { reply, state, trace };
  }
  ...
  if (btn_id === 'confirm:yes' && state.pending_cancel) {
    ...
    if (wasInFlight) {
      ...
      return {
        ...
        _buttons: [
          { id: 'confirm:yes', title: '🔁 حاول الإلغاء مجدداً' },
          { id: 'status:check', title: '📊 حالة الطلب' }
        ]
      };
    }
...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  trace.push({ step: 'runAgentV2_entered', build_marker: 'iter15-diag-v2' });
      ...
      if (m && m.source === 'launch' && m.service?.name_ar) {
        ...
        if (svc) {
          ...
          return {
            reply, state, trace,
            _buttons: [
              { id: 'discover:license', title: '🚗 رخصة قيادة' },
              { id: 'discover:title',   title: '📑 سند ملكية' },
              { id: 'discover:cr',      title: '🏢 سجل تجاري' }
            ]
          };
        }
      }
  ...
  trace.push({ step: 'service_match_pre_check', raw_len: (raw||'').length, raw_type: typeof raw, status: state?.status, has_attachment: !!attachment });
  if (raw && typeof raw === 'string' && state.status === 'idle' && !attachment) {
    trace.push({ step: 'service_match_block_entered', raw_preview: raw.slice(0, 50) });
    ...
    if (FEE_OR_INFO_QUERY_RE.test(raw)) {
      ...
      trace.push({ step: 'service_match_calling_matchservice' });
      const m = await matchService(raw, { trace, useHybrid: false });
      trace.push({ step: 'service_match_result', source: m?.source, code: m?.code, name_ar: m?.service?.name_ar });
      if (m && m.source === 'launch' && m.code && m.service) {
        ...
        trace.push({ step: 'service_match_catalog_lookup', name_ar: m.service.name_ar, found: !!rows[0]?.id });
        if (rows[0]?.id) {
          ...
          if (r?.ok) {
            await saveSession(session_id, state);
            const docList = (r.required_documents || [])
              .map((d, i) => `${i + 1}) ${d.label_ar || d.label_en || d.code}`)
              .join('\n');
            const reply =
              `✅ بدأت طلب *${r.name_ar || r.name_en}*.\n\n` +
              `الملفات المطلوبة:\n${docList}\n\n` +
  ...
  if (isGreeting && isFreshSession) {
    // Trimmed to ≤200 chars (codex iter-3, 2026-05-08): the long
    // intro front-loaded too much marketing copy on the first turn.
    // Citizen-perspective: just tell me what to type.
    const reply =
      'أهلاً 👋 أنا *ساند* — جاهز أُجهّز معاملتك الحكومية وأرسلها لمكتب سند للتنفيذ.\n\n' +
      'اكتب اسم الخدمة (مثلاً: *تجديد رخصة القيادة* / *إصدار سجل تجاري*) أو وصف ما تحتاجه.';
    await saveSession(session_id, state);
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
