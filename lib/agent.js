...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  // EXPERIMENT (SANAD_DISABLE_GATES=true): skip the deterministic LLM-bypass
  // shortcuts (triage / greeting / thanks / out-of-scope / fee) and send those
  // turns to the LLM instead — for A/B-ing the full LLM-driven flow (e.g. on
  // Qwen). The OTP safeguard and slash commands are NOT gated. Default: gates ON.
  const GATES_DISABLED = process.env.SANAD_DISABLE_GATES === 'true';
