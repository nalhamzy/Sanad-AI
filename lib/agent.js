    ...
    try {
      ...
      if (result?.ok) {
        ...
        const partial = haveCount < docs.length
          ? `\n\n*ملاحظة:* تبقّى ${docs.length - haveCount} مستند${docs.length - haveCount === 1 ? '' : 'ات'} في القائمة. سيتواصل معك المكتب المتولّي إن احتاج إليها.`
          : '';
        const reply =
          `📤 *أرسلت طلبك إلى مكاتب سند المرخّصة.*\n\n` +
          `🆔 رقم الطلب: *#R-${result.request_id}*\n` +
          `📦 المرفقات: ${total} ${fileWord}` +
          partial +
          `\n\nالمكاتب ستراجع الطلب وترسل عروضها قريباً. سأخبرك عند وصول أول عرض.`;
        await storeMessage({
