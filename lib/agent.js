...
async function drainBurst(session_id) {
  ...
  if (n <= 1) {
    ...
    if (cur.kind === 'replacement') {
      ...
      // No request yet — files arrived but not all of them necessarily
      // landed in required slots. Honest wording: "received" not "saved
      // into the right slots". The detail (which slot got which) is in
      // the per-file replies that runAgentV2 produced and overwrote into
      // cur.lastReply — we can't reconstruct it here. The burst:done /
      // burst:more buttons let the citizen drive the next step.
      headline = `✅ استلمت ${n} ${wordFile}.`;
    }
    const followUp = '\n\nاضغط *✓ انتهيت* لمتابعة الطلب، أو أرسل بقية الملفات الآن.';
...
async function handleButtonIntent({ session_id, state, btn_id, attachment, citizen_phone, trace }) {
  ...
  if (btn_id === 'burst:more') {
    ...
    if (!r) {
      ...
      const reply = '✅ كل المستندات المطلوبة عندي بالفعل. اضغط *📤 أرسل للمراجعة* لإنهاء الطلب.';
      const _buttons = [
    ...
    const reply =
      `📋 *المستندات المتبقية* (${r.count}):\n\n${r.lines}\n\n` +
      `📎 ابدأ بإرسال: *${arabicLabelFor(r.first)}*`;
    const _buttons = [
  ...
  if (btn_id === 'burst:done') {
    ...
    if (state.status === 'collecting' && pending.length === 0 && haveAny) {
      ...
      state.status = 'reviewing';
      await saveSession(session_id, state);
      const summary = (state.docs || [])
        .filter(d => !isPlaceholderDoc(d))
        .map(d => `✅ ${arabicLabelFor(d)}`)
        .join('\n');
    ...
    if (state.status === 'collecting' && pending.length > 0) {
      // Still missing required docs — do NOT advance, list what's missing.
      const r = renderRemaining();
      const reply =
        `⏸ لا يمكنني الإرسال للمراجعة بعد — تبقّى ${r.count} مستند${r.count === 1 ? '' : 'ات'}:\n\n${r.lines}\n\n` +
  ...
  // doc:list — citizen wants to see what's left
  if (btn_id === 'doc:list') {
    ...
    if (!r) {
      state.status = 'reviewing';
      await saveSession(session_id, state);
      const reply = '✅ كل المستندات المطلوبة عندي. اضغط *📤 أرسل للمراجعة*.';
      await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
      return {
        reply, state, trace,
        _buttons: [
    ...
    const collected = state.collected || {};
    const collectedLines = (state.docs || [])
      .filter(d => collected[d.code] && !isPlaceholderDoc(d))
      .map(d => `✅ ${arabicLabelFor(d)}`)
      .join('\n');
    const reply =
      (collectedLines ? `*استلمت:*\n${collectedLines}\n\n` : '') +
      `📋 *المتبقي* (${r.count}):\n${r.lines}\n\n` +
      `📎 أرسل الآن: *${arabicLabelFor(r.first)}*`;
    await storeMessage({ session_id, direction: 'out', actor_type: 'bot', body_text: reply });
    return {
...
async function runAgentV2({ session_id, state, raw, attachment, citizen_phone, trace }) {
  ...
  if (
    ...
    let body;
    if (pending.length === 0) {
      body = `${savedLine}\n\n📦 اكتمل ملفك. اضغط *📤 أرسل للمراجعة* لإرسال الطلب إلى مكاتب سند.`;
    } else {
      const nextLabel = arabicLabelFor(pending[0]);
      const remainCount = pending.length;
      body =
        `${savedLine}\n\n` +
        `📎 التالي: *${nextLabel}* — أرسله الآن.\n` +
        `🔢 المتبقي: ${remainCount} مستند${remainCount === 1 ? '' : 'ات'}.`;
    }
    trace.push({
