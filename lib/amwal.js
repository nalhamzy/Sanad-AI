...
import { THAWANI_ENABLED, createThawaniSession } from './thawani.js';
...
// Multi-provider dispatcher. Picks Thawani when its keys are set (preferred
// for new pilots — modern hosted checkout, OMR-native, sandbox available),
// then Amwal, then a local stub. Returns the same { url, merchantReference,
// amwalOrderId, ... } shape regardless so call sites don't change.
export async function createPaymentLink({ amountOmr, merchantReference, customerEmail, description, publicBase }) {
  if (THAWANI_ENABLED) {
    const out = await createThawaniSession({
      amountOmr, merchantReference, customerEmail, description,
