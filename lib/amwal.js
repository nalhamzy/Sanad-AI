...
export async function createPaymentLink({ amountOmr, merchantReference, customerEmail, description }) {
  if (!AMWAL_ENABLED) {
    // Stub mode: the client redirects to our own endpoint that fakes success.
    // Request payments use a different stub path so they redirect to the
    // citizen's tracking page instead of the office payment page. We detect
    // them by the `req…` prefix the officer route uses (newMerchantRef('reqN')).
    const isRequestPayment = /^req\d+/i.test(merchantReference);
    const path = isRequestPayment ? '/api/payments/_stub/request_pay' : '/api/payments/_stub/pay';
    return {
      url: `${PUBLIC_BASE}${path}?ref=${encodeURIComponent(merchantReference)}`,
      merchantReference,
