...
export async function sweepExpiredSLA() {
  ...
  for (const r of post) {
    ...
    if (upd.rowsAffected) {
      ...
      // Notify the citizen — their request is being re-assigned.
      const { rows: ctx } = await db.execute({
        sql: `SELECT session_id FROM request WHERE id=?`, args: [r.id]
      });
      if (ctx[0]?.session_id) {
        try {
          const { storeMessage } = await import('./agent.js');
          await storeMessage({
            session_id: ctx[0].session_id,
            request_id: r.id,
            direction: 'out',
            actor_type: 'system',
            body_text: '⏳ مكتب سند المسؤول لم يُكمل في الوقت المحدد. طلبك المدفوع يُحوَّل تلقائياً إلى مكتب آخر — لن تدفع مرّة أخرى. سنُخطرك فور الاستلام.'
          });
        } catch {}
      }
    }
