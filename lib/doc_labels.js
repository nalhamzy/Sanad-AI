...
// ─── Verified label_en → Arabic map ────────────────────────
// 1,492 unique document labels translated EN→AR by a multi-agent workflow
// (translate + independent verification pass), using Omani government
// terminology. Keyed by the whitespace-collapsed English label. This is the
// authoritative, comprehensive layer — it covers 100% of catalogue docs (all
// have label_en), so the heuristic is now only a defensive last resort.
let LABEL_AR_BY_EN = {};
try {
  const _dir = dirname(fileURLToPath(import.meta.url));
  LABEL_AR_BY_EN = JSON.parse(readFileSync(join(_dir, 'doc_labels_ar.json'), 'utf8'));
} catch { /* file optional — falls back to curated dict + heuristic */ }

const _normEn = (s) => String(s || '').replace(/\s+/g, ' ').trim();
...
export function arabicLabelFor(doc) {
  ...
  // Verified EN→AR map (comprehensive, covers all catalogue docs). Keyed by the
  // whitespace-collapsed English label.
  const en = _normEn(doc.label_en);
  if (en && LABEL_AR_BY_EN[en]) return LABEL_AR_BY_EN[en];
