...
async function likeSearch(query, candidateIds, limit = 50) {
  // Tokens are NORMALIZED (normalize(): إأآٱ→ا, ى→ي, ة→ه, ؤ→و, ئ→ي). The
  // search_blob is only LOWERCASED — NOT normalized — so a normalized token
  // never LIKE-matched it (user's "تاشيرة"/ا vs catalogue "تأشيرة"/أ, or
  // "رخصة"/ة). That silently killed the lexical lane for most Arabic service
  // words → an undifferentiated semantic-only list (every row ~0.016, noisy).
  // Fix: normalize the COLUMN inline with the SAME letter map so the match is
  // spelling-agnostic. (Active set is small, so per-row REPLACE is cheap.)
  const tokens = normalize(query).split(/\s+/).filter(t => t.length >= 3).slice(0, 8);
  if (!tokens.length) return [];
  const N = (c) => `REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LOWER(COALESCE(${c},'')),'إ','ا'),'أ','ا'),'آ','ا'),'ٱ','ا'),'ى','ي'),'ة','ه'),'ؤ','و'),'ئ','ي')`;
  const tokExpr = `(${N('name_en')} LIKE ? OR ${N('name_ar')} LIKE ? OR ${N('search_blob')} LIKE ?)`;
