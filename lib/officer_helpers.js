...
import { isWindowOpen, enqueuePendingWa, sendKnockTemplate } from './wa_templates.js';
...
/**
 * The phone we can actually deliver an office→citizen WhatsApp message to:
 *  • WhatsApp session → the phone IS the session id ("wa:<phone>").
 *  • Web session      → only once the citizen verified a phone via OTP
 *                       (citizen_phone). This is what lets a request applied
 *                       from the WEB app still reach the citizen on WhatsApp.
 * Returns null when no deliverable phone is known (web citizen, pre-OTP).
 *
 * Use this for EVERY office→citizen WhatsApp send so the channel decision is
 * "do we know their phone?" — never "did they happen to start on WhatsApp?".
 */
export function deliverablePhone({ session_id, citizen_phone } = {}) {
  if (isWhatsAppSession(session_id)) {
    return citizen_phone || String(session_id || '').replace(/^wa:/, '') || null;
  }
  return citizen_phone || null;
}

/**
 * Notify the citizen: persist the outbound message, then push to WhatsApp
 * when a phone is known (WhatsApp session OR web citizen who verified via OTP).
 * WA delivery is best-effort — failures are logged (was: silent
 * `.catch(() => {})`) so we can spot outages.
 *
 *   await notifyCitizen({
...
export async function notifyCitizen({
  ...
  meta = null, citizen_phone = null, media = null, template = null
}) {
  await storeMessage({
    session_id,
    request_id,
  ...
  // Resolve a deliverable WhatsApp number (WhatsApp session, or a web citizen
  // who verified a phone via OTP — issue #1). Delivery is best-effort: Meta's
  // 24h customer-care window means a free-form text only lands if the citizen
  // messaged us recently; the in-app thread above is always the source of truth.
  // Returns the WhatsApp delivery outcome so callers (and the office UI) can
  // SEE whether the message/file actually reached the citizen's phone — the
  // in-app thread above is always written regardless.
  const out = { stored: true, wa_attempted: false, text: null, media: null };
  let phone = deliverablePhone({ session_id, citizen_phone });
  if (phone) {
    phone = canonicalPhone(phone) || phone;
    out.wa_attempted = true;
    // Meta only allows free-form messages/media within 24h of the citizen's last
    // inbound. Inside the window → send directly. Outside → queue the real
    // content + send an approved TEMPLATE ("you have an update — reply"); the
    // webhook flushes the queue when the citizen next replies. The in-app thread
    // above is written regardless (طلباتي always has it).
    let windowOpen = true;
    try { windowOpen = await isWindowOpen(session_id); } catch { windowOpen = true; }
    if (windowOpen) {
      try {
        if (body) out.text = await sendWhatsAppText(phone, body);
        if (media?.link) {
          out.media = await sendWhatsAppDocument(phone, media.link, {
      ...
      out.deferred = true;
      try {
        await enqueuePendingWa(session_id, phone, { body, media });
        out.template = await sendKnockTemplate(phone, template?.kind || 'update', template?.param);
      } catch (e) {
        console.warn(`[notifyCitizen] defer/template failed for ${session_id}:`, e?.message || e);
        out.error = e?.message || String(e);
      }
    }
  }
