...
// DEBUG_ENABLED is hard-off in production, so the CSRF guard can never be
// silently disabled on a real deployment even if DEBUG_MODE leaks into the
// prod env. Local dev (DEBUG_ENABLED=true) still bypasses so curl/tooling
// works without origin headers.
